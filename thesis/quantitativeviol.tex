...
To tackle this limitation, it is necessary to transition from a boolean verdict to a \emph{quantitative semantics}, changing the emphasis from the question ``Did a violation occur?'' to ``How many violations occurred, and how much each agent contributed to those violations?''
...
\begin{itemize}
    \item \textbf{Step 1 ($A_1$):} The tenant pays. The instance of $C_3$ for Month 1 is discharged. Due to the repetition operator, the residual is $\emptc; \repit{C_3} \equiv \repit{C_3}$. The contract ``resets'' for the next month.
...
\begin{claim}[Finiteness of Reachable Residual Contracts]\label{clm:rrcs-finite}
  ...
  \vspace{-2em}
  \paragraph{Intuition.}
The claim follows from the finite syntactic structure of $C$ and the progression function $\Prog$. The function $\Prog$ rewrites contracts using only sub-contracts of the contract, residuals of regular expressions, and the contract terminal state $\emptc$. That is, no new syntactic contract is generated beyond its components and recursive applications of the progression function always terminate without exploring all possible traces.
\input{qviol}