...
Despite being computationally efficient for stopping compliance systems, such as when a safety-critical property is failing, this ``first-fail'' approach is insufficient for post-hoc auditing or dispute resolution.
...
\end{cases}
\]
If $C$ is discharged by step $A$ (i.e., its residual is $\emptc$), the monitor immediately activates the first step of the next portion $C'$.
...
After establishing the evolution of single-step reparations, the analysis is extended to still open-ended contracts. The following example demonstrates how the \CPM handles infinite streams with recurring duties, showing how violations in one period persist into subsequent periods.
...
\begin{claim}[Finiteness of Reachable Residual Contracts]\label{clm:rrcs-finite}
  ...
  \vspace{-2em}
  \paragraph{Intuition.}
The claim follows from the finite syntactic structure of $C$.  The progression function $\Prog$ rewrites contracts using only sub-contracts of the contract, residuals of regular expressions, and the contract terminal state $\emptc$. No new syntactic contract is generated beyond its components.
\input{qviol}