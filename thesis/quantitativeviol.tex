...
Let $C$ be a contract from \cDL, The \emph{reachable residual contract set} of $C$, written \rrcs{C}, is the set from $2^{\cDL \cup \emptc}$ capturing all the possible residual contracts obtainable from a contract $C$ by any trace $\pi \in \Gamma^*$ using the progress function
\[\rrcs{C} := \{ C_r \mid \exists \tau \in \Gamma^*: \Prog(\pi,C)= C_r\}.\]
\end{definition}
