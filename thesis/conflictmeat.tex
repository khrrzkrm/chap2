...
The previous sections established the semantics of \TDDLfm and the satisfiability problem on the logic as well as the construction of normal forms. We now address how to formally capture \emph{conflicts} in temporal-deontic specifications.

Our motivation, grounded in practical use cases, is that deontic conflicts materialize at \emph{specific time points} that must be isolated and explained. The goal is not only to show that a specification is inconsistent, but to determine \emph{where}, \emph{when}, and \emph{why} inconsistency arises. We first define punctual ontic and deontic conflicts as atomic patterns.  Then, we demonstrate a $\dpnf$ transformation to explicit punctual conflict and introduce syntax-aware invariants named \LUCO and \LAP that prove the correctness of the conflict detection under the \dpnf rewriting, and finally develop a systematic procedure and quantitative measure that detect and classify all punctual conflicts that a formula \emph{has}.



\subsection{Syntactic Definitions for Conflicting Formulas}
To reason about conflicts in \TDDLfm, we first need a precise vocabulary that identifies what constitutes a conflict and when a formula specifies a conflict. We therefore begin by isolating the most elementary patterns that materialize at individual time points. These punctual patterns form the atomic units from which all further reasoning about conflicts in formulas \emph{has or specifies}.
...
\begin{itemize}
    \item A \defn{punctual deontic conflict}, written (\textbf{P-Deon-Conf}), is a  conjunction of obligation and a prohibition that prescribe and forbid the same action at the same time:
    \[
...
These patterns are purely syntactic formulas of \TDDLfm. Whenever they appear as sub-terms of a product term composed of punctual literals (modulo commutativity and associativity), a conflict is present at that time point. A formula may contain one or several such conflicts as sub-formulas, and a single literal may participate in several conflicts.

\begin{example}[Punctual conflicts appear as sub-formulas]
We illustrate three formulas in which conflicts occur as sub-terms. Underlined sub-terms indicate punctual conflicts.
...
The formula $\phi_2$ contains two punctual conflicts at $t=5$: one ontic and one deontic.
...
\Big(
  ...
  \, \sqcap\,
  \tikzmark{bpick}\ob{\{[3,3]\}}{\pickU\tikzmark{epick}}
\Big)
$$
...
$\{\delAtPark,\pickU\}$, and
$\{\delAtGate,\pickU\}$.
As $\ob{\{[2,2]\}}{\delAtPark}$ is present in all conflicts, removing this literal alone is enough to resolve all conflicts in the formula.
\end{example}

\subsubsection{Why Punctual Conflicts Are Not Enough}
In real-life normative specification, the clauses rarely talk about a conjunction of punctual literals, but they use intervals. For example:
...
The other direction is also possible; conflict interaction is hidden by disjunction and does not manifest as a single product term. For instance,
\[
...
At $t=1$, the first disjunct exposes a punctual conflict. 
...
We demonstrate equivalences by erasing or introducing conflicts using the absorption properties of the disjunction of an unsatisfiable formula with any other formula, as shown in Lemma~.\ref{empty_inter_union}.
\begin{inparaenum}[(i)]
\item \textbf{Erasing a genuine conflict.}
\[
  \phi := (\ob{\{[2,2]\}}{a} \sqcap \forb{\{[2,2]\}}{a}) \;\sqcup\; \phi'
\]
is equivalent to $\phi'$,  equivalence would falsely conclude that $\phi$ has no conflict.
...
To avoid such pathologies, conflict extraction must respect the formula's syntactic structure.
...
We introduce two invariants that govern how transforming literal behave in conflict detection: (i)  \emph{Lowest Upper Common Operator (\LUCO):} preserves the Boolean position where literals meet; and (ii) \emph{Literal Ancestor Preservation (\LAP):} ensures punctual descendants refine—but never alter—their ancestor literals.

These invariants ensure that conflicts are not introduced nor hidden by the  \dpnf normalization.
...
\begin{enumerate}[(i)]
    ...
    \item no descendant of $v$ satisfies \emph{(i)} and \emph{(ii)}.
\end{enumerate}
We write $\LUCO_\phi(\ell_i,\ell_j)=\sqcap$ (resp.\ $=\sqcup$) to denote the common operator label.
\end{definition}
...
This invariant ensures that normalization only refines the syntactic representation of literals without changing their specified time points, deontic operator, or action. For the specified time points, especially for this invariant, no time point is introduced and no time point is removed.
...
\end{enumerate}
\end{definition}
With these invariants in hand, we can prove the correctness of our conflict detection in the next subsection.
...
\begin{enumerate}[(i)]
    ...
    \item $\phi'$ is syntactically formed by a puntcual conflict.
\end{enumerate}
\end{definition}
...
\noindent
 According to our definition, $\phi$ specifies the punctual deontic conflict at time point 2, $\ob{\{[2,2]\}}{a} \sqcap \forb{\{[2,2]\}}{a}$ as:
\begin{enumerate}[(i)]
  \item \textbf{Semantic equivalence.}  
  $\phi'$ is the punctual normal form of $\phi$:
  \[
    \ob{\{[2,3]\}}{a} \equiv \ob{\{[2,2]\}}{a} \sqcup \ob{\{[3,3]\}}{a}, \qquad
    \forb{\{[2,4]\}}{a} \equiv \forb{\{[2,2]\}}{a} \sqcap \forb{\{[3,4]\}}{a}.
  \]
  These equivalences matches exactly $\phi'$, hence $\phi \equiv \phi'$.
  ...
  \begin{itemize}
    \item $\ob{\{[2,2]\}}{a}$ and $\ob{\{[3,3]\}}{a}$ are descendants of $\ob{\{[2,3]\}}{a}$, and their interval sets are contained within in $[2,3]$.
  ...
  The first disjunct of $\phi'$ is syntactically formed by a punctual deontic conflict.
  \[
...
\noindent
Therefore, $\phi$ \emph{has} a conflict according to Definition~\ref{def:has-conflict}, witnessed by the rewriting $\phi'$.
\end{example}
...
We prove that the transformation into \dpnf\ preserves semantics, and we establish 
...
This ensures that every punctual conflict present in a formula can be decidably detected, 
...
We now define a new normal form specific to our logic to exhibit punctual conflict by decomposing literals into punctual literals.
...
\begin{example}
Conisder $\ob{\{[2,4],[6,8]\}}{a}$, applying \textbf{RightPunctOb} results in 
\[
\ob{\{[2,4],[6,8]\}}{a}
\to
...
Repeated application of these rules introduces new punctual conflicts, eventually reaching their upper bounds. However, prohibitions may include infinite intervals (e.g.\ $[k,\infty]$) leading to an infinite chain of repetition. As conflicts only occur between obligation and obligation and Prohibition and obligation, to ensure a good enough enrollment, we define a bounded variant.
...
We, for now, manually choose  a bound \(k\), for illustration purposes. Later we will show how to  choose it depending on the formula.
...
\end{align*}
No further rule applies beyond \(k=16\), producing the punctual normal form up to the bound.
\end{example}
...
\end{cases}
\]
where $\is\ominus[k+1,\infty]$ restricts $\is$ to time points $\le k$.
\end{definition}
...
For any literal of \TDDLfm $\ell = \monadop{D}{\is}{a}$   and  $k \in \mathbb{N}$. We have:
\[
\frac{\size{\pnf(\ell,k)}}{\size{\ell}} 
\le 5\,\expf(\is^k),
\]
where $\is^k= \is \ominus [k, \infty]$ and $\expf(\is) := \frac{\size{\is}}{\card(\is)}$ is the \emph{expansion factor} of the interval set $\is$.
\end{lemma}

\begin{proof}
For  $\ell = \monadop{D}{\is}{a}$, we have the size of the literal
$\size{\ell} = 2 + 2\card(\is)
$ as defined in Definition.~\ref{def:size}

By the definition.~\ref{def:pnf} for $\pnf$ for literals, each time point $t$ in $\is \ominus [k{+}1,\infty]$ becomes a punctual literal $\monadop{D}{\{[t,t]\}}{a}$, The returned formula is formed by conjunctions or disjunctions depending on $D$. The number of punctual literals is at most $\size{\is^k}$, and the number of binary connectives is $\size{\is^k} - 1$. Each punctual literal has a fixed size of $4$ , giving

\[
\size{\pnf(\ell,k)} \le 4\size{\is^k} + (\size{\is^k}-1) + C 
= 5\size{\is^k} - 1 + C,
\]
where $C$ accounts for a possible tail term $\monadop{D}{\is \cap [k+1, \infty]}{a}$,

Thus, we can approximate the complexity by :
\[
\frac{\size{\pnf(\ell,k)}}{\size{\ell}}
\le \frac{5\size{\is^k}-1}{2+2\card(\is^k)}
= \frac{5\expf(\is^k)\card(\is^k)-1}{2+2\card(\is^k)}
\le 5\,\expf(\is^k) .
\]
\end{proof}
...
\item  $\mathcall{I}_\phi$ is the array of interval sets occurring in the literals from  $\phi$ all reduced by the interval $[k+1,\infty]$.\\
\end{enumerate}
\end{lemma}
...
The \textsc{MaxOPoint}$(\phi)$ returns the largest finite time point from any obligation interval in $\phi$.  
The algorithm then scans each product term in the disjunctive form and records all literal pairs that match the deontic or ontic conflict patterns, returning them as tuples $(\pt,\mathcall{C})$.
...
Subsequent $\dnf$ and conflict extraction produce the results summarized in \ref{table:conflict-analysis}.
...
Composing the two preservation steps proves $\dnf(\pnf(\phi,k))\dutyequiv\phi$.
\end{proof}

Before proving correctness, we establish that the \emph{structural invariants} are preserved by both the
...
\begin{proof}
By Definition~\ref{def:pnf}, $\pnf$ each literal is unfolded \emph{in place} into a disjunction
(for $\tool{O}$) or conjunction (for $\tool{F}$) of punctual or child components.
No change occurs at the parent level for the boolean connective along the path from root to leaf as it is moved from one level to another.
That is, the lowest upper common operator (\LUCO) for both positions remains unchanged.
\end{proof}
...
$\ell$, the disjunction for $\tool{O}$ or conjunction for $\tool{F}$, and by
\textbf{ObDis}/\textbf{FConj} the resulting expression is syntactically equivalent to~$\ell$.
\end{proof}

\begin{lemma}[Punctual witness preservation under $\pnf$]\label{lem:witness-punct-pnf}
...
which stays under the same conjunctive~\LUCO relation.
\end{lemma}

\begin{proof}

By Definition~\ref{def:pnf}, the descendants $\hat\ell^p=\monadop{D}{\{[t,t]\}}{a}$ and
$\hat\ell^{\prime p}=\monadop{D'}{\{[t,t]\}}{a'}$ remain in place.

 By Lemma~\ref{lem:pnf-luco}, the \LUCO relation is preserved.
\end{proof}

\begin{lemma}[\LUCO and \LAP preservation under $\dnf$]\label{lem:dnf-luco}
Let $\phi\in\TDDLfm$, we have :
\begin{enumerate}[(i)]
\item \textbf{LUCO preservation.}  
...
\begin{proof}
The $\dnf$ normalization is obtained by orienting the distributivity rule to push the disjunction inside 
(e.g.\ $\phi_1\sqcap(\phi_2\sqcup\phi_3)\!\to\!(\phi_1\sqcap\phi_2)\sqcup(\phi_1\sqcap\phi_3)$)
These rewritings do not change $\sqcap$/ into $\sqcup$  and vice-versa, but only propagate one operator outward,
and that is how the original relations along root-to-leaf paths remain unchanged.
...
occur: \LUCO is preserved.

(ii) Under the cross-product semantics of $\dnf$, a conjunctive node produces product terms
...
the literals $\hat\ell,\hat\ell'$ are punctual and match
...
If $\ell_i,\ell_j$ specify a conflict in~$\phi$ with
...
\]
So this transformation step size remains polynomial in $\size{\phi}$ when $\expf_a(\phi)$ is bounded.
...
exponential cross-product. Each obligation literal $\ob{\is}{a}$ in $\pnf(\phi,k)$ generates a
disjunction of $\size{\is}$ punctual obligations, and each such disjunction multiplies the
number of disjuncts. Therefore, the number of product terms in
...
\]
and it captures  the worst-case space of the normalization chain.
\end{proof}


\section{Semantic Characterization of Timed Normative Conflicts and Conflict Elimination}\label{sec:unsat-conflicts}

This section lays the semantic basis for timed normative conflicts. We first recall propositional unsatisfiability, unsatisfiable cores, and minimal unsatisfiable subsets (MUS). We then transfer these notions to \TDDLfm\ by viewing product terms of the disjunctive punctual normal form (\dpnf) as local clause sets. The key outcome is that punctual conflicts in \TDDLfm\ coincide with MUSes over punctual literals inside a single product term. This provides an exact assessment and explainability for formula unsatisfiability via conflict density and supports conflict-free rewriting. We close with the syntactic cost of conflict elimination.

\subsection{Unsatisfiable Cores and MUS in Propositional Logic}

When a formula from propositional logic \(F= f_1 \cap f_2 \dots f_n\) in CNF is unsatisfiable, an \emph{unsatisfiable core} is any subset \(F'\subseteq F\) of disjuncts that is unsatisfiable. Unsat cores are usually discovered during learning-based SAT solving when the solver reaches a dead end.

\emph{Minimal unsatisfiable subsets} (MUS) have been suggested to refine unsat cores \cite{Liffiton2008MUS}. A MUS is an unsat core \(U\subseteq F\) such that removing any clause from \(U\) makes it satisfiable. Identifying these subsets offers a significant advantage over standard unsat cores, as MUS clauses represent the minimal sufficient cause of unsatisfiability. Consequently, debugging efforts and formula relaxations should be targeted precisely there. This targeted approach is more practical than debugging standard unsat cores, which typically contain redundant clauses and are much larger.

Furthermore, analyzing an unsatisfiable formula can reveal several MUSes, which isolate the small fragments to be corrected. Within these fragments, a single clause may appear in multiple MUSes, since different minimal combinations of clauses can independently give rise to unsatisfiability. Enumerating all such MUSes is a computationally demanding problem. The difficulty arises not only because the result size can be exponential, but also because verifying whether a given subset is a MUS is \(D^P\)-complete.

%\emph{Minimal unsatisfiable subsets} (MUS) have been suggested to refine unsat cores \cite{Liffiton2008MUS}, where a MUS is an unsat core \(U\subseteq F\) such that removing any clause from \(U\) makes it  satisfiable. The advantage of identifying MUS over unsat cores is that MUS clauses are the minimal sufficient cause of unsatisfiability, and correcting or relaxing the formula should be targeted there. It is more practical than debugging on unsat cores, which are larger in most cases. Secondly, an unsatisfiable formula can contain several MUSes, which isolate the small fragment to be corrected. A single clause may also appear in multiple MUSes, since different minimal combinations of clauses can independently give rise to unsatisfiability. The problem of enumerating all MUSes is computationally demanding because checking whether a given subset is minimal is itself coNP-complete and the result size may be exponential.  

In our setting, this perspective is guided by the output of our conflict detection procedure Algorithm~\ref{fig:conflict-extraction}: conflicts surface locally inside individual product terms of \dpnf, not across the disjunction. We therefore analyze MUSes \emph{per product term}, where the roles of clauses are played by punctual literals. The minimal unsatisfiable subsets of these literals are exactly the punctual conflicts that cause unsatisfiability in \TDDLfm.

\subsection{From propositional MUSes to punctual MUSes in \TDDLfm}

In the propositional setting, MUS analysis is performed on sets of clauses, i.e a formula in CNF.
A clause belongs to an MUS when it is part of the smallest subset of clauses that cannot all be true at the same time. 
In \TDDLfm, the surface syntax contains interval-based obligations and prohibitions, but after unfolding to disjunctive punctual normal form, the structure becomes closer to the propositional one, as the overall resulting DNF formula is formed by product terms that we can check for MUS
 as in 
$$\dpnf(\phi,k) \;=\; \bigsqcup_i \pt_i$$
 The satisfiability of a product term $ pt_i$ depends on whether there exists a trace that jointly satisfies all its punctual literals . This introduces a key distinction: while the \dpnf of $\phi$
 encodes all possible planning scenarios; each product term captures a local scenario in which specific temporal conditions are met.
Unsatisfiability of \(\phi\) means that \emph{every} such local scenario is impossible, therefore every product term is unsatisfiable. The natural analogue of a clause-level MUS is then not a subset of the whole formula, but a minimal unsatisfiable subset of punctual literals \emph{inside a single product term}. 
This is exactly what we call a punctual MUS in \TDDLfm.

We aim to capture with punctual MUS both normative and SAT-based insights: (i) Normatively in the sense that the concept captures the exact moment or condition where an agent faces an impossible choice. From a SAT perspective as it exposes the minimal subset of constraints within one product term that render its corresponding trace invalid.

\begin{definition}[Punctual MUS]\label{def:punctual-MUS}
Let \(L=\{\ell_1,\dots,\ell_n\}\) be a set of punctual literals.
The conjunction \(\bigsqcap_{\ell\in L}\ell\) is a \emph{punctual minimal unsatisfiable subset} if:
\begin{enumerate}[(i)]
  \item \(\bigsqcap_{\ell\in L}\ell\) is unsatisfiable, and
  \item for any non empty strict subset \(L'\subset L\), the conjunction\(\bigsqcap_{\ell\in L'}\ell\) is satisfiable.
\end{enumerate}
\end{definition}

The two conditions reflect exactly the notion of MUS in propositional logic when the formula is a conjunction of literals.

Our goal is to study all possible patterns of a set of literals that could specify an MUS. More precisely, we are interested in the possible size of MUS and the literals that could appear in a Mus. To do so, we progressively study larger sets, starting from 1, to determine the minimal size of an MUS.

\begin{lemma}[Satisfiability of punctual literals]\label{lem:punct-solo-sat}
Any literal punctual from \TDDLfm is satisfiable.
\end{lemma}

\begin{proof}
Witnesses for \(\ob{\{[t,t]\}}{a}\) is \(\trace{(a,t)}\)  and \(\emptytrace\) for \(\forb{\{[t,t]\}}{a}\).
\end{proof}
...
Any set of punctual literals formed by a deontic conflict, i.e. \(\{\ob{\{[t,t]\}}{a},\ \forb{\{[t,t]\}}{a}\}\), is a punctual MUS.
\end{lemma}

\begin{proof}
We have:
\begin{itemize}
\item By semantic definition, this pattern is unsatisfiable.
\item Minimality follows from Lemma~\ref{lem:punct-solo-sat}, as we proved that each literal alone is satisfiable.\end{itemize}
\end{proof}

\begin{lemma}[Ontic punctual conflict is a punctual MUS]\label{lem:punct-ontic}
Any set of punctual literals formed by an ontic conflict, i.e. \(\{\ob{\{[t,t]\}}{a},\ \ob{\{[t,t]\}}{b}\}\) with \(a\neq b\), is a punctual MUS.
\end{lemma}
\begin{proof}
We have:
\begin{itemize}
\item This pattern is unsatisfiable as the trace semantics admits at most one action at time \(t\).
\item Minimality follows from Lemma~\ref{lem:punct-solo-sat}.
...
In \TDDLfm, every punctual minimally unsatisfiable subset is either a deontic or an ontic punctual conflict.
\end{lemma}

\begin{proof}
We explore all the possibilities of combinations of punctual literals as summarized:

\begin{enumerate}
\item If the time points differ, the conjunction of the two literals is satisfiable.
\item If they concern the same action at the same time, pairs of the form $\ob{\{[t,t]\}}{a}$ with $\forb{\{[t,t]\}}{a}$
produces a deontic punctual conflict. the pairs \(\ob{\{[t,t]\}}{a}\) with \(\ob{\{[t,t]\}}{a}\) and 
\(\forb{\{[t,t]\}}{a}\) with \(\forb{\{[t,t]\}}{a}\) are redundant and consequently satisfiable as a unique literal.

\item If they concern distinct actions at the same time,  any other combination than punctual ontic conflict from this pattern category would contain a prohibition and thus is satisfiable, since prohibitions do not force any action to occur.
\end{enumerate}
\end{proof}

\begin{theorem}[Conflict detection algorithm is sound and complete for detecting all MUSes]\label{thm:mus-structure}

Let $\phi$ be a formula from \TDDLfm. The Algorithm.~\ref{fig:conflict-extraction} is sound and complete for detecting all MUS for each product term in $\dpnf(\phi)$.
...
Algorithmically, this means that conflict detection at the punctual level reduces to a simple pattern-matching task within the product terms of \dpnf. 
\end{proof}
...
\subsection{Conflict Density and Exact Unsatisfiability Criterion}
We move now to introduce the \emph{conflict density} of a formula, which is a quantitative metric for conflicts presence in formulas from \TDDLfm.  
The density measures how many conflicts occur within its punctual representation of a formula.  
A high conflict density indicates an overloaded specification with conflicting timed conditions: the set of feasible behaviors is less than the timed conditions specified in the literals, which complicates compliance planning, and makes interpretation of the overall desired behavior more difficult. The following definition defines it.

\begin{definition}[Conflict density]\label{def:Conflict density-measure}
Let $\phi$ be a formula in \TDDLfm and let the upper bound\\ $k=\textsc{MaxOPoint}(\phi)$.  
The \defn{conflict density} of $\phi$, written $\cd(\phi)$, is the ratio between the number of conflicting product terms and the total number of product terms in $\dpnf(\phi,k))$:
\[
...
From \ref{table:conflict-analysis} for Example~\ref{ex:easy}, we have:
\[
...
A formula with $\cd(\phi)=0$ is \defn{conflict-free}; a formula with $\cd(\phi)=1$ is \defn{fully conflicting} in the sense that every product term in its \dpnf contains at least one punctual conflict.

\subsubsection{Exact Unsatisfiability Criterion}

The extreme value \(\cd(\phi)=1\) has a precise semantic meaning. It states that every product term obtained from \(\dpnf(\phi,k)\) contains a punctual conflict. The next results show that this syntactic condition is not only sufficient for unsatisfiability, but also necessary.

\begin{lemma}[Full conflict implies unsatisfiable]\label{lem:full-conflict-implies-unsat}
If \(\cd(\phi)=1\), then \(\phi\) is unsatisfiable.
\end{lemma}

\begin{proof}
Every product term in \(\dpnf(\phi,k)\) is unsatisfiable, so their disjunction is unsatisfiable.
\end{proof}
...
\begin{proof}
\(\phi\) is equivalent to the disjunction of its product terms in \(\dpnf(\phi,k)\). If \(\phi\) is unsatisfiable, then no product term can be satisfiable; otherwise the whole disjunction would be satisfiable. Hence each product term is unsatisfiable. For each such term, take a minimally unsatisfiable subset. By Lemma~\ref{lem:punct-completeness}, this subset is either a deontic or an ontic punctual conflict. Therefore every product term is accounted for in \(\mathcall{PC}(\phi)\), and thus \(\cd(\phi)=1\).
\end{proof}
...
After establishing that when the conflict density of a formula \(\cd(\phi)<1\) is still satisfiable, we establish now that pruning all conflicting terms produces an equivalent conflict-free disjunction.

\begin{theorem}[Conflict-free rewriting]\label{thm:cfr}
For any $\phi$ from \TDDLfm, if  $ \cd(\phi) \neq 1 $, then there exists $\phi'$ with $\phi \equiv \phi'$ with $\cd(\phi')=0$.
\end{theorem}

\begin{proof}[Proof sketch]
Using Algorithm.~\ref{fig:conflict-extraction}   computes first the \dpnf rewriting $\phi_{\mathrm{dpnf}}=\bigsqcup_i \pt_i$, then it classifies its product term. By computing a variant at the classification step, we can compute $\text{PT}\text{safe}={ \pt_i \mid \pt_i \in \phi{\mathrm{dpnf}}  \text{ with } \pt_i \text{ has no punctual conflict}}$ as illustrated in Algorithm~.\ref{alg:conflict-free-rewriting}. Set \(\phi'=\bigsqcup_{i\in I}\pt_i\). Consequently, all unsatisfiable product terms are filtered and $\phi \equiv \phi'$ by Lemma~.\ref{empty_inter_union}, and by construction $\cd(\phi')=0$.
\end{proof}
...
Nonetheless, conflict elimination could return larger formulas. In particular, many ontic conflicts in the \dpnf formula can lead to a substantial blow-up.
...
\ob{{[0,3]}}{\mathsf{c}}.
\]
\ref{table:ontic-triple-conflict} summarizes the transformation pipeline from \pnf, \dnf, conflict extraction, to the conflict-free rewriting $\Namb(\phi)$.
The final formula is roughly $20\times$ as large as the original.

Specifically, $\dnf(\pnf(\phi,3))$ enumerates all $4^3$ assignments of times to $(\mathsf{a},\mathsf{b},\mathsf{c})$.
To avoid conflicts, only combinations with pairwise distinct times remain, i.e.\ $4\cdot3\cdot2=24$ product terms, which demonstrates a substantial increase.
...
This example illustrates worst-case scenarios of a verbose conflict-free formula. Other formulas admit concise equivalents by \emph{merging} punctual literals specified for the same action: the disjunction of punctual obligations can be compacted via \textbf{ObDis}, and conjunctions of punctual prohibitions via \textbf{FConj}.
...
\ref{table:refactoring-uc2} compares the original, verbose rewriting, and a merged equivalent $\UC_2''$.
...
\end{table}
We have $\UC_2''$ merges the surviving punctual obligations on \delAtPark\ into a single interval set, while prohibitions on \delAtGate\ are recombined into their original form.
\end{example}

These examples motivate a strategy of \emph{controlled conflict-density management}:
When the concise representation of a normative specification is essential, the user of the framework may choose between:
(i) keeping formulas that contain ontic conflicts and optionally flagging them for online planning, or
(ii) eliminate all conflicts and try refactor the resulting \dpnf\ using the merging rules (\textbf{ObDis}, \textbf{FConj}) to restore compactness.
