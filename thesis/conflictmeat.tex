...
\begin{lemma}[LUCO preservation under $\pnf$]\label{lem:witness-punct-pnf}
...
\subsubsection{Conflict Density}
...
A high conflict density indicates an overloaded specification with conflicting timed conditions: the set of feasible behaviors is less than the timed conditions specified in the literals, which complicates compliance planning, and makes interpretation of the overall desired behavior more difficult. We compute this measure via a simple formal definition.
...
\emph{Minimal unsatisfiable subsets} (MUS) have been studied in \cite{MarquesSilva2010MUSsurvey} to refine unsat cores where a MUS is an unsat core \(U\subseteq F\) such that removing any clause from \(U\) makes it  satisfiable. The advantage of MUS over unsat cores is that it identifies which clauses cause unsatisfiability, reducing the unsat core from hundreds to tens, which is useful for debugging. Secondly, an unsatisfiable formula can contain several MUSes, which map directly to which clauses conflicts directly with others. A single clause may also appear in multiple MUSes, since different minimal combinations of clauses can independently give rise to unsatisfiability. The problem of enumerating all MUSes is nonetheless intractable because of the potentially exponential size of the result as discussed in \cite{Liffiton2008MUS}.  
...
the set \(\{\ob{\{[t,t]\}}{a},\ \forb{\{[t,t]\}}{a}\}\) is a punctual MUS.
\end{lemma}
...
\item Minimality follows from Lemma~\ref{lem:punct-solo-sat}; each literal alone is satisfiable.\end{itemize}
\end{proof}

\begin{lemma}[Ontic punctual conflict is a punctual MUS]\label{lem:punct-ontic}
the set \(\{\ob{\{[t,t]\}}{a},\ \ob{\{[t,t]\}}{b}\}\) is a punctual MUS.
\end{lemma}
\begin{proof}
We have:
\begin{itemize}
...
\begin{proof}
\(\phi\) is the disjunction of its product terms. If \(\phi\) is unsatisfiable, each product term is unsatisfiable as each contains a punctual conflict, so every term is correctly accounted for in \(\mathcall{PC}(\phi)\).
\end{proof}
...
After establishing that when the conflict density of a formula \(\cd(\phi)<1\) is still satisfiable, pruning all conflicting terms produces an equivalent conflict-free disjunction.
...
To avoid conflicts, only combinations with pairwise distinct times remain, i.e.\ $4!\cdot!3!\cdot!2=24$ product terms, which demonstrates a substantial increase.
