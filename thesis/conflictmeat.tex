...
This invariant ensures that normalization only refines the syntactic representation of literals without changing their specified time points, deontic operator an action. For the specified time points espcially for this invariant, no time point is introduced and no time point is removed.
...
Subsequent $\dnf$ and conflict extraction yield the results summarized in \ref{table:conflict-analysis}.
...
A high conflict density indicates a an overloaded specification with conflicting timed conditions: the set of feasible behaviors is less then the the timed conditions specified in the literals, which complicates compliance planning, and makes interpretation of the overall desired behavior more difficult. We compute this measure via a simple formal definition.
...
After establishing that when the conflict density of a formula \(\cd(\phi)<1\) is still satisfiable, pruning all conflicting terms yields an equivalent conflict-free disjunction.
