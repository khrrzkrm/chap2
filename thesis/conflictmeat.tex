...
\end{inparaenum}
\end{example}
In this section, the notion of conflict is treated primarily as a syntactic phenomenon, rather than as a property defined over the full semantic domain, that is, the set of models. The goal is not merely to determine whether a specification admits some compliant execution, but to identify incompatible normative prescriptions directly at the level of their formal structure. By focusing on syntax, conflicts can be detected compositionally, prior to model construction, and in a way that scales with the size of the specification. This shift is deliberate: semantic emptiness of the model set is too coarse a signal, as it obscures where and why a specification becomes problematic.
...
\ob{\{[0,3]\}}{\mathsf{c}}.
\]
\ref{table:ontic-triple-conflict} summarizes the transformation pipeline—\pnf, \dnf, conflict extraction, and the conflict-free rewriting $\Namb(\phi)$.  
