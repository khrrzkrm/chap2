...
For any event $e \in \event$, we write
$e^{\omega}$
to denote the infinite word obtained by repeating $e$ at every position, that is,
\[
e^{\omega} \;:=\; \trace{e,e,e,\dots} \;\in\; \event^{\omega}.
\]
...
The notation \( w = \trace{e_0,\,e_1,\,\dots,\,e_{n-1}} \in E^* \) symbolizes a finite word $w$ defined by a sequence of events $e_i$ from \( E \). We define the following canonical notions:

\begin{itemize}
    \item \defn{Size}: The length of the word corresponds to the number of events forming it and is denoted by \( |w| := n \).
    ...
    \item \defn{Empty word}: We write \( \emptytrace \) for the empty sequence, where \( |\emptytrace| = 0 \).
\end{itemize}

\paragraph{Boundary conventions.}
For uniform treatment of splitting and concatenation, we adopt the following conventions for any finite word $w$:
\[
  w[0,-1] := \emptytrace,\qquad
  w^{|w|} := \emptytrace.
\]
These conventions ensure that prefixes and suffixes are well-defined at the boundaries of a word.
...
\end{definition}
    ...
    \begin{definition}[Timed distance between two events]
        Let $\taumt=\trace{(a_0,t_0),\dots,(a_{n-1},t_{n-1})}\in(\Sigma\times\mathbb{N})^{*}$ be a metric-timed word.
