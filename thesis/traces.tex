...
When we speak of traces, we mean observed words during concrete executions of a system or agent performance.
...
Logical-time words capture \emph{order only}, abstracting away any notion of specific timing. They are the staple semantic objects in interleaving models of process calculi, such as CCS \cite{Milner89} and CSP \cite{hoare1978communicating}. Moreover, Mazurkiewicz's trace theory represents concurrent behavior by regrouping words over an alphabet modulo an independence relation: adjacent independent actions may be swapped, and a trace is the resulting equivalence class of words~\cite{diekert1995book,Leucker2002Logics}. In this thesis, we do not adopt this concurrency model; instead, we assume a single, globally ordered sequence of events.
...
\begin{example}[Metric-time operators]
  Consider the metric timed trace \(\taumt\) defined on the alphabet $\Sigma=\{\textsf{badge\_tapped},\textsf{door\_open}\}$:
  \[
...
The synchronous-time word timing model occupies a middle ground between logical-time words and metric-time words. Like logical time, they are discrete sequences, but they come with an explicit global round index that plays the role of a coarse clock. Like metric time, one can talk about \emph{when} something happens, but only in terms of round numbers rather than absolute timestamps. The crucial difference to both earlier models is that the \emph{absence} of an action is itself observable, represented by the stutter symbol ``$(-)$''. 
...
The map \(\ST\) aligns events to a global round clock by inserting ``\((-)\)'' at rounds with no event whilst erasing ``\((-)\)'' brings us back to logical time.
...
In this subsection, we illustrate the complexity of synchronizing traces across different timing settings. We define three word-level operators, adapted from standard process and synchronous-language formulations to fit our specific two-agent models denoted by indices 1 and 2, with alphabets $\Sigma_1$ and $\Sigma_2$. They should therefore be read as adapted trace operators used in this dissertation, not as verbatim reproductions of the cited operational semantics. We cover:
...
The \emph{lockstep} operator combines two synchronous words by aligning them round by round under a shared logical clock.  If the two words have different horizons, the shorter one is right-padded with the stutter symbol ``$(-)$'' so that both align on the common index set $\{1,\dots,T\}$, where $T=\max(T_1,T_2)$.
...
Let \( P_1 = \Sigma_1 \setminus A \) and \( P_2 = \Sigma_2 \setminus A \) be the sets of private actions. The collapsed alphabet \( \Sigma_{\parallel} \) is defined as:
  ...
  \]
where \( P_i^{(i)} \) means the private actions are labeled with the agent's ID.
  
The collapsed lockstep with handshakes is defined by applying the function \(\mathsf{coll}_A\) to each pair \((s_t, r_t)\) in the raw lockstep word:
  \[
...
\begin{example}[Handover as handshake, failures and successes]
  Let \(\Sigma_1=\{\textsf{pick},\textsf{handover}\}\), \(\Sigma_2=\{\textsf{scan},\textsf{handover}\}\). We have \(A=\{\textsf{handover}\}\) is the collaborative handshake action and 
  ...
  In summary, this section presented three models for discrete-action words and explained how they relate to each other, as detailed in \ref{tab:word-models-summary}. We started by setting up an abstract algebraic framework for both finite and infinite words. Then, we described three specific word types: (i) logical-time words, which only keep the order of actions, (ii) metric-time words, which capture better timing order using strictly increasing timestamps, and (iii) synchronous-time words, which organize observations by rounds and show inactivity through stuttering. We linked these models using the forgetful morphisms $\LT$ and $\ST$, which let us view a metric-time execution (i) in terms of order only, or (ii) as round-synchronous. Next, we introduced three ways for two agents to synchronize: asynchronous interleaving $\shuffle$ for logical time, lockstep zip $\parallel_{\mathrm{lock}}$ for synchronous time, and lockstep with handshakes $\parallel_{\mathrm{hs}}^{A}$, plus a version that combines everything into a single word. 
\begin{table}[h!]
  \centering
  ...
  Async. Interleaving & $\shuffle$ & $\tault \times \tault$ & All order-preserving interleaving. \\
  Lockstep Zip & $~~~~~\parallel_{\mathrm{lock}}$ & $\taust \times \taust$ & Aligns rounds strictly. \\
  Handshake & $~~~\parallel_{\mathrm{hs}}^{A}$ & $\taust \times \taust$ & Synchronizes + interleaves private. \\
  \bottomrule
  \end{tabular}
  \end{table}
