...
We begin by establishing three word models, each characterized by its timing setting, together with the canonical morphisms between them. Subsequently, we study how to synchronize two words of the same type.

\subsection{Notation for Discrete-Action Words}
For the remainder of this dissertation, let \(\Sigma\) denote a non-empty set of discrete \emph{actions} (instantaneous, with no durations). Before introducing the specific timing models, we fix a small algebraic core shared by all discrete-action words, specifically the basic sequence structure and standard operators.

\paragraph{Events and Words.}
We formalize the universe of atomic occurrences as a type called the \emph{event domain}, denoted by $\mathbb{E}$.
An individual object of this type is an \emph{event}, denoted by $e$.
The action alphabet \(\Sigma\) names the possible action labels, while the event domain \(\mathbb{E}\) fixes the structure carried by each word position. The structure of \(\mathbb{E}\) is determined by the timing model applied to \(\Sigma\), for instance \(\mathbb{E}=\Sigma\) for logical-time words and \(\mathbb{E}=\Sigma\times\mathbb{N}\) for metric-time words.
...
The empty word, denoted by $\emptytrace$, is the only finite word containing no events. The set of non-empty finite words is $\mathbb{E}^+$, and \(\mathbb{E}^*=\{\emptytrace\}\cup\mathbb{E}^+\).
...
We adopt \emph{single indexing} where indices correspond to the position of events in the sequence.

\begin{itemize}
    \item \defn{Size}: The length of the word is defined as \( |w| := n \).
    ...
    \item \defn{Prefix}: For any \( 0 \le k \le n \), the prefix of length \( k \) is denoted by \( w_k \):
    \[
        w_k :=
        \begin{cases}
        \emptytrace, & \text{if } k=0,\\
        \langle e_1, \dots, e_k \rangle, & \text{if } 1\le k\le n.
        \end{cases}
    \]
    In particular, \(w_n=w\).
...
We conceptually distinguish a \emph{finite word}, which is a conceptual model, from a \emph{finite trace}, which is a concrete observed behavior of a system/agent. However, formally, both are finite sequences of events in $\mathbb{E}^*$. In this dissertation, we use \emph{word} as the default technical term and \emph{trace} when emphasizing that the sequence is the result of a recorded observation.

\paragraph{Omega Notation.}
For any non-empty finite word \(u=\langle e_1,\dots,e_n\rangle\in\mathbb{E}^{+}\) with \(n\ge 1\), we write \(u^{\omega}\in\mathbb{E}^{\omega}\) for the infinite word obtained by repeating \(u\) indefinitely. It is defined pointwise by
\[
u^{\omega}[k] := e_{((k-1) \bmod n)+1}
\qquad\text{for all } k\ge 1.
\]
\paragraph{Prefix Order.}
For any finite word \(u \in \mathbb{E}^{*}\) and any word \(v \in \mathbb{E}^{\infty}\), we write \(u \preceq v\) if there exists \(k \in \mathbb{N}\) such that \(|u|=k\), $k\leq_\infty |v|$ and 
\[
...
For a subset of events $S \subseteq \mathbb{E}$, the projection $w{\upharpoonright}S$ is the subsequence retaining exactly those events $e_k$ such that $e_k \in S$.


\subsection{Logical-Time Words}

Logical-time words capture \emph{order} of occurrence between events forming a word. They are the basic model in the theory of process calculi, such as CCS \cite{Milner89} and CSP \cite{hoare1978communicating}, where they are defined over actions and in LTL, where they are defined over sets of atomic propositions. 
...
Having fixed the logical-time vocabulary, we now enrich the model with quantitative timestamps.


\subsection{Metric-Time Words}

Metric-time words extend the previous model by recording \emph{which} action occurs and \emph{when} it occurs. They originate from the theory of timed automata~\cite{DBLP:journals/tcs/AlurD94}. We call elements of \(\Sigma\times\mathbb{N}\) \emph{timed events}.

\begin{definition}[Finite metric-time word] \label{timedwords} 
    A \emph{finite non-empty metric-time word} $\tau$ is defined as a finite sequence of timed events, $\tau \in (\Sigma\times\mathbb{N})^+$.
     \[\tau := \trace{(a_1,t_1), (a_2,t_2), \ \dots, \ (a_{n},t_{n}) }\]
     with \(a_k\in\Sigma\), \(t_k\in\mathbb{N}\), and strictly increasing timestamps \(1\le t_1<\cdots<t_{n}\).
\end{definition}
...
We denote the set of all finite metric-time words by $\domtr$. Beyond action counts, metric-time words allow measuring the time elapsed between events. This distance is measured by the following simple function.

 \begin{definition}[Time distance between two events]
...
The metric-time model records the exact time point at which each action occurs. For this, we now define the action lookup function that checks what action occurs at an exact time point.
...
\noindent\textbf{Basic operators and shorthands.}
...
Prefixes can be defined by \emph{length} \( \taumt[1,k] \), or by \emph{time} \( \taumt{\upharpoonright}\{t\le T\} \) (shorthand: \( \taumt\{t\le T\}\)).
 ...
 As events are composed of an action label and the timestamp at which they occur, concatenation is not defined for arbitrary pairs of words, as the combined sequence must respect the strictly increasing order of timestamps.  

\begin{example}[Metric-time operators]
  Consider the metric-time word \(\taumt\) defined on the alphabet $\Sigma=\{\textsf{badge\_tapped},\textsf{door\_open}\}$:
  \[
...
The synchronous-time word timing model occupies a middle ground between logical-time words and metric-time words: it preserves a coarse notion of when an action occurs, but only by round number rather than by absolute timestamp.

Notice the subtle difference of notations: the empty trace \(\emptytrace\) (a trace of length 0) is different from the single event trace \(\trace{(-)}\), which represents exactly one round of inactivity.
...
\begin{example}[controller at 1\,Hz, illustrating all operators]
    Consider an HVAC (Heating, Ventilation, and Air Conditioning) actuator (1) and a security sensor (2), both controlled by a synchronous controller:
    \[
...
The objective of this subsection is to connect the three action-centric word models introduced above. Given a metric-time word \(\taumt\), we introduce two canonical morphisms that deliberately discard part of the information: the \emph{logical-time projection} \(\LT\), which erases timestamps while preserving the order of events, and the \emph{synchronous padding} \(\ST\), which aligns events to a global round clock by inserting explicit stutter symbols ``\((-)\)''. We fix the clock frequency of the synchronous-time word to correspond to a single unit of metric time.

\noindent\textbf{Summary diagram.}
\ref{fig:word-models-summary} illustrates the two morphisms on a running two-event example.
...
\begin{tikzpicture}[
    ...
    map line/.style={->, dashed, color=black!80, shorten >=2pt, shorten <=2pt}
]

    % --- 1. Metric-Time Word (Top Layer) ---
    \node[label text] (mt_label) at (-2, 0) {Metric-Time\\ $\tau_{mt}$};
    ...
    \node[right, black, font=\footnotesize, align=left] at (7.5, -1.25)
        {\textbf{Synchronous Padding} $\ST$\\
         Aligns events to clock ticks\\
         Inserts `$(-)$' when an event \\is missing at time $t$};

    \node[right, black, font=\footnotesize, align=left] at (7.5, -3.75)
...
The map \(\LT\) drops timestamps but preserves labels and their order. It is the coarsest view that still distinguishes different event sequences.

\begin{lemma}[Logical-time projection \(\LT\): order-only view]
Let \(\taumt=\trace{(a_1,t_1),\dots,(a_{n},t_{n})} \) be a metric-time word. There exists a unique logical word \(\tault=\LT(\taumt)\in\Sigma^{*}\) satisfying:
\begin{itemize}
  \item \textbf{Size preservation}: \( |\tault| = |\taumt| = n \).
...
\begin{proof}
\defn{Construction.}\;
Define \(\LT(\taumt):=\trace{a_1,\,a_2,\,\dots,\,a_{n}}\). All properties follow from erasing timestamps while preserving labels and their order.
\end{proof}


\paragraph{From metric time to synchronous time.}
The map \(\ST\) aligns events to a global round clock by inserting ``\((-)\)'' at rounds with no event; erasing these stutters recovers the corresponding logical-time word.
...
\begin{itemize}
  \item \textbf{Size / horizon relation}: If \(|\taumt|>0\), then \(|\ST(\taumt)| = T\) and \(\mathsf{pos}(\ST(\taumt))=\{1,\dots,T\}\).
  \item \textbf{Exact time-of-occurrence}: For any timestamp \(t \in \{1, \dots, T\}\), the synchronous event at index \(t\) is determined by:
  \[
  \ST(\taumt)[t] = 
  \begin{cases}
...
We construct the word of length \(T\).
For every integer \(t \in \{1, \dots, T\}\) (representing the metric time), we define the event at index \(t\) as:
\[
\ST(\taumt)[t] :=
\begin{cases}
...
\end{cases}
\]
Since metric timestamps are strictly increasing, there is at most one \(k\) for any \(t\), making this well-defined.
Uniqueness holds because, for each \(t\) in \(\{1,\dots,T\}\), the value \(\ST(\taumt)[t]\) is uniquely set by the exact time-of-occurrence property. Also, the size/horizon property ensures that \(|\ST(\taumt)|=T\).
All properties follow from this construction.
\end{proof}

\paragraph{On non-invertibility.}
Both \(\LT\) and \(\ST\) are \emph{many-to-one} when viewed as abstractions from concrete timed executions. In particular, \(\LT\) is \emph{not} invertible: one cannot reconstruct a unique metric or synchronous word from logical time alone. For \(\ST\), the abstraction fixes a one-tick-per-time-unit convention and forgets the underlying physical duration and origin of that clock. Once those conventions are fixed externally, the integer timestamps are recoverable from the indices of non-stutter symbols.
...
In this subsection, we illustrate the complexity of synchronizing words across different timing settings. We define three word-level operators, adapted from standard process and synchronous-language formulations to fit our specific two-agent models denoted by indices 1 and 2, with alphabets $\Sigma_1$ and $\Sigma_2$. They should therefore be read as adapted word-level operators used in this dissertation, not as verbatim reproductions of the cited operational semantics.
\begin{itemize}
  \item \emph{Asynchrony} on logical-time words (\(\tault\)).
  \item \emph{Lockstep synchrony} on synchronous words (\(\taust\)).
  \item \emph{Synchronous handshake} on synchronous words with an explicit set \(A\) of shared actions.
\end{itemize}

\subsubsection{Asynchronous Interleaving on Logical-Time Words}
The asynchronous operator models purely interleaved joint behavior, generating \emph{all shuffles} that preserve each agent’s local order. No global clock or rendezvous is assumed. To avoid accidental identification of simultaneous events, the global alphabet is taken as the \emph{disjoint union} $\Sigma=\Sigma_1\uplus\Sigma_2$, tagging each action with its agent of origin. This is the standard pure-interleaving view used in process calculi such as CCS and CSP when no synchronization constraint is imposed \cite{Milner89,Hoare85}.
...
The operator \(\shuffle\) used here keeps all order-preserving interleavings as explicit words rather than interpreting their interleaving through an additional independence relation as in the theory of Mazurkiewicz traces \cite{diekert1995book,Leucker2002Logics}.

\subsubsection{Lockstep Zip on Synchronous-Time Words}

The \emph{lockstep} operator combines two synchronous words by merging their event on the same round, assuming that they operate under a shared clock.  For the synchronization of two words with different horizon, the shorter one is right-padded with the stutter symbol $(-)$ so that both align on the common horizon that is the maximal of both traces.
...
\subsubsection{Handshake Operator on Synchronous-Time Words}

In real-life collaborative scenarios, certain actions can succeed only when executed \emph{jointly} by several agents, such as a handover that requires one agent to release an object while another receives it at the same time. We now explore how to model this in the synchronous time setting by adapting the notion of synchronization in CSP, where a process offers a set of events it is willing to engage in; then execution proceeds on an event only when all processes that mention that event agree to it simultaneously \cite{Hoare85}.
CCS has a related rendezvous mechanism, but it synchronizes complementary actions and hides the communication as an internal action rather than requiring the same visible label on both sides \cite{Milner89,milner1983calculi}. We use two constructions to capture this synchronization: one keeps the two local observations as a product word, and the other collapses the synchronized product into a single word.

The operator \(\parallel_{\mathrm{hs}}^{A}\) keeps the two local observations visible by returning a product word in which each round is a tuple; the collapsed form is introduced below.
\begin{definition}[Lockstep with handshakes on synchronous words]\label{def:lockstep-hs}
Let \(A\subseteq \Sigma_1\cap\Sigma_2\) be a nonempty set of handshake actions, and let \(\taust_1=\trace{(s_1),\dots,(s_{T_1})}\) and \(\taust_2=\trace{(r_1),\dots,(r_{T_2})}\) be extended with stutters to the common horizon \(T=\max(T_1,T_2)\).

The lockstep with handshakes operator is
\[
\taust_1 \parallel_{\mathrm{hs}}^{A} \taust_2
:=\trace{(\widehat{s}_1,\widehat{r}_1),\dots,(\widehat{s}_T,\widehat{r}_T)}.
\]
where for each round $t\in\{1,\dots,T\}$,
\[
\widehat{s}_t :=
\begin{cases}
s_t, & \text{if } s_t\notin A \text{ or } s_t=r_t,\\
(-), & \text{otherwise,}
\end{cases}
\qquad
\widehat{r}_t :=
...
Another possible representation is to merge each paired round event returned by \(\parallel_{\mathrm{hs}}^{A}\) into a single event. The collapsed construction below turns successful shared actions into one common letter and keeps private actions traceable by recording which agent performed them.

\begin{definition}[Collapsed lockstep with handshakes]\label{def:collapsed-hs}
Let \(P_1=\Sigma_1\setminus A\) and \(P_2=\Sigma_2\setminus A\), and let \(\tau=\taust_1\parallel_{\mathrm{hs}}^{A}\taust_2\) be a product word returned by Definition~\ref{def:lockstep-hs}.
To collapse such pairs into single events, we use the alphabet
\[
\Sigma_{\parallel}:=A\cup P_1^{(1)}\cup P_2^{(2)}\cup(P_1^{(1)}\times P_2^{(2)}),
\]
where, for \(i\in\{1,2\}\), \(P_i^{(i)}:=\{p^{(i)}\mid p\in P_i\}\). The notation \(p^{(i)}\) means that agent \(i\) performs the private action \(p\). The collapse \(\mathsf{coll}_A\) is applied pointwise to each tuple \((s_t,r_t)\):
\[
...
\end{definition}
Thus, \(\mathsf{coll}_A\) maps an uncollapsed product word returned by \(\parallel_{\mathrm{hs}}^{A}\) to a word over \(\Sigma_{\parallel}\cup\{(-)\}\).
\begin{example}[Handover as handshake, failures and successes]
  Let \(\Sigma_1=\{\textsf{pick},\textsf{handover}\}\), \(\Sigma_2=\{\textsf{scan},\textsf{handover}\}\). The set \(A=\{\textsf{handover}\}\) contains the collaborative handshake action, and
  \( P_1=\{\textsf{pick}\} \) and \( P_2=\{\textsf{scan}\} \) are the specific actions for Agent 1 and Agent 2, respectively.
  \begin{itemize}
      \item \emph{Success (aligned handshake).}\\
      \(\taust_1=\trace{(\textsf{pick}),(-),\textsf{handover}}\), \(\taust_2=\trace{(-),\textsf{scan},\textsf{handover}}\).
      \\The lockstep with handshakes result is
      \[
      \taust_1 \parallel_{\mathrm{hs}}^{A} \taust_2
      =
      ...
      \item \emph{Unmatched handshake attempts.}\\
      \(\taust_1=\trace{(-),\textsf{handover},(-)}\), \(\taust_2=\trace{\textsf{handover},(-),(-)}\).
      \\At \( t=1 \), only Agent 2 attempts \(\textsf{handover}\); at \( t=2 \), only Agent 1 attempts it.
      The unmatched handshake attempts are replaced by stutters, so
      \[
      \taust_1 \parallel_{\mathrm{hs}}^{A} \taust_2
      =
      \trace{((-),(-)),\ ((-),(-)),\ ((-),(-))}.
      \]
      The collapsed result is \(\trace{(-),(-),(-)}\).
      
      \item \emph{Simultaneous private actions.}\\
      \(\taust_1=\trace{(-),\textsf{pick},(-)}\), \(\taust_2=\trace{(-),\textsf{scan},(-)}\).
      \\At round \( t=2 \), \( s_2=\textsf{pick} \) is in $P_1$ and \( r_2=\textsf{scan} \) is in $P_2$.\\
      The lockstep with handshakes result is
      \[
      \taust_1 \parallel_{\mathrm{hs}}^{A} \taust_2
      =
 ...
 \ref{tab:word-models-summary} summarizes the word models, morphisms, and synchronization operators introduced in this section.
\begin{table}[h!]
  \centering
  ...
  Metric-Time & $\taumt$ & $(\Sigma \times \mathbb{N})^*$ & quantitative timestamps \\
  ...
  Async. Interleaving & $\shuffle$ & $\tault \times \tault$ & All order-preserving interleavings. \\
  Lockstep Zip & $~~~~~\parallel_{\mathrm{lock}}$ & $\taust \times \taust$ & Aligns rounds strictly. \\
  Handshake & $~~~\parallel_{\mathrm{hs}}^{A}$ & $\taust \times \taust$ & Synchronizes shared actions,\\
  & & & allows private actions per round. \\
  \bottomrule
  \end{tabular}
  \end{table}

  This concludes the preliminaries. In this section we have fixed the two semantic objects used in the rest of the dissertation: symbolic time constraints and execution words. The interval-set domain introduced in \ref{def:canonis}, together with its arithmetic in \ref{isarithmetic} and metric-time words, provides the basis for the metric-timed conflict analysis in \ref{chap:conflict}. The synchronous-time traces introduced here, together with the abstraction inspired by synchronization over a shared alphabet, provide the execution model used for collaborative contracts and blame reasoning in \ref{chap:monitoring}. 
