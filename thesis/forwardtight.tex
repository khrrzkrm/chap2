...
\begin{lemma}[Local disjointness of tight satisfaction and violation for binary operators]
  ...
  \begin{proof}
    ...
    \paragraph{Case reparation $(\repair)$.}
    ...
    and
    $\trace{A_1'}\ \satt\ C_3$
    (via the direct satisfaction of $\obl[1]{\PAY}$).
    Thus,
    \[
...
For repetition constructors $C^n$ and $\repit{C}$, the claim follows from Lemma~\ref{lem:repetition-disjoint} and the induction hypothesis for $C$.
For triggered and guarded constructors $\trig[re]{C}$ and $\guard[re]{C}$, the claim follows from Lemma~\ref{lem:regex-disjoint} together with the induction hypotheses for $re$ and $C$.
Covering all contract constructors.
\end{proof}
% --- END INSERTED LEMMA ---
...
We reason point-wise on prefixes of $\pi$ (as defined in \ref{traces}) and lift the result to the trace-level relations via Definition~\ref{def:postprecont}.
