...
For the tagged collaboration alphabet $\Sigma = \Sigma_C^{(1)} \cup \Sigma_C^{(2)}$ and the induced letter alphabet $\Gamma = 2^{\Sigma}$, traces, prefixes, suffixes, and their basic operators are defined in \ref{traces}.
...
\]
The relation $\satt$ holds exactly at the \emph{first prefix}
...
Using the direct satisfaction of $\obl[1]{\PAY}$.
    ...
    \quad
    \text{and any extension }\trace{A_1',A_2}\text{ results in }\postsat(C_2\wedge C_3).
    \]
...
\end{definition}
  ...
  Repetition contracts express the iterative enforcement of a subcontract.
   $C^n$, the $n$-th repetition of a contract requires $C$ to be satisfied $n$ times in sequence.
  ...
  The operator $\repit{C}$ models \emph{renewable or continuing} contracts such as subscriptions or recurring payments, where each cycle restarts the same normative condition indefinitely.
...
A_2'=\emptyset.
\]
The trigger activates in month 1, but the obligation is still to be fulfilled:
...
$\notifterm^{(1)}$, and for the three additional steps afterward.”
For each time point within this time window, the obligation to pay rent repeats.
...
Semantic coherence is a required property in logic to ensure that reasoning does not lead to different conclusions. In the case of forward semantics, it means that for any given contract and trace, the semantics must produce exactly one of five possible verdicts. This principle should also apply to prefixes: if a prefix fully satisfies a contract, none of its sub-prefixes can violate it, and vice versa. The main goal is to show that tight satisfaction and tight violation do not overlap, and then t use this result to generalize coherence for the remaining three verdicts. The next lemma proves both that these cases are separate and that sub-prefixes cannot contradict their prefixes, all at once.
...
\emph{Claim (1).}
Assume for the sake of contradiction that there exists $\pi\ \satt\ (C\wedge C')$ and $\pi\ \violt\ (C\wedge C')$.
...
If $\pi\ \violt\ C$ then either (1) $k=s$, but $\pi\ \satt\ C$ and $\pi\ \violt\ C$, contradicting (IH-$C$-1). (2) $k<s$, the prefix $\pi_k\ \satt\ C$ contradicts (IH-$C$-3).

\emph{Claims (2) and (3).}
Conjunction establishes a maximal position for satisfaction $j = \max(k,k')$ such that $\pi_k\ \satt\ C$ and $\pi_{k'}\ \satt\ C'$.
For any $j' < j$, either $j' < k$ or $j' < k'$, by (IH-$C$-2) and (IH-$C'$-2), no conjunct is tightly violated at $\pi[1,j']$, hence $C\wedge C'$ is not violated either.
If $\pi\ \violt\ (C\wedge C')$ via a violation of conjunct $C$, then (IH-$C$-3) forbids any earlier $\pi[1,j']\ \satt\ C$, which is necessary for $\pi[1,j']\ \satt\ (C\wedge C')$.
...
In case~(i), the satisfaction of $\pi_k\ \satt\ C$ contradicts (IH-$C$-3).
In case~(ii),  using (IH-$C$-2) and (IH-$C$-3) ensures that tight satisfaction for $C$ occurs at a unique prefix, so $k = m$. Thus, $\pi^{k}\ \satt\ C'$ and $\pi^{k}\ \violt\ C'$, contradicting the continuation hypothesis for the right hand-side hypothesis (IH-$C'$-1).

\emph{Claims(2) and (3).}
Sequence operates on the decisive point $k$, for any $j \le k$, applying (IH-$C$-2) ensures that $\pi[1,j]\ \violt\ C$, and similarly for $j > k$, (IH-$C'$-2) for $C'$. 
...
And for the satisfaction semantics, either (a)~$\pi\ \satt\ C$, or (b)~there exists $m$ with $\pi_m\ \violt\ C$ and $\pi^{m}\ \satt\ C'$.
In case~(a), $\pi\ \satt\ C$ with the strict-prefix violation $\pi_k\ \violt\ C$ contradicts (IH-$C$-2).
In the case~(b), tight violation of $C$ is unique by (IH-$C$-2) and (IH-$C$-3), so $k = m$.
...
In the repaired case, (IH-$C$-3) gives the minimality of the failure point of $C$, and (IH-$C'$-2) on the suffix excludes earlier failure of the composition of both contracts before its tight success.

\paragraph{Finite repetition $C^n$.}
The proof proceeds by induction on the unfolding of $ C^n$ as $C;(C^{n-1})$ of length $ n$, and the proof for conjunction.
...
While $\pi$ \emph{closes} $re$ (i.e.\ $\pi\ \presat\ re$ or $\pi\ \satt\ re$), any tight failure of $C$ results in a tight failure of the composite.
...
We generalize the lemma to the 5 overall forward semantics.
