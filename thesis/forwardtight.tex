...
We first introduce tight satisfaction and violation as \emph{frontier-based} judgements over prefixes of a synchronous .
...
\begin{definition}[Literal tight satisfaction]\label{def:lattsat}
  ...
  For the empty word, tight satisfaction never holds (an event is required):
  \[
  \emptytrace\ \not\satt\ \ell \quad \text{for any literal } \ell.
  \]
  ...
  Similarly, the empty word never tightly violates a literal (violation requires an observed mismatch):
  \[
  \emptytrace\ \not\violt\ \ell \quad \text{for any literal } \ell.
  \]
  For a single event word $\trace{A}$:
  \[
  \begin{array}{l@{\quad}c@{\quad}l}
...
\begin{definition}[Binary Contract Operators]
  ...
  and let $k,k',k''\in \mathsf{pos}(\pi)$.
  ...
  & & \forall\,k' < \size{\pi}-1:\ \lnot(\pi_{k'}\ \violt\ (C \wedge C')),
  \\[2ex]
...
\begin{lemma}[Local disjointness of tight satisfaction and violation for binary operators]
  ...
  From $\pi \satt (C\wedge C')$ there exist $k',k''$ such that
  $\pi_{k'} \satt C$ and $\pi_{k''} \satt C'$.
  From $\pi \violt (C\wedge C')$, by its defining clause, either $\pi \violt C$ or $\pi \violt C'$ holds.
  If $\pi \violt C$, then $\pi$ both tightly satisfies $C$ (at prefix $\pi_{k'}$ and hence on $\pi$ by the tight frontier discipline) and tightly violates $C$, contradicting the disjointness hypothesis for $C$.
  The case $\pi \violt C'$ is symmetric.
  
  \paragraph{Case $;$.}
  Assume $\pi \satt (C;C')$ and $\pi \violt (C;C')$.
  From $\pi \satt (C;C')$ there exists $k$ such that
  $\pi_k \satt C$ and $\pi^{k+1} \satt C'$.
  From $\pi \violt (C;C')$, either $\pi \violt C$ or there exists $m$ such that
  $\pi_m \satt C$ and $\pi^{m+1} \violt C'$.
  If $\pi \violt C$, then $\pi_k \satt C$ yields a satisfaction frontier for $C$ on a prefix of $\pi$, contradicting disjointness for $C$ along prefixes.
  Otherwise, we have $\pi^{m+1} \violt C'$ while also $\pi^{k+1} \satt C'$ for some split point.
  By applying the disjointness hypothesis to the relevant suffix, this is impossible.
  
  \paragraph{Case $\repair$.}
  Assume $\pi \satt (C\repair C')$ and $\pi \violt (C\repair C')$.
  From $\pi \violt (C\repair C')$ there exists $k$ such that
  $\pi_k \violt C$ and $\pi^{k+1} \violt C'$.
  For $\pi \satt (C\repair C')$ there are two possibilities.
  If $\pi \satt C$, then $\pi_k \violt C$ gives a violation frontier of $C$ on a prefix while $C$ is tightly satisfied, contradicting disjointness for $C$ along prefixes.
  Otherwise, there exists $m$ such that $\pi_m \violt C$ and $\pi^{m+1} \satt C'$.
  Then $\pi^{k+1} \violt C'$ and $\pi^{m+1} \satt C'$ contradict the disjointness hypothesis for $C'$ on suffixes.
  
  In all cases we obtain a contradiction, so $\pi$ cannot both tightly satisfy and tightly violate the composite.
  \end{proof}
...
From the violation clause in Definition~\ref{def:trigger-guard-semantics},
there exists $k<s$ such that $\pi_k\ \satt\ re$ and $\pi^{k+1}\ \violt\ C$.
From the satisfaction clause, either (a) $\pi\ \violt\ re$ or (b) there exists $m<s$ such that
$\pi_m\ \satt\ re$ and $\pi^{m+1}\ \satt\ C$.
Case (a) contradicts the disjointness assumption for $re$ because $\pi_k\ \satt\ re$.
In case (b), we have a suffix that tightly violates $C$ (namely $\pi^{k+1}$)
while some suffix tightly satisfies $C$ (namely $\pi^{m+1}$).
By applying the disjointness hypothesis for $C$ to the relevant suffix (using the suffix-disjointness assumption),
this is impossible.
...
Case (ii) contradicts disjointness for $C$ since it would give $\pi\ \satt\ C$ and $\pi\ \violt\ C$.
In case (i), we have both $\pi\ \clossat\ re$ and $\pi\ \violt\ re$, which is impossible because
$\pi\ \clossat\ re$ abbreviates $(\pi\ \presat\ re\ \sor\ \pi\ \satt\ re)$ and by assumption
$\presat re$ excludes $\violt re$, while $\satt re$ excludes $\violt re$.
...
\begin{definition}[Repetition Contracts]
  ...
  We denote by $\pi^k$ the suffix $\pi[k,s]$, and by $\pi_k$ the prefix $\pi[0,k]$.
  ...
  \big(\exists\,k < s,\ \exists\, 1 \le m < n:\ \pi_k\ \satt\ C^m\ \nd\ \pi^{k+1}\ \violt\ C\big),
  \\[1.5ex]
  \pi\ \satt\ \repit{C}
  &\mydef&
  \text{false},
  ...
  A violation occurs either when the first occurrence of $C$ fails, or when some later repetition cannot be fulfilled after a previously satisfied segment (captured by the split $\pi_k \satt C^m$ and $\pi^{k+1} \violt C$).
  ...
  \pi\ \satt\ \trig[re]{C}
    ...
    \pi\ \violt\ re\
    \sor\
    \big(\exists\,k < s:\ \pi_k\ \satt\ re\ \nd\ \pi^{k+1}\ \satt\ C\big),
    \\[1.2ex]
  
    \pi\ \violt\ \trig[re]{C}
    &\mydef& 
    \exists\,k < s:\ \pi_k\ \satt\ re\ \nd\ \pi^{k+1}\ \violt\ C,
    \\[2ex]
...
\emph{Base: literals.} Disjointness holds by Definition~\ref{def:lattsat}, since on a single letter the satisfaction and violation clauses are complementary.
...
\begin{theorem}[Consistency of the forward-looking five tight semantics]\label{thm:five-partition}
...
We reason pointwise on prefixes of $\pi$ (as defined in Section~\ref{traces}) and lift the result to the trace-level relations via Definition~\ref{def:postprecont}.
...
Moreover, constructor-wise disjointness holds pointwise for every prefix, that is, $\neg(\Pi_{\min}\ \satt\ C\ \nd\ \Pi_{\min}\ \violt\ C)$ for every prefix $\Pi_{\min}$.
This is established formally in Lemma~\ref{lem:constructor-disjoint}.

Now consider any prefix position $j$ with $0\le j<|\pi|$ and view the corresponding trace prefix $\pi[0,j]$.
By Definition~\ref{def:postprecont}, for a given prefix $\Pi_{\min}:=\pi[0,j]$ exactly one of the following holds:
