...
\qquad
\pi\ \vDash_{\botpp{S}}\ \ell\ \Longleftrightarrow\ \exists\,j<|\pi|:\ \pi[1,j]\ \vDash_{\bottp{S}}\ \ell.
\]
\end{definition}
...
\[
\mathsf{Blame}(C,\pi[1,k]) \in \mathbb{V}_{11}
\]
as follows:
\[
\mathsf{Blame}(C,\pi[1,k]) =
\begin{cases}
\mathsf{?}
  & \text{if }\pi[1,k]\ \presat\ C,\\[4pt]
\topt
  & \text{if }\pi[1,k]\ \satt\ C,\\[4pt]
\topp
  & \text{if }\pi[1,k]\ \postsat\ C,\\[4pt]
\bot^t_S
  & \text{if }\pi[1,k]\ \vDash_{\bottp{S}} C,\\[4pt]
\bot^p_S
  & \text{if }\pi[1,k]\ \vDash_{\botpp{S}} C.
\end{cases}
\]
