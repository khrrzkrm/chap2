...
Therefore, we cannot simply relabel the outputs of the Moore machine on tight satisfaction verdicts as the literals blame, for obligations and permissions, is symmetric. Instead, the monitor construction is constructed in our next definitions from the semantics again while reusing some core ideas from the previous monitor construction.
...
\begin{itemize}
    ...
    \item \textbf{Example (Reparation):} In $\tsmc_{\repair}$, transitions to any state $q$ where $\lambda_5(q)=\bott$ are redirected to the initial state of $C'$. In $\bmc_{\repair}$, transitions to \emph{any} state $q$ where $\lambda_{11}(q)=\bottp{S}$ (regardless of $S$) are redirected to the initial state of $C'$.
\end{itemize}
...
Capturing such multi-point violations would require a mechanism to accumulate and memories blames for all violation and keeps the responsability of each violation too. 
In the next section, we developp a first version of refined semantics where we keep a score for violation as well as a blame score for each agent.
