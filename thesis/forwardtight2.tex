...
\begin{proof}
    ...
    Now consider the blame semantics for literals .
    ...
    If instead $\trace{A}\ \violt\ \ell$, then by the literal blame rules the violation is refined into a (unique) blame set $S\subseteq\{1,2\}$:
...
For Sequence ($C_1 ; C_2$), Reparation ($C_1 \repair C_2$), Repetition ($C^n, \repit{C}$), and Guarded contracts, the construction follows the exact same mapping and redirection to states as the Tight Satisfaction Monitor constructions (Definitions \ref{def:moore-seq} to \ref{def:moore-repeat-unbounded}), with one key adaptation:
\begin{itemize}
