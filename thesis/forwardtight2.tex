...
However, this prefix-based view, represents a traditional and theoretic point of view of runtine verification, is a na implies a limitation regarding the completeness of the violation history. 
...
A practical consequence of this property is that, even when processing infinite words or streams, the monitor can be programmed to halt execution immediately after the first tight violation is detected, as no future event can alter the blame assignment. The resulting problem is that any subsequent violations committed by other agents at later time steps are ignored. This limitation is particularly evident for open-ended contracts involving the repetition operator. 
...
This is a notable flaw, as in law and normative systems, one typically has to account for all violations to determine the full extent of liability or penalties.
Capturing such multi-point violations would require a mechanism to reset or accumulate, and parallelize monitoring threads after a failure.
Finally, extending this framework to support a cumulative blame semantics suggests interesting theoretical challenges that we attempt to solve in the next part.
