...
\[
  ...
  \]
This refined judgment structure allows the monitors constructed in the next section to pinpoint the agents responsible for each contractual breach.
...
\begin{example}[Obligation, prohibition, and power blame]
  Fix $p=1$ (tenant) and $\bar p=2$ (landlord), and consider the same four one-step letters $A_1,\dots,A_4\in\Gamma$ under each literal $\obl[1]{a}$, $\frb[1]{a}$, and $\perm[1]{a}$.  
  These letters exhaust all possible attempt patterns in a single step, namely no attempt, only agent~2 attempts, only agent~1 attempts, and joint attempt, and we report the corresponding verdict in $\mathbb{V}_{11}$ for each case.

\smallskip
\noindent\emph{Obligation \(\obl[1]{a}\).}
\[
\begin{array}{lcl}
A_1=\emptyset: & \trace{A_1}\ \vDash_{\bottp{\{1\}}}\ \obl[1]{a} & \text{(subject of the obligation did not attempt)}\\
A_2=\{a^{(2)}\}: & \trace{A_2}\ \vDash_{\bottp{\{1\}}}\ \obl[1]{a} & \text{(subject did not attempt)}\\
A_3=\{a^{(1)}\}: & \trace{A_3}\ \vDash_{\bottp{\{2\}}}\ \obl[1]{a} & \text{(other party did not cooperate)}\\
A_4=\{a^{(1)},a^{(2)}\}: & \trace{A_4}\ \satt\ \obl[1]{a} & \text{(joint execution present)}
\end{array}
\]
\noindent\emph{Prohibition \(\frb[1]{a}\).}
\[
\begin{array}{lcl}
A_1=\emptyset: & \trace{A_1}\ \satt\ \frb[1]{a} & \text{(the prohibited joint act does not occur)}\\
A_2=\{a^{(2)}\}: & \trace{A_2}\ \satt\ \frb[1]{a} & \text{(joint act does not occur)}\\
A_3=\{a^{(1)}\}: & \trace{A_3}\ \satt\ \frb[1]{a} & \text{(joint act does not occur)}\\
A_4=\{a^{(1)},a^{(2)}\}: & \trace{A_4}\ \vDash_{\bottp{\{1\}}}\ \frb[1]{a} & \text{(subject should have refrained)}
\end{array}
\]
\noindent\emph{Power \(\perm[1]{a}\).}
\[
\begin{array}{lcl}
A_1=\emptyset: & \trace{A_1}\ \satt\ \perm[1]{a} & \text{(subject does not attempt)}\\
A_2=\{a^{(2)}\}: & \trace{A_2}\ \satt\ \perm[1]{a} & \text{(no unsupported subject attempt)}\\
A_3=\{a^{(1)}\}: & \trace{A_3}\ \vDash_{\bottp{\{2\}}}\ \perm[1]{a} & \text{(subject attempted, other party withheld)}\\
A_4=\{a^{(1)},a^{(2)}\}: & \trace{A_4}\ \satt\ \perm[1]{a} & \text{(properly supported)}
\end{array}
\]
\end{example}
...
For $S \subseteq\{1, 2\}$ of agent(s), and  two contracts $C$ and $C'$ from \cDL and a synchronous trace $\pi$, blame is defined for the conjunction $C \wedge C'$ is defined as:
\[
...
\end{cases}
\]
Where $\dbot$ stands for a non violation verdict, that is, $\dbot \in \{\,?,\ \topp,\ \topt\,\}$.\\
\paragraph{Intuition.}
...
\end{cases}
\]
\paragraph{Intuition.} The first decisive failure before \(C\) has tightly succeeded belongs to \(C\), so its blame propagates. Once \(C\) has tightly succeeded (\(\topt\)) or is in post-success (\(\topp\)), only \(C'\) can still fail, so the blame comes from \(C'\). There is no tie, since \(C'\) becomes active only after \(C\) has tightly succeeded.
...
\]
\paragraph{Intuition.}  A reparation clause becomes active only after a violation of \(C\). The global blame set \(S\), therefore, corresponds to the agents responsible for violating the reparation \(C'\) once it is triggered. The blame for $C$ is not considered, as one cares only for the overall violation of the combined contracts.

\paragraph{Finite repetition.}
For $S\subseteq\{1,2\}$ and a finite repetition contract $C^n$:
\[
\pi\ \vDash_{\bottp{S}}\ C^n \;\iff\;
\begin{cases}
\pi\ \vDash_{\bottp{S}}\ C,\\[2pt]
\exists \pi_k\ \vDash_{\topt} \ C \ \nd\ \pi^{k}\ \vDash_{\bottp{S}}\ C^{n-1}.
\end{cases}
\]

\paragraph{Unbounded repetition.}
For $S\subseteq\{1,2\}$ and an unbounded repetition contract $\repit{C}$:
\[
\pi\ \vDash_{\bottp{S}}\ \repit{C} \;\iff\; \exists n \ge 1 \text{ such that } \pi\ \vDash_{\bottp{S}}\ C^n.
\]
\paragraph{Intuition.} For repetition, the blame is assigned to the first iteration that violates the contract.

\paragraph{Trigger.}
For $S\subseteq\{1,2\}$ and a triggered contract $\trig[re]{C}$:
\[
\pi\ \vDash_{\bottp{S}}\ \trig[re]{C} \;\iff\;
\exists\,k \text{ such that } \pi_k\ \vDash_{\topt}\ re \ \nd\ \pi^{k}\ \vDash_{\bottp{S}}\ C.
\]

\paragraph{Guard.}
For $S\subseteq\{1,2\}$ and a guarded contract $\guard[re]{C}$:
\[
\pi\ \vDash_{\bottp{S}}\ \guard[re]{C} \;\iff\;
(\forall j \le |\pi|: \neg(\pi_j \vDash_{\topt} re)) \ \nd\ \pi\ \vDash_{\bottp{S}}\ C.
\]
\paragraph{Intuition.} For triggered and guarded contracts, the control is determined by the regular expression. The blame is always inherited from $C$ exactly when the contract mandates $C$ to hold.
...
\[
\pi_1=\langle A_1\rangle,\quad A_1=\{\OCC^{(1)}\}
\]
In this case, $\perm[1]{\OCC}$ and $ \obl[1]{\PAY}$ are violated, the blame verdicts are:
\begin{itemize}
...
Moreover, consider the trace of  $\pi_2:= \trace{\{\OCC^{(1)}\}, \{\OCC^{(1)}\}}$, the extension of $\pi_1$ with the same event, as the blame is forward and tight-looking, the blame is still assigned only to agent $2$ (landlord), as agent 2 is responsible for the first violation.

Let us consider instead the following trace $\pi_3:=\trace{A_1',A_2}$ with $A_1':= \{\OCC^{(1)}, \OCC^{(2)}\}$ and $A_2:= \{\OCC^{(1)}\}$.
...
That is, for all $v,v'\in\mathbb{V}_{11}$,
\[
...
For instance, for $\obl[p]{a}$, either $a^{(p)}\notin A$ (producing $\bottp{\{p\}}$), or $a^{(p)}\in A\land a^{(\bar p)}\notin A$ (producing $\bottp{\{\bar p\}}$), or the joint act occurs (no violation).
...
Assuming the hypothesis holds for $C_1$ and $C_2$, we prove the composition by combining them.
For conjunction $C_1\wedge C_2$, the inductive hypothesis produces unique verdicts $v_1$ and $v_2$ for $C_1$ and $C_2$ on $\pi$.
The conjunction clause then produces a unique combined verdict: it matches a unique blame set for both contracts by inductive hypothesis and the union operation is also deterministic.
...
The triggering point is unique because the underlying tight regions are prefix-monotone, and the inductive hypothesis produces a unique verdict for the suffix evaluated against $C_2$.
Hence, the overall verdict of blame is unique.
...
For literals, blame is sensitive to the tagged agent assigned to the literal. In both cases, the blame is attributed based on disjoint conditions, but their union covers exactly all the cases that are not literal violations.
\paragraph{Inductive step.}
Assume the statement holds for all strict subcontracts of $C$. 
For the conjunction $C=C_1\wedge C_2$, if a blame set is assigned to one component for a fixed prefix, then that is the case for the violation semantics. In the case where both contracts are violated simultaneously, the assigned blame set is their union, which is the only difference from the violation.
For all other compound operators, namely sequence, repetition, trigger, and guard, the blame semantics are carried in the exact same manner as the violation semantics by propagating their inductive blame set.
The uniqueness of the blame set follows from Lemma~\ref{lem:blame-deterministic}.

\end{proof}

\subsection{From Tight Contract Satisfaction Monitor to Tight Blame Monitor}

In the previous subsection, we defined a denotational blame semantics that refines tight and post violations by assigning responsibility to one or both agents. We now show that this refinement can be implemented in practice.
...
Therefore, we cannot simply relabel the outputs of the Moore machine on tight satisfaction verdicts as the literals blame, for obligations and permissions, is symmetric. Instead, the blame monitor is constructed in our next definitions from the semantics again, while reusing some core ideas from the previous monitor construction.

\begin{definition}[Moore machine on blame verdicts]
\label{def:blamemonitor}
A \emph{Moore machine on blame verdicts}, written $\mathcal{M}_{11}$, is a Moore machine whose output alphabet is the eleven-valued blame verdict set $\mathbb{V}_{11}$. that is:
\[
...
We now define the construction function $\bmc(C)$ inductively. This follows the same structural approach as the tight satisfaction monitor construction $\tsmc(C)$ in Definition~\ref{def:tsmc}, but creates distinct states for distinct blame assignments.
...
The blame monitor for conjunction $\bmc(C \wedge C')$ runs the two sub-monitors synchronously on the Cartesian product of their state spaces, identical to the tight satisfaction construction. Let $q$ and $q'$ be states from the monitors for $C$ and $C'$ respectively, and let $\lambda_{11}$ and $\lambda'_{11}$ be their respective output functions. The output function $\lambda_{11}^{\wedge}$ explicitly combines blame sets when both states indicate a violation in the exact same step, where (q,q') is the product state in $\bmc(C \wedge C')$:
\[
\lambda_{11}^{\wedge}(q, q') =
\begin{cases}
\bottp{S \cup S'} & \text{if } \lambda_{11}(q) = \bottp{S} \text{ and } \lambda'_{11}(q') = \bottp{S'}, \\
\bottp{S} & \text{if } \lambda_{11}(q) = \bottp{S} \text{ and } \lambda'_{11}(q') \in \{\mathsf{?}, \topp, \topt\}, \\
\bottp{S'} & \text{if } \lambda'_{11}(q') = \bottp{S'} \text{ and } \lambda_{11}(q) \in \{\mathsf{?}, \topp, \topt\}, \\
\botpp{S \cup S'} & \text{if } \lambda_{11}(q) = \botpp{S} \text{ and } \lambda'_{11}(q') = \botpp{S'}, \\
\botpp{S} & \text{if } \lambda_{11}(q) = \botpp{S} \text{ and } \lambda'_{11}(q') \in \{\mathsf{?}, \topp, \topt\}, \\
\botpp{S'} & \text{if } \lambda'_{11}(q') = \botpp{S'} \text{ and } \lambda_{11}(q) \in \{\mathsf{?}, \topp, \topt\}, \\
\lambda_{5}^{\wedge}(q, q') & \text{otherwise (non-violating cases)}.
\end{cases}
\]

\paragraph{Other Operators.}
For Sequence ($C ; C'$), Reparation ($C \repair C'$), Repetition ($C^n, \repit{C}$), and Guarded contracts, the construction follows the exact same mapping and redirection to states as the Tight Satisfaction Monitor constructions in Definitions \ref{def:moore-seq} to \ref{def:moore-repeat-unbounded}, with one key adaptation:
\begin{itemize}
    ...
    \item \textbf{Example (Reparation):} In $\tsmc_{\repair}$, transitions to any state $q$ where $\lambda_5(q)=\bott$ are redirected to the initial state of $C'$. In $\bmc_{\repair}$, transitions to \emph{any} state $q$ where $\lambda_{11}(q)=\bottp{S}$ are redirected to the initial state of $C'$.
\end{itemize}
This preserves the control-flow semantics of the operators: for example, in reparation, \emph{any} fault by \emph{any} party in the primary contract triggers the secondary contract, thereby concealing the initial blame in favor of the reparation's outcome.
\end{definition}
...
The proof proceeds by structural induction on $C$.
\begin{itemize}
\item \textbf{Literals:} The states output function $\bmc_{\mathit{lit}}$ matches explicitly the blame partition rules.
\item \textbf{Conjunction:} The construction definition for pairs of states $\lambda_{11}^{\wedge}$ outputs the blame state  $S \cup S’$ when both components are in violation states, and selects the single responsible party when only one violates. This matches the semantic clause $\pi \vDash_{\bottp{S \cup S’}} C \wedge C’$.
\item \textbf{Other Operators:} The correctness relies on the fact that operators like Sequence and Reparation are defined by switching control phase and only editing the \emph{content} of the blame.
\end{itemize}
\end{proof}
...
\begin{example}[Blame Monitor for $C_2 \wedge C_3$]
  ...
  \node[state,fill=red!18,right=50mm of qv2] (qv1) {$s_{\bott}^1$\\[2pt]$\bottp{\{1\}}$};
...
To illustrate a scenario where the monitor can output the joint blame verdict $\bottp{\{1,2\}}$, we consider the reparation-free reduction of the specification: $\perm[1]{\OCC} \wedge \obl[1]{\PAY}$.
  ...
  \begin{example}[Blame Monitor with double blame]\label{example:joint-blame}
    ...
    \node[state,fill=red!18,below=7mm of qs] (q1) {$s_{\bott}^{1}$\\[2pt]$\bottp{\{1\}}$};
    ...
    \begin{example}[Blame Monitor for $\repit{C_3}$]
      The figure below shows the blame monitor for the unbounded repetition of the rent-and-reparation contract. The generic violation state $q_{\bott}$ from the standard monitor is split into $s_{\bott}^1$ and $s_{\bott}^2$. In particular, once the monitor transitions to a post-violation sink such as $s_{\botp}^1$, it loops on any input $\Gamma$. This shows the "first blame" limitation: if the tenant is blamed for missing a fine, the monitor will never blame the landlord for any future misconduct.
      ...
      \node[state,fill=red!18,above right= 4 mm and 20mm of qw]       (qv1)  {$s_{\bott}^1$\\[2pt]$\bottp{\{1\}}$};
...
We have presented a first family of forward-looking semantics and a corresponding monitor construction that refine the standard satisfaction verdicts by assigning responsibility. 
This approach is computationally fast and provides immediate feedback on the \emph{status} of the contract, enabling runtime enforcement and dispute resolution at the moment of the breach.
However, this prefix-based view represents a traditional, theoretical perspective on runtime verification in a true or false lazy computation, which implies a limitation in the completeness of the violation history:
the semantics are designed to detect the \emph{first} decisive violation that renders the contract permanently unsatisfiable. Once the monitor transitions to a post-violation sink state ($\botpp{S}$), the verdict becomes immutable. 
A practical consequence of this property is that, even when processing infinite words or streams, the monitor can be programmed to halt execution immediately after the first tight violation is detected, as no future event can alter the blame assignment. The resulting problem is that any subsequent violations committed by other agents at later time steps are ignored. This limitation is particularly evident for open-ended contracts as our motivating example where the termination of the contract enforcement is non related to the first violation. 
...
Even if the interaction continues and the landlord subsequently violates their permission or obligation in a future cycle, such as by blocking a valid payment attempt, the monitor remains fixed on the initial verdict $\botpp{1}$. 
Therefore, while this first family of semantics is sufficient for determining \emph{why} a contract failed, it does not support a cumulative accounting of \emph{all} violations that occur throughout the lifespan of a long-running interaction beyond the first violation.

 In contracts law, one always has to account for all violations to determine the full extent of liability or penalties for litigation.
Capturing such multi-point violations would require a mechanism to accumulate and remember blame for all violations, while also tracking the responsibility for each individual violation. 
In the next section, we develop a first version of refined semantics where we keep a score for violations as well as a blame score for each agent.
