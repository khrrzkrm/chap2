...
\[
\begin{array}{lcl}
A=\emptyset: & \trace{A}\ \vDash_{\bottp{\{2\}}}\ \obl[1]{a} & \text{(subject did not attempt)}\\
A=\{a^{(2)}\}: & \trace{A}\ \vDash_{\bottp{\{2\}}}\ \obl[1]{a} & \text{(subject did not attempt)}\\
...
\[
\begin{array}{lcl}
A=\{a^{(1)},a^{(2)}\}: & \trace{A}\ \vDash_{\bottp{\{2\}}}\ \frb[1]{a} & \text{(subject should have refrained)}\\
...
\begin{itemize}
\item The tenant (1) gets blamed for violating the obligation to pay rent:\\ $\pi_1 \vDash_{\bottp{\{2\}}} \obl[1]{\PAY}$. 
...
For a literal $\ell = \obl[p]{a}$, the monitor must distinguish between subject failure (blame $p$) and counterparty interference (blame $\bar{p}$).
\[
\bmc_{\mathit{lit}}(\ell) = (Q_{\ell}, q_0, \Gamma, \mathbb{V}_{11}, \delta_{\ell}, \lambda_{11}).
\]
\begin{itemize}
    \item The state set splits the generic violation state into two distinct blame states:
    \[
    Q_{\ell} = \{q_0, q_s, q_{ps}\} \cup \{q_{\bott}^p, q_{\botp}^p\} \cup \{q_{\bott}^{\bar{p}}, q_{\botp}^{\bar{p}}\}.
    \]
    \item The transition function $\delta_{\ell}$ directs the violation to the correct state based on the input letter $A$:
    \[
    \delta_{\ell}(q_0, A) = 
    \begin{cases}
        q_s & \text{if } A \satt \ell, \\
        q_{\bott}^p & \text{if } A \violt \ell \text{ and } a^{(p)} \notin A, \\
        q_{\bott}^{\bar{p}} & \text{if } A \violt \ell \text{ and } a^{(p)} \in A \land a^{(\bar{p})} \notin A.
    \end{cases}
    \]
    Transitions from success states ($q_s$) go to $q_{ps}$. Transitions from tight blame states ($q_{\bott}^S$) go to their corresponding post-blame states ($q_{\botp}^S$).
    
    \item The output function $\lambda_{11}$ maps $q_{\bott}^S \mapsto \bottp{S}$ and $q_{\botp}^S \mapsto \botpp{S}$.
\end{itemize}
(The construction for $\frb[p]{a}$ and $\perm[p]{a}$ is analogous, with transitions defined by their respective blame rules).

\paragraph{Inductive Step: Conjunction ($C_1 \wedge C_2$).}
Let $\bmc(C_1)$ and $\bmc(C_2)$ be the blame monitors for the sub-contracts. The blame monitor for the conjunction is the synchronous product:
\[
\bmc_{\wedge}(C_1, C_2) = (Q_1 \times Q_2, (q_0^1, q_0^2), \Gamma, \mathbb{V}_{11}, \delta_{\times}, \lambda_{11}^{\wedge}).
\]
The transition function $\delta_{\times}$ is the standard synchronous product of $\delta_1$ and $\delta_2$. The output function $\lambda_{11}^{\wedge}$ implements the union of blame sets when multiple violations occur. Let $v_1 = \lambda_{11}^1(q_1)$ and $v_2 = \lambda_{11}^2(q_2)$.
\[
\lambda_{11}^{\wedge}(q_1, q_2) = 
\begin{cases}
  \bottp{S_1 \cup S_2} & \text{if } v_1 = \bottp{S_1} \text{ and } v_2 = \bottp{S_2}, \\
  \bottp{S_1} & \text{if } v_1 = \bottp{S_1} \text{ and } v_2 \in \{\mathsf{?}, \topt, \topp\}, \\
  \bottp{S_2} & \text{if } v_2 = \bottp{S_2} \text{ and } v_1 \in \{\mathsf{?}, \topt, \topp\}, \\
  \lambda_{11}^{\textsf{comb}}(v_1, v_2) & \text{otherwise (non-violating combination)}.
\end{cases}
\]
Post-violations ($\botpp{S}$) follow the same logic.
...
\begin{example}[Blame Monitor for $C_2 \wedge C_3$]
  ...
  \node[state,fill=red!18,right=50mm of qv2] (qv1) {$s_{\bott}^1$\\$\bottp{\{2\}}$};
...
To illustrate a scenario where the monitor can output the joint blame verdict $\bottp{12}$, we consider the reparation-free reduction of the specification: $\perm[1]{\OCC} \wedge \obl[1]{\PAY}$.
  ...
  \begin{example}[Blame Monitor with double blame]\label{example:joint-blame}
    ...
    \node[state,fill=red!18,below=7mm of qs] (q1) {$s_{\bott}^{1}$\\$\bottp{\{2\}}$};
    ...
    \begin{example}[Blame Monitor for $\repit{C_3}$]
      ...
      \node[state,fill=red!18,above right= 4 mm and 20mm of qw]       (qv1)  {$s_{\bott}^1$\\$\bottp{\{2\}}$};
