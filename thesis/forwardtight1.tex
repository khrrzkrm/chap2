...
\begin{remark}[Contradicting literals]
  \label{rem:contradicting-literals}
   ...
   $ \nexists A\in\Gamma:\ \bigl(\trace{A}\ \satt\ \obl[1]{a}\ \land\ \trace{A}\ \satt\ \frb[1]{a}\bigr), \nd\\
    \nexists A\in\Gamma:\ \bigl(\trace{A}\ \satt\ \perm[1]{a}\ \land\ \trace{A}\ \satt\ \frb[2]{a}\bigr).$
    ...
    \tsmc_{\wedge}\bigl(\tsmc(\perm[1]{a}),\tsmc(\frb[2]{a})\bigr),
  $$
  ...
  \paragraph{Perspective of unsatisfiability via monitor structure.}
  This observation suggests a natural automata-theoretic characterization of contract unsatisfiability.
  We propose to study this property by identifying contracts whose tight satisfaction monitors admit no accepting behaviors.
  Specifically, we define a contract $C$ as unsatisfiable if \emph{every} run of its corresponding tight forward monitor construction $\tsmc(C)$ inevitably reaches a tight violation state $\bott$ and subsequently the permanent violation sink $\botp$.
  In graph-theoretic terms, this condition holds if the monitor graph contains no infinite path that avoids $\bott$.
  This definition links semantic inconsistency to operational inevitability, enabling unsatisfiability checks via reachability analysis rather than direct semantic reasoning.
...
\begin{remark}[Analogy with Thompson's Construction]
  ...
  In Thompson's method, sub-automata for smaller expressions are wired together via $\varepsilon$-transitions: for concatenation, the accepting state of the first fragment is linked to the start state of the second; for alternation, a new initial state fans out to both sub-fragments.
  Our sequence and reparation compositions apply the same principle: the decisive states of the left monitor ($\topt$ for sequence, $\bott$ for reparation) are redirected to the initial state of the right monitor, effectively wiring the two sub-monitors into a single automaton.
  ...
  This is possible because the contract grammar of \cDL does not include a disjunction operator at the contract level: contracts are combined via conjunction, sequence, and reparation, all of which admit a deterministic left-to-right evaluation.
...
We move now to guarded contracts monitor construction. Unlike conjunction, the guarded contract $\guard[re]{C}$ cannot be monitored via a naive synchronous product. While the tight frontier is determined by the instantaneous state of $re$ and $C$, the stable post-verdicts ($\topp, \botp$) require historical context: the monitor must remember if the decisive frontier has effectively been crossed. A simple pointwise combination of component outputs fails to capture this latching behavior. Consequently, our construction restricts the synchronous product to the active tight region and handles post-verdicts via explicit transitions to global sinks, ensuring the monitor stabilizes correctly immediately after the decisive step. 
    ...
    \begin{definition}[Guarded Monitor Construction]
      ...
      \paragraph{Intuition.}
      The guarded monitor construction separates the detection of the decisive frontier from the maintenance of the stable verdict.
      
      \smallskip
      \noindent\emph{Phase~1: Tight-core product.}
      The core machine $M_{\guardd}^{\circ}$ restricts the synchronous product to the ``active'' regions of the components. Its responsibility is solely to identify the \emph{earliest decisive prefix}.
      While the interaction is undecided, the monitor evolves within this core.
      If the guard closes safely or the contract succeeds, the core reaches a state labeled $\topt$.
      Conversely, if the contract is breached while the guard is active, the core reaches a state labeled $\bott$.
      
      \smallskip
      \noindent\emph{Phase~2: Post-verdict stabilization.}
      The full monitor wraps the core with a transition logic that enforces stability.
      Instead of computing product states for the post-regions (which would be redundant), the transition function detects when the core has reached a frontier.
      If the current state emits a tight verdict ($\topt$ or $\bott$), the \emph{next} transition leaves the core and enters the corresponding global sink ($q_{\topp}$ or $q_{\botp}$).
      This ensures that the monitor emits the tight verdict for exactly one step, preserving the boundary semantics, before stabilizing permanently.
      ...
      The proof relies on the decomposition of the trace into a decisive prefix and a stable suffix.
      
      \smallskip
      \emph{Tight-core correctness.}
      For any prefix $\pi$ that has not strictly passed a decisive point, the monitor remains in the core set $Q_r^{\circ} \times Q_c^{\circ}$.
      In this region, $\delta_{\guardd}$ acts as a standard synchronous product.
      The core output function $\lambda_{\guardd}^{\circ}$ directly implements the tight semantic clauses: it evaluates to $\bott$ upon an active violation, and to $\topt$ upon release (guard failure) or active satisfaction.
      ...
      Since $q_{\topp}$ is absorbing and outputs $\topp$, the monitor correctly stabilizes.
