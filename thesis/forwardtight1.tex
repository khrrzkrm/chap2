...
For every contract $C$ in \cDL, every finite trace $\pi$, and every prefix $\pi[0,k]$,
the state reached by the monitor $\tsmc(C)$ after reading $\pi[0,k]$
emits exactly the verdict prescribed by the five-valued tight semantics, that is:
\[
\lambda_5\bigl(\delta(q_0,\pi[0,k])\bigr) \;=\; \semfive{\pi[0,k] \vDash C}.
\]
