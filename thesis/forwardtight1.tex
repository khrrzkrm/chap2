...
Building on the formal definition of \cDL traces' semantics, we define an operational implementation using a finite-state monitor that incrementally tracks compliance accordingly.
Those monitors are designed as Moore-style machines, maintaining a running summary of the interaction in which each state is labeled with one of the five possible verdicts. This deterministic evaluation provides an automated mechanism for compliance checking that conforms strictly to the prefix semantics of \cDL.
...
\]
We rely on correctness lemmas to establish, inductively, that this invariant holds for all monitor constructors. The next subsections construct the tight satisfaction monitors compositionally based on the contract syntax. 

In our construction, we use the same principle as Thompson's construction for regular expressions. However, we do not have an intermediate transformation to non-deterministic automata. Instead, the construction is performed directly on deterministic Moore machines. Verdict propagation is encoded natively in the monitor's states and their labels, allowing tight satisfaction and violation to be observed on trace prefixes.
\subsubsection*{Construction for Literal Contracts}
\paragraph{From Tight Semantics to 5-Valued Monitoring.}
The literal clauses above define one-step satisfaction and violation judgments for a single event word $\trace{A}$.
...
$
\tsmc_{\mathit{lit}}(\ell)
  = (Q, q_0, \Gamma, \tightverdicts, \delta, \lambda_5), \text{ with }$
...
\paragraph{Construction mechanism.} For an atomic literal, the monitor initializes at $q_0$, emitting the undecided verdict $\mathsf{?}$.
Upon consuming the first event, the execution transitions to either $q_s$ with verdict $\topt$ if the literal is satisfied, or $q_v$ with verdict $\bott$ if it is violated.
On any subsequent step, the monitor enters the absorbing states $q_{ps}$ or $q_{pv}$, permanently emitting $\topp$ or $\botp$ respectively.
...
The first joint execution of $a^{(1)}$ and $a^{(2)}$ produces~$\topt$,
...
$C \wedge C'$, written $\tsmc_{\wedge}(\tsmc(C),\tsmc(C'))$, is defined as:
\[
\tsmc_{\wedge}(\tsmc(C),\tsmc(C'))
...
\begin{itemize}
  ...
  \emph{Note:} The case $\topt \sqcap \topp = \topt$ reflects that if one component is exactly at its satisfaction frontier ($\topt$) while the other is already past it ($\topp$), the conjunction as a whole is determined by the latter of the two, thereby placing the global frontier at the current step.
...
into the initial state of $C'$. This produces a tight prefix monitor that exactly matches the semantics of sequential composition.

\begin{remark}[Contradicting literals]
  \label{rem:contradicting-literals}
    Consider the literal pairs $\obl[1]{a}\wedge\frb[1]{a}$ and $\perm[1]{a}\wedge\frb[2]{a}$ are contradictory in the sense that no single event $A\in\Gamma$ can satisfy both components simultaneously as in Definition \ref{def:lattsat}, there semantics clash:
    
   $ \nexists A\in\Gamma:\ \bigl(\trace{A}\ \satt\ \obl[1]{a}\ \land\ \trace{A}\ \satt\ \frb[1]{a}\bigr)$
  
    Consequently, following the in the synchronous-product conjunction rules from $\lambda_{5}^{\textsf{comb}}$ \\
    $$
    \tsmc_{\wedge}\bigl(\tsmc(\obl[1]{a}),\tsmc(\frb[1]{a})\bigr)
  $$
    no state with output $\topt$ or $\topp$ is reachable.
    Tight acceptance for these conjunctions is therefore unreachable: every execution either remains undecided or produces a tight or post violation, aligning exactly with the five-valued tight semantics as shown in \ref{fig:tsmc-obl-and-frb-unreachable}.
  \end{remark}

  \begin{figure}[h!]
  \centering
  \begin{tikzpicture}[
    shorten >=1pt,auto,node distance=30mm,semithick,
    every state/.style={rectangle,rounded corners,draw,minimum width=14mm,
      minimum height=7.5mm,inner sep=5pt,font=\footnotesize,align=center}
  ]
  
  % States (reachable quotient)
  \node[initial,state,fill=gray!10] (s0) {$q_0$\\[0.2pt]$\mathsf{?}$};
  \node[state,fill=red!18,right=of s0] (sv) {$q_{\bott}$\\[0.2pt]$\bott$};
  \node[state,fill=red!10,right=of sv] (sp) {$q_{\botp}$\\[0.2pt]$\botp$};
  
  % Transitions
  \path[->]
    (s0) edge node[above] {$\Gamma$} (sv)
    (sv) edge node[above] {$\Gamma$} (sp)
    (sp) edge[loop right] node {$\Gamma$} (sp);
  \end{tikzpicture}
  \caption{Reachable part of the conjunction monitor $\tsmc_{\wedge}(\tsmc(\obl[1]{a}),\tsmc(\frb[1]{a}))$.}
  \label{fig:tsmc-obl-and-frb-unreachable}
  \end{figure}
  
  \paragraph{Unsatisfiability and conflicts via monitor structure.}
The example above illustrates unsatisfiability: a contract $C$ is unsatisfiable whenever every run of $\tsmc(C)$ reaches $\bott$ and then $\botp$, that is, whenever no state labeled $\topt$ or $\topp$ is reachable from $q_0$. Checking unsatisfiability thus reduces to a reachability query.

However, for the case $\perm[1]{a}\wedge\frb[2]{a}$, the conflict only arises when agent 1 exercises their right, which is not easily detectable in either the semantics or the tight monitor construction, as reaching a tight satisfaction state remains possible. This case motivates a more refined classification of right and duty conflicts.
...
Let the two $\tsmc©$ and $\tsmc(C’)$ be the monitors for $C$ and $C’$,
and let $\tsmc_{\bm{;}}(\tsmc©,\tsmc(C’))
...
\begin{proof}
    We prove by induction on the length of the input trace $n:=|\pi|$.
    Let $M:=\tsmc_{\bm{;}}(\tsmc(C),\tsmc(C'))=(Q_{;}, q_0^{;}, \Gamma, \tightverdicts, \delta_{;}, \lambda_5^{;})$. We have:
    \[
    \lambda_5^{;}\big(\delta_{;}(q_0^{;},\pi)\big)
    \;=\;
    \semfive{\pi \vDash C;C'}.
    ...
    \mathsf{?}.
    \]
    This emulates exactly the semantic step where $C$ reaches the tight satisfaction frontier on the prefix $\pi$, and the contract $C'$ is then enforced on the remaining suffix of the trace.
    At this phase switch, the suffix is empty, so the status of $C'$ is undecided, and the sequence verdict is $\mathsf{?}$.
    Thus, the monitor output matches $\semfive{\pi \vDash C;C'}$.
    
    \smallskip
    \noindent\emph{Case 2(b): $\lambda_5(t)\notin\{\topt,\topp\}$.}
    in the switch has not occurred, so $\delta_{;}(s,A)=t$ and $\lambda_5^{;}(t)=\lambda_5(t)$.
    Therefore
    \[
    \lambda_5^{;}\big(\delta_{;}(q_0^{;},\pi)\big)
    =
    ...
    \semfive{\pi \vDash C}.
    \]
    Before $C$ reaches tight satisfaction, the sequence semantics is evaluated for the left component on the current prefix, hence $\semfive{\pi \vDash C}=\semfive{\pi \vDash C;C'}$ in this case. So the equality holds.
    
    All the possible cases are enumerated and proved, so the induction holds.
    \end{proof}
...
Let \(C\) and \(C'\) be contracts from \(\cDL\), with corresponding tight satisfaction monitors, respectively \(\tsmc(C)\) and \(\tsmc(C')\), and let
\[
\tsmc_{\repair}(\tsmc(C),\tsmc(C'))
  = (Q_{\repair}, q_0^{\repair}, \Gamma, \tightverdicts, \delta_{\repair}, \lambda_5^{\repair})
\]
...
Since all other transitions remain unchanged and the decisive repair point is unique to each trace, the proof holds.
\end{proof}
...
\begin{remark}[Analogy with Thompson's Construction]
  \label{rem:thompson-analogy}
  The compositional monitor constructions in Definitions~\ref{def:moore-seq} and~\ref{def:moore-repair} follow a pattern closely analogous to Thompson's construction for regular expressions~\cite{Thompson1968}.
Our sequence and reparation constructions work the same way as for the sequence construction in Thomson’s method: when the left monitor reaches its decisive state, $\topt$ for sequence and $\bott$ for reparation, the transition is redirected to the right monitor's initial state.
  The key difference is that we compose \emph{deterministic} Moore machines directly, without an intermediate nondeterministic step.
  This is possible because the contract grammar of \cDL does not include a disjunction operator at the contract level: contracts are combined via conjunction, sequence, and reparation, all of which admit a deterministic left-to-right evaluation. Another possible explanation is the eager definition of tight semantics itself, where we always pick the shortest prefix that satisfies or violates the contract.
  As a result, no subset construction or $\varepsilon$-closure is required, and the composed monitor is deterministic by construction.
...
&\quad \PAY^\times   &:= \Gamma\setminus \PAY^\surd,\\[2pt]
\PAYF^\surd  &:= \{\,A\in\Gamma \mid \{\PAYF^{(1)},\PAYF^{(2)}\}\subseteq A\,\},
&\quad \PAYF^\times  &:= \Gamma\setminus \PAYF^\surd,\\[2pt]
\OCC^{\surd} &:= \{\,A\in\Gamma \mid \OCC^{(2)}\in A \Rightarrow \OCC^{(1)}\in A\,\},
&\quad \OCC^{\times} &:= \Gamma\setminus \OCC^{\surd}.
\end{aligned}
\]
...
\,$\PAYF^{\times}$\ produces $\bott$. After $\topt$ (accepting frontier), all continuations
...
reaches its first tight match. Before that point, the monitor behaves exactly as the regular-expression monitor for
$re$ and emits only $\mathsf{?}$. Once the trigger fires, the monitor switches permanently to the contract monitor $\tsmc(C)$. If the pattern becomes impossible before it fires, the contract is
...
\begin{definition}[Triggered Monitor Construction]
    ...
    \begin{itemize}
      ...
      Let $q'=\delta_r(q,A)$, and denote $Q_r^{v} := \{\,q \in Q_r \mid \lambda_5^r(q) = v\,\}$.
      \[
      \delta_{\trigg}(q,A)=
      \begin{cases}
        c_0
    ...
    \paragraph{Construction mechanism.}
    ...
    \lambda_5^{\trigg}\bigl(\delta_{\trigg}(q_0^{\trigg},\emptytrace)\bigr)
      = \lambda_5^{\trigg}(r_0) = \mathsf{?} \quad \text{(since } r_0 \in Q_r^{\text{open}}\text{)}
    \]
...
We move now to guarded contracts monitor construction. Unlike conjunction, the guarded contract $\guard[re]{C}$ cannot be monitored via a naive synchronous product. While the tight frontier is determined by the instantaneous state of $re$ and $C$, the stable post-verdicts ($\topp, \botp$) require historical context: the monitor must remember if the decisive frontier has already been crossed. A simple pointwise combination of component outputs fails to capture this latching behavior. Consequently, our construction restricts the synchronous product to the active tight region and handles post-verdicts via explicit transitions to global sinks, ensuring the monitor stabilizes correctly immediately after the decisive step. 


    \begin{definition}[Guarded Monitor Construction]
      \label{def:moore-guard}
      Let the tight satisfaction monitors for the guard and the contract be:
      \[
      \tsmc(\re)=(Q_r, r_0, \Gamma, \tightverdicts, \delta_r, \lambda_5^r)
      \quad\text{and}\quad
      \tsmc(C)=(Q_c, c_0, \Gamma, \tightverdicts, \delta_c, \lambda_5^c).
      \]
      We construct the guarded monitor in two phases: first building a \emph{core} product restricted to the tight regions \(\{\mathsf{?},\topt,\bott\}\), and then extending it with single-step transitions to post-verdict sinks.
      
      \medskip
      \noindent\textbf{1. Core (3-valued) guarded product.}
      We define the sets of \emph{active states} (those that have not yet reached a stable post-verdict):
      \[
      Q_r^{\circ} := \{\,x\in Q_r \mid \lambda_5^r(x) \notin \{\topp, \botp\}\,\},
      \qquad
      Q_c^{\circ} := \{\,y\in Q_c \mid \lambda_5^c(y) \notin \{\topp, \botp\}\,\}.
      \]
      The core product is the restricted Moore machine
      \[
      M_{\guardd}^{\circ}:=
      \bigl(Q_r^{\circ}\times Q_c^{\circ},\ (r_0,c_0),\ \Gamma,\ \{\mathsf{?},\topt,\bott\},\ \delta_{\guardd}^{\circ},\ \lambda_{\guardd}^{\circ}\bigr),
      \]
      where the transition function is the standard synchronous product
      \[
      \delta_{\guardd}^{\circ}((x,y),A) := (\delta_r(x,A),\delta_c(y,A)).
      \]
      The core output function $\lambda_{\guardd}^{\circ}$ is defined by the following rules, using the component verdicts \((v_r,v_c)=(\lambda_5^r(x),\lambda_5^c(y))\):
      \[
      \lambda_{\guardd}^{\circ}(x,y)=
      \begin{cases}
      \bott & \text{if } v_r\in\{\mathsf{?},\topt\} \ \nd\ v_c=\bott,\\[4pt]
      \topt & \text{if } \bigl(v_r=\bott \ \nd\ v_c\in\{\mathsf{?},\topt\}\bigr) \\
            & \quad \sor\ \bigl(v_r\in\{\mathsf{?},\topt\} \ \nd\ v_c=\topt\bigr),\\[4pt]
      \mathsf{?} & \text{otherwise.}
      \end{cases}
      \]
      
      \medskip
      \noindent\textbf{2. Full monitor with post-verdicts.}
      We extend the core monitor with two fresh sink states \(q_{\topp}\) and \(q_{\botp}\) to obtain:
      \[
      \tsmc_{\guardd}(\tsmc_{re}(\re),\tsmc(C))
      := (Q_{\guardd},\ (r_0,c_0),\ \Gamma,\ \tightverdicts,\ \delta_{\guardd},\ \lambda_5^{\guardd}),
      \]
      where the state space is \(Q_{\guardd}:=(Q_r^{\circ}\times Q_c^{\circ})\ \cup\ \{q_{\topp},q_{\botp}\}\).
      
      The \textbf{output function} extends the core outputs:
      \[
      \lambda_5^{\guardd}(s)=
      \begin{cases}
      \lambda_{\guardd}^{\circ}(s) & \text{if } s\in Q_r^{\circ}\times Q_c^{\circ},\\[2pt]
      \topp & \text{if } s=q_{\topp},\\[2pt]
      \botp & \text{if } s=q_{\botp}.
      \end{cases}
      \]
      
      The \textbf{transition function} handles the redirection from tight frontiers to stable sinks.
      For any state $s \in Q_{\guardd}$ and input $A \in \Gamma$:
      \[
      \delta_{\guardd}(s,A)=
      \begin{cases}
      % Sinks Loop
      q_{\topp} & \text{if } s=q_{\topp},\\
      q_{\botp} & \text{if } s=q_{\botp},\\[4pt]
      
      % Redirection from Frontier to Sink
      q_{\topp} & \text{if }  \lambda_5^{\guardd}(s)=\topt,\\
      q_{\botp} & \text{if }  \lambda_5^{\guardd}(s)=\bott,\\[4pt]
      
      % Normal Core Evolution (from undecided state)
      \delta_{\guardd}^{\circ}(s,A) & \text{if }\lambda_5^{\guardd}(s)=\mathsf{?}.
      \end{cases}
      \]
      \end{definition}
 ...
 \paragraph{Construction mechanism.}
      ...
      The guarded monitor separates the detection of the decisive frontier from the maintenance of the stable verdict.

      

      \smallskip

      \noindent\emph{Phase~1: Tight-core product.}

      The core machine $M_{\guardd}^{\circ}$ restricts the synchronous product to the active regions of the components to identify the earliest decisive prefix.

      If the guard closes or the contract succeeds, the core transitions to a state labeled $\topt$.

   In the other case, if the contract is breached while the guard is still active, the resulting composition reaches a state labeled $\bott$.

      

      \smallskip

      \noindent\emph{Phase~2: Post-verdict sinks.}

      The full monitor extends the core with a transition logic that routes tight states to permanent sinks.

      If the current state is a tight verdict ($\topt$ or $\bott$), the following transition leaves the enforcement phase and enters the corresponding fresh global sink ($q_{\topp}$ or $q_{\botp}$).
      ...
      be the monitor constructed in Definition~\ref{def:moore-guard}.

      Then, for every finite trace $\pi$,
      \[
      \lambda_5^{\guardd}\bigl(\delta_{\guardd}((r_0,c_0),\pi)\bigr)
        \;=\;
      ...
      The proof is based  on the decomposition of the trace into a decisive prefix and a sink state for the suffix.

      

      \smallskip

      \emph{Tight-core correctness.}

      For any prefix $\pi$ that has not strictly passed a tight verdict of one of the original sub-automata, the monitor remains in the core set $Q_r^{\circ} \times Q_c^{\circ}$.

      In this region, $\delta_{\guardd}$ acts as a standard synchronous product.

      The enforcement phase part of the monitor outputs function $\lambda_{\guardd}^{\circ}$ directly implements the tight semantic clauses: it evaluates to $\bott$ upon an active violation, and to $\topt$ upon release (guard failure) or active satisfaction.

      Thus, as long as the semantics evaluate to $\{\mathsf{?}, \topt, \bott\}$, the monitor output matches the semantics.

      

      \smallskip

      \emph{Post-verdict correctness.}

      Once the monitor reaches a state $s$ with a tight verdict (e.g., $\lambda(s)=\topt$), the semantic evaluation of the trace $\pi$ is exactly $\topt$.

      For any extension $\pi \cdot A$, the semantics require the verdict to become $\topp$.

      The transition function satisfies this by definition: $\delta_{\guardd}(s, A) = q_{\topp}$.

      Since $q_{\topp}$ is a sink state and outputs $\topp$, the monitor correctly 

      The same reasoning applies to the violation case ($\bott \text{ to } \botp$).
...
Repetition contracts enforce the sequential satisfaction of a behavior over multiple iterations. The construction chains independent copies of the monitor for $C$, activating the next copy when the previous one reaches tight success. For finite repetition $C^n$, this produces $n$ linked monitors. For unbounded repetition $\repit{C}$, the construction forms an infinite cycle.
...
\[
Q_{\nrep}
  := \Bigl(\bigcup_{i=1}^{n-1} \bigl(Q^{(i)} \setminus Q^{(i)}_{\top}\bigr)\Bigr) \cup Q^{(n)},
\qquad
q_0^{\nrep} := q_0^{(1)},
\]
where $Q^{(i)}_{\top} = \{\, q^{(i)} \in Q^{(i)} \mid \lambda_5^{(i)}(q^{(i)}) \in \{\topt,\topp\}\,\}$.
...
\[
\delta_{\nrep}(q^{(n)},A) = \delta^{(n)}(q^{(n)},A).
\]

\medskip
\noindent\emph{Output function.}
The output function simply maps directly to the native outputs of the components:
\[
\lambda_5^{\nrep}(q^{(i)}) = \lambda_5^{(i)}(q^{(i)}) \quad \text{for all } i \in \{1,\dots,n\}.
\]
...
q_0
  & \text{if }\lambda_5(\delta(q,A))=\topt 
    \quad(\text{restart again after tight success}),\\[2pt]
\delta(q,A)
  & \text{otherwise}.
\end{cases}
\]
...
\medskip
\noindent\emph{Output function.}
The monitor never declares satisfaction, as the success states were removed. Its outputs correspond directly to the native outputs of the remaining states:
\[
\lambda_5^{\omega}(q) = \lambda_5(q).
\]

This produces the tight five-valued monitor for $\repit{C}$.
\end{definition}
...
\begin{subfigure}[t]{0.94\textwidth}
  ...
  \begin{tikzpicture}
    ...
    \node[state,fill=gray!10,right=3cm of q0]      (qw)  {$q_w$\\$\mathsf{?}$};
    \node[state,fill=red!18,right=2.5cm of qw]       (qv)  {$q_{\bott}$\\$\bott$};
    \node[state,fill=red!10,right= 2cm of qv]       (qpv) {$q_{\botp}$\\$\botp$};
  
    \path
      % In-cycle behavior:
      (q0) edge[loop above] node[above] {\footnotesize $\PAY^\surd$} ()
      (q0) edge[bend left=15] node[above] {\footnotesize $\PAY^\times$} (qw)
      (qw)  edge[bend left=15] node[below] {\footnotesize $\PAYF^\surd$} (q0) % restart on tight success
      (qw)  edge node[above] {\footnotesize $\PAYF^\times$} (qv)
      % Failure sink:
      (qv)  edge node[above] {$\Gamma$} (qpv)
      (qpv) edge[loop above] node {$\Gamma$} ();
  \end{tikzpicture}
  \caption{Monitor construction for $\repit{C_3}$.}
...
\node[state,fill=green!10,right=19mm of acc] (ap) {$s^{+}$\\[0.2pt]$\topp$};
...
$\PAY^\surd := \{\,A\in\Gamma \mid \{\PAY^{(1)},\PAY^{(2)}\}\subseteq A\,\}$ denotes a joint payment event,$\PAY^\times := \Gamma \setminus \PAY^\surd $ denotes failure of payment.
...
The unbounded repetition of \(C_3\) enforces an infinite sequence of payments with repair when payment first fails. The regular expression \(re\) represents the termination notice.

The guarded product synchronizes according to its definition: (1) when the guard remains open, all failures of \(\repit{C_3}\) propagate to the global verdict, and (2) when the guard activates, the open-ended obligation is discharged. The monitor then collapses to post acceptance, provided that no violation has occurred.

We now turn to the blame semantics and their equivalent Moore machine construction.