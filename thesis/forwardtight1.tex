...
\begin{itemize}
  ...
  \emph{Note:} The case $\topt \sqcap \topp = \topt$ reflects that if one component is exactly at its satisfaction frontier ($\topt$) while the other is already past it ($\topp$), the conjunction as a whole is determined by the latter of the two, effectively placing the global frontier at the current step.
...
The example above illustrates unsatifiability: a contract $C$ is unsatisfiable whenever every run of $\tsmc(C)$ reaches $\bott$ and then $\botp$, that is, whenever no state labeled $\topt$ or $\topp$ is reachable from $q_0$. Checking unsatisfiability thus reduces to a reachability query.

However, for the case $\perm[1]{a}\wedge\frb[2]{a}$, the conflict only raises whe agent 1 uses his right. Which is not easly spotable at both the semantics and the tight monitor construction as reaching a tight satifaction state is still possible. This case motivates a more refined classification of right and duty conflicts.
