...
Repetition contracts enforce the sequential satisfaction of a behavior over multiple iterations. The construction chains independent copies of the monitor for $C$, activating the next copy when the previous one reaches tight success. For finite repetition $C^n$, this yields $n$ linked monitors. For unbounded repetition $\repit{C}$, the construction forms an infinite cycle.
...
The unbounded repetition of \(C_3\) enforces an infinite sequence of payments with repair when payment first fails. The regular expression \(re\) captures the termination notice.
