...
The formula $\qsem{\trace{A}\concat \pi, C}$ treats the contract execution as a path-accumulation problem. At every time step, the function performs three operations:
...
The contract progress function handles Sequence and Reparation directly, ensuring that transitions with no delay are penalized properly. For example, if a contract requires $C$ then $C'$, and an event completes $C$ but violates $C'$ at the same time, the summation logic ($\qsem{\trace{A}, C} + \qsem{\trace{A}, C'}$) ensures the violation of $C'$ is still counted, even if it happens in a continuation.
...
We summarize these properties in the following theorem, which defines that the quantitative score is a monotonically increasing function that acts as a ``super-set'' of the binary violation semantics. While a binary trace might be ``Satisfied'' (via reparation), the quantitative score reveals the cumulative cost of that path.
...
\begin{cases}
  \qblame{\trace{A}, C},
    & \text{if } \pi = \emptytrace \text{ or } C_A=\emptc,\\[2pt]
  \qblame{\trace{A}, C} + \qblame{\pi, C_A},
    & \text{otherwise.}
\end{cases}
\]
where \(C_A := \Prog(\trace{A},C)\).
...
So far, we have introduced two quantitative views of non-compliance. The first one, $\qsem{\pi,C}$, returns a single number that counts violations along the trace, without assigning responsibility to the agents. The second one, $\qblame{\pi,C}$, keeps the same idea but splits the count into two parts, one per agent. Since both definitions use the same progress function and add up step-by-step contributions, it is natural to ask how the split version relates to the original violation score. In other words, does adding up the two blame counters always return the same score as $\qsem{\pi,C}$? The following lemma addresses this by proving that the overall quantitative violation score serves as a lower bound on the total assigned blame.
...
Importantly, the conjunction is \emph{additive} at a time point.
...
The development of \cDL collaborative interaction and responsibility attribution shares some core concepts with the logic of agency \logic{STIT} \cite{belnap2001facing,canavotto2022responsibility} and the contract specification language \logic{CSL} \cite{HvitvedKZ12}. Despite these common conceptual roots, \cDL differs from both formalisms in its concrete treatment of simultaneous agency, sequential reparation semantics, and quantitative blame assignment.
...
(ii) Reparation semantics: \cDL includes a reparation operator, while \logic{STIT} does not. In \logic{STIT}, reparation can only be represented by temporal disjunction, which is too permissive because it does not separate primary payment from fine payment. In \cDL, the secondary obligation $\obl[1]{\PAYF}$ only appears after a violation of $\obl[1]{\PAY}$. This setup ensures that a reparation contract is needed only if there is a failure, so situations like $\langle \PAYF, \PAYF \dots \rangle$, where fines are paid without cause, are avoided.
