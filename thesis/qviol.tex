...
The \cDL framework synthesizes these perspectives through a dual-semantic approach. For its \textit{tight forward semantics}, the language adheres to the classical construction advocated by Leucker and Schallhart: monitors are derived as correct-by-construction Moore machines where every transition is strictly induced by the contract's semantic definition. However, the \textit{quantitative semantics} necessitates a hybrid methodology. While the monitor retains the rigorous structural skeleton of a deterministic state machine, its state valuations integrate stream-based computations similar to SRV. This synthesis allows the monitor to track evolving quantitative data without sacrificing the semantic faithfulness of the underlying automaton.



In conclusion, \cDL occupies a distinct niche within this landscape. While it draws foundational inspiration from the agency axioms of \logic{STIT} and the trace-based specifications of \logic{CSL}, it departs from them to address the specific needs of runtime accountability. By prioritizing persistent monitoring over one-shot verification, and normative causality over counterfactual reasoning, \cDL bridges the gap between abstract deontic theory and the operational reality of interference-prone collaborative interactions.
