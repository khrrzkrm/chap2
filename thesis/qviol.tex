...
After establishing formal properties on the new defined quantitative blame semantics relative to other semantics, we move now to the quantitative blame monitor construction.
            ...
            Therefore, unfolding Definition~\ref{def:qbm-score} provides
            \[
            \qscore(\qbm(C),\pi)
...
The \cDL framework synthesizes these distinct perspectives via a dual-semantic approach. regarding \textit{tight forward semantics}, the language follows the classical correct-by-construction model advocated in~\cite{Bauer2011}: monitors arise as Moore machines where every transition derives strictly from the contract's semantic definition.
% TODO: Mention that the state space of this monitor, our reachable residual set of contracts, is closely related to the closure of an LTL formula, as introduced by Vardi and Wolper \cite{VardiWolper1986}.
The \textit{quantitative semantics}, conversely, requires a hybrid methodology. While the monitor preserves the rigid structural skeleton of a deterministic state machine, it enriches state valuations with stream-based computations characteristic of SRV. This design enables the tracking of evolving quantitative data while maintaining the semantic faithfulness of the underlying automaton.
