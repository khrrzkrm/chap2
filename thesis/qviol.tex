...
The governance of autonomous digital interactions has evolved from simple protocol compliance to complex, open-ended normative relationships. Whether in financial trading, autonomous supply chains, or service-level agreements, agents require mechanisms to verify behavior that is often collaborative, prone to interference, and long-running.

This section situates the proposed \textbf{Two-Agents Collaborative Normative Logic (\cDL)} within the broader landscape of formal methods, deontic logic, and multi-agent systems (MAS).\cite{bulling2015strategic,HvitvedKZ12} The distinguishing thesis is that existing frameworks either idealize away the \emph{bilateral} and \emph{interference-prone} character of contracts, or they do not turn their rich semantics into lightweight, trace-first monitoring artefacts for runtime accountability.\cite{broersen2011probabilistic,halpern2016causality,sanchez2018online}

To make this contrast precise, the discussion is organized along four analytical dimensions:
\begin{enumerate}
  \item \textbf{Temporal scope (the ``when''):} From infinite-trace verification to persistent, prefix-based monitoring.
  \item \textbf{Normative expressivity (the ``what''):} Handling contrary-to-duty (CTD) and reparation patterns without paradoxes.
  \item \textbf{Agency and collaboration (the ``who''):} Distinguishing attempts from outcomes and making interference a first-class trace object.
  \item \textbf{Quantitative evaluation (the ``how much''):} Moving from Boolean verdicts to cumulative, juridically motivated costs.
\end{enumerate}

\subsection{Temporal Scope: From Infinite Verification to Persistent Monitoring}
...
Runtime verification (RV) adapts temporal reasoning to finite domains, introducing multi-valued semantics for prefixes and monitor synthesis pipelines.\cite{leucker2011teaching,Bauer2011}, while the semantics are 3 valued ($\top,\bot,?$) for finite prefixes, \cDL forward has a 5 semantics by decomposing $\bot$ to $\botp$ and $\bott$, and $\top$ to $\topt$ and $\topp$, as the requirement of the logic requires determintic satisfaction and violation points. 

\paragraph{The main \cDL difference (persistence).}
Standard RV monitors are often \emph{abortive}: once a property is definitively violated or satisfied, the monitor halts or issues a terminal verdict.\cite{leucker2011teaching,Bauer2011} \cDL instead adopts a \emph{persistent}, contract-centred view in which the Contract Progress Function $\Prog$ evolves a normative state machine that reacts to violations (e.g., by activating reparations) and continues monitoring the residual contract, thus supporting long-horizon accountability rather than one-shot property checking.

\subsection{Contrary to duties Dynamic Contracts vs.\ Static Rules}
...
In parallel, the AI and legal-informatics literature explores more operational perspectives: defeasible deontic reasoning, dynamic deontic operators, and trace-based contract models.\cite{PrisacariuS12,rohaninezhad2017defeasible,HvitvedKZ12,wyner2006sequences}. These approaches bring contracts closer to executable structures but still typically emphasize inference over evolution of a single monitored contract instance. \cDL adopts a similar approach.



\subsection{Zero-sum vs Collaborative Interactions}
...
\subsection{Responsibility and Blame: Conterfactual Causality Vs Normative Causality}

Structural causal models (SCM) provide a principled account of causality and responsibility based on counterfactual dependence and minimality. In this setting, an agent is responsible for an outcome if, under a suitable intervention, changing the agent’s action would change the outcome of interest. This framework supports fine-grained notions of actual causation and graded responsibility, and has been extended to decentralized and partially observable multi-agent systems.\cite{halpern2014structural,halpern2016causality,triantafyllou2022causality}

The causal question addressed by SCM is inherently counterfactual. Responsibility is explained by asking what would have happened under alternative actions, for example whether a violation would still have occurred if the tenant had paid the rent in the second month. Such reasoning presupposes an explicit causal model of the environment and its dynamics, together with the ability to evaluate hypothetical interventions.

In contrast, \cDL adopts a normative notion of causality grounded in the rules of the contract itself. Responsibility is not derived from hypothetical alternatives, but from compliance with, or deviation from, the obligations explicitly stated in the contract. A violation is explained by identifying which contractual rule was breached on the observed trace, and blame is assigned to the party whose required contribution was missing, delayed, or blocked according to that rule.

This shift also applies to graded responsibility. While SCM derives degrees of responsibility from counterfactual minimality and causal contribution, \cDL defines degrees of responsibility internally, relative to the contract. Grading is computed from the contract structure and its progress semantics, for instance by counting or weighting violations along the progress-induced run. The resulting notion of responsibility is therefore contract-relative and trace-based: it answers who is responsible, and to what extent, according to the normative commitments the parties agreed upon, rather than according to an external causal model.



\subsection{Monitoring Methodology: between Correct-by-Construction and  Stream Based Runtime Verification}

Principled methodologies for compiling logical specifications into executable monitors have been a central focus of the runtime verification (RV) community. Foundational to this field is the automata-based approach of Leucker and Schallhart, where temporal specifications are systematically translated into monitors with verdicts that are provably aligned with the underlying logic \cite{leucker2011teaching}. Generalizing this paradigm to quantitative and data-rich settings is Stream Runtime Verification (SRV). Tools such as Lola, hLola, and TeSSLa support expressive stream transformations and aggregation, making them well suited for monitoring numerical properties over evolving data streams \cite{sanchez2018online,gorostiaga2018striver,gorostiaga2021hlola,tessla2015language}.

The \cDL framework synthesizes these perspectives through a dual-semantic approach. For its \textit{tight forward semantics}, the language adheres to the classical construction advocated by Leucker and Schallhart: monitors are derived as correct-by-construction Moore machines where every transition is strictly induced by the contract's semantic definition. And the formal semantics are of course studied and designed to have essential properties. However, the \textit{quantitative semantics} necessitates a hybrid methodology. While the monitor retains the rigorous structural skeleton of a deterministic state machine, its state valuations integrate stream-based computations similar to SRV. This synthesis allows the monitor to track evolving quantitative data without sacrificing the semantic faithfulness of the underlying automaton.


% \subsection{Quantitative Evaluation: From Boolean to Juridical Cost}

% To reason about resource consumption or robustness, quantitative extensions of temporal logics and weighted automata have introduced cost and reward structures into the verification process \cite{Droste2009,alvianotemporal}. Demonstrating that such measures can be added without incurring prohibitive complexity are works on cost-parameterized LTL and related calculi \cite{fionda2016ltl,cupcalculus2015costpltl}.

% These existing approaches are typically tailored to physical or computational resources, such as energy, bandwidth, or response times \cite{Droste2009,alvianotemporal}. Often missing from this landscape is an explicitly juridical or contractual interpretation of cost. In most normative settings, deontic logics retain a purely qualitative treatment of violation and compliance, leaving a gap that \cDL aims to fill with its quantitative accounting of liability \cite{carmo2001deontic,sep_deontic2018}.

% \subsection{Quantitative : Quantitative violation Semantics}

% Quantitative extensions of temporal logics and weighted automata introduce cost and reward structures into verification, enabling reasoning about resource consumption, performance, or robustness.\cite{Droste2009,alvianotemporal} Work on cost-parameterized LTL and related calculi shows how to add such measures without incurring prohibitive complexity.\cite{fionda2016ltl,cupcalculus2015costpltl}

% Most of these approaches, however, are tailored to physical or computational resources (e.g., energy, bandwidth, response times) and remain detached from explicitly juridical or contractual interpretations of cost.\cite{Droste2009,alvianotemporal} In normative settings, deontic logics often retain a purely qualitative treatment of violation and compliance.\cite{carmo2001deontic,sep_deontic2018}

% \paragraph{The \cDL difference (juridical accumulation).}
% \cDL defines a quantitative semantics $\qsem{\cdot}$ that aggregates \emph{normative} penalties over time, distinguishing repaired from unrepaired violations and assigning them different cost trajectories.\cite{HvitvedKZ12,carmo2001deontic} This accumulation is tightly coupled to the contract-progress semantics, enabling automated dispute analysis that goes beyond binary satisfaction and reflects legally motivated distinctions such as partial performance, remediation, and repeated minor breaches.\cite{rohaninezhad2017defeasible}

% \subsection{Logical Expressiveness: Encoding the Complexity of Contracts}

% Deontic traditions document the foundational difficulties of CTD obligations and related paradoxes, providing critical baselines for normative expressiveness.\cite{carmo2001deontic,sep_deontic2018,wyner2006sequences} Trace-based models for multiparty contracts demonstrate how contractual clauses can be compiled into trace properties amenable to algorithmic analysis while preserving domain structure.\cite{HvitvedKZ12}

% Strategic logics and their verification tools privilege feasibility and ability analysis over direct compliance monitoring, and their complexity profiles reflect these goals.\cite{bulling2015strategic,caltais2014mcmas} From the perspective of executable contracts, they serve as benchmarks rather than immediate monitoring solutions.\cite{bulling2015strategic,HvitvedKZ12}

% \begin{table}[t]
%   \centering
%   \caption{Logical expressiveness: deontic and strategic baselines vs.\ \cDL.}
%   \label{tab:expressiveness}
%   \begin{tabular}{p{3.2cm}p{3.1cm}p{3.1cm}p{3.1cm}}
%     \hline
%     \textbf{Dimension} & \textbf{\cDL} & \textbf{Deontic traditions} & \textbf{Strategic logics} \\
%     \hline
%     CTD and repair structure &
%     Native contract-level operators &
%     Paradoxes and CTD issues documented~\cite{carmo2001deontic,sep_deontic2018} &
%     Encodable but not primary focus \\
%     Periodicity and recurrence &
%     First-class contract patterns &
%     Requires temporal enrichment &
%     Implicit in strategies \\
%     Collaboration and interference &
%     Explicit accountability signals in traces &
%     Modal abstraction of obligations &
%     Interaction as game play, not blame \\
%     Primary use &
%     Online compliance monitoring &
%     Norm reasoning and paradox analysis~\cite{wyner2006sequences} &
%     Ability/feasibility verification~\cite{bulling2015strategic} \\
%     \hline
%   \end{tabular}
% \end{table}
...
\paragraph{Gap references.}
STIT-inspired analyses articulate the conceptual need to represent attempts and failures but are not designed as lightweight, trace-first monitoring frameworks for bilateral contracts.\cite{broersen2011probabilistic,broersen2011intentional,kuncova2025ability} Structural causal models provide principled counterfactual blame but presuppose rich causal models and offline reasoning that are difficult to maintain in an online monitoring architecture.\cite{halpern2014structural,halpern2016causality,triantafyllou2022causality}

\paragraph{Method references.}
Runtime verification and stream-based monitoring offer the compilation mindset and infrastructures that \cDL leverages: monitor synthesis from logical specifications, synchronous and asynchronous stream processing, and quantitative aggregation mechanisms.\cite{leucker2011teaching,sanchez2018online,gorostiaga2021hlola,tessla2015language} Trace-based models for multiparty contracts supply the trace-theoretic foundation for turning contract clauses into monitorable properties.\cite{HvitvedKZ12}

\paragraph{Benchmark references.}
Deontic logic surveys and CTD-focused work benchmark normative expressiveness and paradox management, against which \cDL’s operational handling of reparations can be evaluated.\cite{sep_deontic2018,carmo2001deontic,wyner2006sequences} Strategic reasoning surveys and model-checking tools provide the ability-centric alternative to monitoring-centric approaches and highlight the different complexity and semantic trade-offs.\cite{bulling2015strategic,caltais2014mcmas}

\subsection{Positioning \cDL}

Within this landscape, \cDL can be viewed as a synthesis that targets a niche not well covered by existing frameworks: \emph{monitorable accountability under interference for long-running bilateral contracts}.\cite{HvitvedKZ12,broersen2011probabilistic} Agency-centred logics motivate explicit notions of attempt and failure but do not natively provide trace-level, implementation-ready monitors; SCM-based causality offers rigorous blame definitions but is counterfactual and model-heavy; and RV/SRV, while operational, are typically agnostic about normative structure and bilateral blame.\cite{halpern2016causality,leucker2011teaching,sanchez2018online,gorostiaga2021hlola}

\cDL occupies the intersection of these lines of work: it borrows the contract structuring and trace orientation of trace-based models, the attention to CTD and reparation from deontic traditions, the agency sensitivity of STIT-style accounts, and the implementability ethos of RV, but reorients them around persistent, quantitative, and interference-aware monitoring for collaborative contracts.\cite{HvitvedKZ12,carmo2001deontic,broersen2011intentional,sanchez2018online}

\begin{table}[t]
  \centering
  \caption{Global framework comparison.}
  \label{tab:global-summary}
  \begin{tabular}{p{3.0cm}p{3.2cm}p{3.2cm}p{3.2cm}}
    \hline
    \textbf{Feature} & \textbf{\cDL} & \textbf{Deontic baselines} & \textbf{Strategic / verification frameworks} \\
    \hline
    Primary goal &
    Monitoring compliance and accountability &
    Norm reasoning and CTD analysis~\cite{sep_deontic2018} &
    Ability and feasibility checking~\cite{bulling2015strategic} \\
    Evidence model &
    Trace-first, observable attempts and blocks &
    Modal or rule-level abstractions &
    Game- or model-based structures \\
    Interference &
    Explicitly representable and attributable &
    Typically indirect or abstract &
    Encoded in interaction outcomes \\
    Implementability &
    RV-oriented compilation to monitors &
    Proof or inference oriented &
    Model checking oriented \\
    \hline
  \end{tabular}
\end{table}
