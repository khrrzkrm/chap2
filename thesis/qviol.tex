...
\begin{theorem}[Consistency of Quantitative and Tight Semantics]
  ...
  \begin{enumerate}
      ...
      The converse of (1) does not hold. In other words, a positive score is not equivalent to a violation.
      If $C = C_1 \repair C_2$ and $\pi \satt C$ through the reparation branch (i.e., some prefix tightly violates $C_1$ and the subsequent suffix satisfies $C_2$), then $\qsem{\pi, C} \ge 1$.
      In particular, there exist traces with $\qsem{\pi, C} > 0$ and $\pi \satt C$.
  \end{enumerate}
  \end{theorem}
  ...
  \item \textbf{Violation implies positive score.}
  Assume $\pi \violt C$.
  By the tight violation semantics, $\pi$ is the earliest prefix at which a decisive failure occurs: there exists a residual contract $C_k = \Prog(\pi_k, C)$ and a step $A_k$ such that $\trace{A_k} \violt C_k$.
  Since $C$ is $\bot$-free, this tight violation must stem from an active obligation, prohibition, or power.
  By the definition of the instantaneous scoring function, $\qsem{\trace{A_k}, C_k} \ge 1$.
  Since $\qsem{\pi, C}$ is the cumulative sum of non-negative instantaneous scores, we obtain $\qsem{\pi, C} \ge 1 > 0$.
  
  If $\pi \postviol C$, then by definition there exists a strict prefix $\pi' \prec \pi$ with $\pi' \violt C$.
  By the argument above, $\qsem{\pi', C} > 0$.
  By monotonicity (part~3), $\qsem{\pi, C} \ge \qsem{\pi', C} > 0$.
  
  \item \textbf{Zero score excludes violation.}
  This is the contrapositive of part~(1): the five verdicts $\{\presat, \satt, \postsat, \violt, \postviol\}$ partition all traces.
  If both $\violt$ and $\postviol$ are excluded by part~(1), the remaining possibilities are exactly $\presat$, $\satt$, or $\postsat$.
  
  \item \textbf{Score monotonicity.}
  By the recursive definition of $\qsem{}$:
  \[
    \qsem{\pi \concat \trace{A}, C}
    = \qsem{\pi, C} + \qsem{\trace{A},\, \Prog(\pi, C)}.
  \]
  Since $\qsem{\trace{A}, \Prog(\pi, C)} \ge 0$ for all $A$ and all residual contracts, the claim follows.
  
  \item \textbf{Reparation cost.}
  Consider $C = C_1 \repair C_2$.
  If $\pi \satt C$ through the reparation branch, then some prefix $\pi'$ tightly violates $C_1$.
  By the definition of the instantaneous scoring function for reparation, the step at which $C_1$ is tightly violated contributes $\qsem{\trace{A}, C_1 \repair C_2} = 1 + \qsem{\trace{A}, C_2}$, contributing at least~$1$.
  Since all subsequent instantaneous scores are non-negative, the cumulative score satisfies $\qsem{\pi, C} \ge 1$.
  Hence, $\pi \satt C$ and $\qsem{\pi, C} > 0$ can coexist: the score records the cost of the reparation path even though the contract is ultimately satisfied.
\end{enumerate}
  \end{proof}
...
Let $C$ be a contract and $\pi = \trace{A_1, \dots, A_n}$ be a trace.
...
Write $q_0 := C$ and, for each $1 \le i \le n$, define the unique run of $\qmc(C)$ on $\pi$ by
\[
  q_{i} := \delta(q_{i-1},A_i) = \Prog(\trace{A_i},q_{i-1}).
\]
By definition of the monitor output function, for every $i$ we have
\[
  \lambda(q_{i-1},A_i) = \qsem{\trace{A_i},q_{i-1}}.
\]
...
\[
  \qsem{\pi,C} = \sum_{i=1}^{n} \qsem{\trace{A_i},q_{i-1}}.
\]

\paragraph{Base case ($n=1$).}
If $\pi = \trace{A_1}$, then by Definition~(Quantitative Violation Semantics)
\(
  \qsem{\pi,C} = \qsem{\trace{A_1},C} = \qsem{\trace{A_1},q_0}.
\)
This is exactly the required sum.

\paragraph{Inductive step.}
Assume the claim holds for all traces of length $n$.
Let $\pi' = \trace{A_1,\dots,A_{n+1}}$ be a trace of length $n+1$.
Unfolding the recursive definition of $\qsem{}$ gives
\begin{align*}
  \qsem{\pi',C} &= \qsem{\trace{A_1},C} + \qsem{\trace{A_2,\dots,A_{n+1}},\Prog(\trace{A_1},C)}\\
  &= \qsem{\trace{A_1},q_0} + \qsem{\trace{A_2,\dots,A_{n+1}},q_1}.
\end{align*}
Applying the induction hypothesis to the suffix trace $\trace{A_1,\dots,A_n}$ with initial contract $q_1$ gives
\[
  \qsem{\trace{A_2,\dots,A_{n+1}},q_1} = \sum_{i=2}^{n+1} \qsem{\trace{A_i},q_{i-1}}.
\]
Therefore,
\[
  \qsem{\pi',C} = \qsem{\trace{A_1},q_0} + \sum_{i=2}^{n+1} \qsem{\trace{A_i},q_{i-1}}
  = \sum_{i=1}^{n+1} \qsem{\trace{A_i},q_{i-1}}.
\]
...
So far, we have introduced two quantitative views of non-compliance. The first one, $\qsem{\pi,C}$, gives a single number that counts the number of violations along the trace, without caring who caused them. The second one, $\qblame{\pi,C}$, keeps the same idea but splits the count into two parts, one per agent. Since both definitions use the same progress function and add up step-by-step contributions, it is natural to ask whether the split version still matches the original violation semantics total. In other words, does adding up the two blame counters always give back the same score as $\qsem{\pi,C}$? The following lemma answers this by proving that the overall quantitative violation score is exactly the sum of the two blame components, so quantitative blame is just a finer view of the same quantity, not a different measure.
    ...
    \begin{lemma}[The relation of the blame score to the violation score]
        ...
        \begin{proof}
            We prove the claim by induction on the length of the trace $\pi$ and structurally on the instantaneous evaluation.
            ...
            \]
            If $\trace{A}$ violates $\LH(C)$, then since $C$ is $\bot$-free, the violation must stem from a blamable duty.
            Therefore $\qsem{\trace{A},C}=1$ and the blame attribution $\qblame{\trace{A},C}$ produces either $(1,0)$ or $(0,1)$. In both cases, $n_1+n_2 = 1$, evaluating to $1 \le 1$.

            For the inductive structural step on conjunction $C = C_1 \wedge C_2$:
            By the inductive hypothesis, if $\qblame{\trace{A},C_1} = (x_1, y_1)$ and $\qblame{\trace{A},C_2} = (x_2, y_2)$, then $\qsem{\trace{A},C_1} \le x_1 + y_1$ and $\qsem{\trace{A},C_2} \le x_2 + y_2$.
            Thus, for the conjunction:
            \[
            \qblame{\trace{A},C_1 \wedge C_2} = (x_1+x_2, y_1+y_2).
            \]
            and the sum of its components is $(x_1+x_2) + (y_1+y_2) = (x_1+y_1) + (x_2+y_2)$.
            Since a double violation in a conjunction of literals may be counted as a single concurrent violation in the global quantitative score, but blame accumulates strictly for each agent, we have:
            \[
            \qsem{\trace{A},C_1 \wedge C_2} \le (x_1+y_1) + (x_2+y_2).
            \]
            Hence, the violation score is less than or equal to the total assigned blame.
