...
\item \textbf{Rejection:} Reading $b$ at the first position triggers an eager rejection because no word in $L'$ starts with $b$. The trace has effectively left the prefix closure of $L'$. By contrast, once the system commits to acceptance at the initial $a$, strict extensions can never become rejections; they remain classified as post-acceptance.
\end{itemize}
...
\begin{example}[Five-region decomposition and Trace Evolution for $L'$]
  ...
  \medskip
  \noindent
  \textbf{Trace A (Success):} The user inputs $a$, then continues with $b$ and $a$.
  \[
    \underbrace{\varepsilon}_{\mathsf{?}\,(\in\Pre{L'})}
    \xrightarrow{a}
    \underbrace{a}_{\topt\,(\in\EAL{L'})}
  ...
  Crucially, this decomposition is \emph{identical} to the previous example where $L=\{a\}$. Because the tight semantics halts at the first definitive verdict, the acceptance of the prefix $a$ effectively masks the longer requirement $aba$, rendering the two languages indistinguishable from a monitoring perspective.
\end{example}
...
Starting from $s_0$, inputs $a$ or $bb$ yield $\topt$, input $ba$ produces $\bott$,
...
By application of the the standard subset construction with $\varepsilon$-closures, we obtain a DFA $\aut_D(re)=(Q,\Gamma,\delta,q_0,F)$ that recognizes the same language~\cite{RabinScott1959,HopcroftUllman2001}.x
...
We now close the pipeline by demonstrating the tight monitor construction. The construction below wraps the language DFA for $\Lang{\re}$ into the five-region Moore machine, yielding a monitor whose output coincides with the five-valued prefix semantics on all prefixes.
