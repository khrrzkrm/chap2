...
% \end{itemize}
% This produces determinacy (exactly one verdict per prefix), uniqueness of frontiers,
...
% However, for behavioral monitoring, the prefix $a$ already defines success; the second $a$ is merely an irrelevant extension, not a failure.
...
% Applying the minimal frontier operator produces two sets of candidates:
% \begin{itemize}
...
% A formal definition of monitoring behavior requires resolving semantic overlaps between these frontiers.
...
%   This produces the canonical five-region partition of $\Sigma^*$ by $\Sigma+$ saturation:
%  \[
% \begin{array}{rcll}
...
%       \item \textbf{Determinism:} Since the partition in Theorem \ref{thm:five-way} is disjoint and complete, the semantics produces exactly one verdict for any input trace.
%   \end{enumerate}
% \end{theorem}
...
In \cdl, contracts allocate responsibilities across two agents over time, making timing essential to their interpretation. Each duty and power is to be enforced at a specific time and under a specific condition. Semantically, this requires partitioning a trace into a contract prefix and a clause suffix for each clause composing the contract. To ensure correct evaluation, the partitioning must be precise. A required property is to have a unique earliest violation point or satisfaction point to decompose observed behavior. Without such unique boundaries in the language's semantics, reparations, for example, may be triggered prematurely, too late, or enforced multiple times. To address this ambiguity, \emph{tight forward} semantics are introduced:
\begin{itemize}
    ...
    \item \textbf{Tight:} The semantics anchor on the \emph{first} decisive instant. This isolates a unique earliest satisfaction or violation frontier. By definition, all strict extensions of this frontier are classified as ``post-frontier,'' preventing the same responsibility from triggering multiple times.
\end{itemize}

\paragraph{The necessity of tightness.}
Without a mechanism to ``lock in'' a verdict at the earliest instance, significant semantic ambiguities arise. If we simply tag a prefix as ``accepted'' whenever it belongs to the target language $L$, we lose the distinction between the \emph{moment} credit is earned and subsequent irrelevant behavior.
We illustrate the notion of tightness on  regular languages. Regular languages lie at the bottom of the Chomsky hierarchy \cite{chomskySchutzenberger1963}, which can be equivalently characterized by regular expressions, deterministic and nondeterministic finite automata, and monadic second-order logic over words. This equivalence is well understood and forms the backbone of automata-theoretic reasoning \cite{Thomas1997}.
\paragraph{Trivial example} Consider the regular language $L=\{a\}$ over $\Sigma=\{a,b\}$ interpreted as ``seeing $a$ once is success''.
\begin{itemize}
    \item \textbf{Forward:} the semantics are evaluated from the earliest event to the next over prefixes and suffices, respecting the chronological order of the contract clauses.
    \item \textbf{Tight:} The semantics fix the \emph{first} decisive instant. All strict extensions of a tight verdict point are considered as a post-verdict.
\end{itemize}

\paragraph{Illustrating prefix ambiguity.} The distinction becomes critical for languages containing prefixes of other member words, such as $L'=\{a, aba\}$ over $\Sigma=\{a,b\}$. In standard language theory, the entire word $aba$ is a valid member of $L'$. However, under a tight behavioral interpretation, success is achieved the moment the initial $a$ is observed. Consequently, the suffix $ba$ is treated as irrelevant post-acceptance behavior.
\begin{itemize}
\item \textbf{Acceptance:} The satisfaction frontier is reached after reading the first symbol $a$. Although $aba \in L'$, recognizing this later membership as a new success would imply that a single execution contains multiple distinct ``acceptance points''.
\item \textbf{Rejection:} Reading $b$ at the first position triggers an eager rejection because no word in $L'$ starts with $b$. The trace has irrevocably exited the prefix closure of $L'$. By contrast, once the system commits to acceptance at the initial $a$, strict extensions can never become rejections; they remain classified as post-acceptance.
\end{itemize}
...
\label{sec:lang-frontiers-tight}
To generalize this construction to any regular language $L \subseteq \Sigma^*$, we must distinguish between classical \emph{static language membership} and \emph{behavioral language evaluation}. While standard language theory views in a static way weither a word $w$ as either in the language $L$ or not, monitoring asks if the desired behavior has \emph{already} been achieved, necessitating the evaluation through all its prefixes.
Our objective is to partition the set of all possible words $\Sigma^*$ into regions that isolate these boundaries of decision. This requires explicitly resolving conflicts between ``good/bad words'' and ``extensions of good /bad words'' in favor of the latter principle, which we term the \emph{eagerness of acceptance and rejection}.
...
\begin{definition}[Closures and Frontiers]

  \label{def:closure-bad}

  For a regular language $L \subseteq \Sigma^*$, we define the following characteristic sets:

  \begin{enumerate}

      \item \textbf{Prefix Closure} ($\closureclass{L}$): The set of all prefixes of words in $L$.
      \[
      \closureclass{L} := \{ u \in \Sigma^* \mid \exists v \in L : u \preceq v \}.
      \]
      \item \textbf{Bad Prefixes} ($\badclass{L}$): The set of prefixes that cannot be extended to any word in $L$, defined as the complement of the prefix closure.
      \[
      \badclass{L} := \Sigma^* \setminus \closureclass{L}.
      \]
      \item \textbf{Minimal Language} ($\minlang{L}$): The set of the shortest words in $L$, containing those elements of the language that do not  have a proper prefix in $L$.
      \[
      \minlang{L} := \{ u \in L \mid \nexists v \in L : v \prec u \}.
      \]
...
\paragraph{Dynamic shift} Using the minimal language notion, we capture exactly the words for eager acceptance. For eager rejection on the other hand, it must not be confused with the minimal bad prefixes: a word can be a minimal bad prefix while extending a minimal accepted word (e.g., $aa$ for $L=\{a\}$), meaning that it has an eager acceptance prefix. To correctly encode this notion of eagerness, we define the canonical partition such that once a prefix is eagerly accepted or rejected, all its extensions are permanently classified as irrelevant satisfaction/violation, even if they technically deviate from $L$.
...
\begin{enumerate}
  ...
  \item The closure of the minimal language coincides with exactly the Pre-verdict and eager acceptance languages: $\closureclass{\minlang{L}} = \Pre{L} \dotcup \EAL{L}$.

  \item The closure splits exactly into minimal closure and irrelevant acceptance languages: $\closureclass{L} = \closureclass{\minlang{L}} \dotcup \IAL{L}$.
...
(1) follows because $\minlang{L} \subseteq \closureclass{L}$ and $\minlang{\badclass{L}} \subseteq \badclass{L}$. As $\closureclass{L}$ and $\badclass{L}$ are complements, $\minlang{L}$ and $\minlang{\badclass{L}}$ are disjoint. 

(2) follows by definition.

For (3), considering $u \in \closureclass{L}$. There exists $z \in L$ such that $u \preceq z$. Let $m$ be the shortest accepted prefix of $z$ ($m \in \minlang{L}$). Either $u \preceq m$ then $u \in \closureclass{\minlang{L}}$) or $m \prec u$  and then $u \in m\Sigma^+ \subseteq \IAL{L}$).

For (4), it follows from the definition of the bad prefix language and as claim (3) of this Lemma  covers the 3 others languages from the 5 partions leaving the rejection languages. Since the irrelevant rejection is the nonempty extension of the Eager rejection, they are disjoint.
...
\begin{proof}
The first claim holds because the closure is included in the language of $L$, which, by definition, has an empty intersection with its complement. 

For the second claim, $\IRL{L}\cap \IAL{L}=\emptyset$, assume $u \in \IRL{L} \cap \IAL{L}$. Then $u$ extends both a minimal bad prefix $b \in \ERL{L}$ and a minimal accepted word $m \in \EAL{L}$. If $m \preceq b$, then $b$ extends an accepted word, so $b \in \IAL{L}$, contradicting $b \in \ERL{L}$. If $b \preceq m$, then $m$ extends a bad prefix, implying $m \in \badclass{L}$, contradicting $m \in L$.

The last claim $\minlang{L} \cap \IRL{L} = \emptyset$ holds as $\minlang{L} \subseteq \closureclass{L}$ while $\IRL{L} \subseteq \badclass{L}$.
...
\begin{proof}
By Lemma~\ref{lem:basic-splits},  we have $\Sigma^* = \closureclass{L} \dotcup \badclass{L}$. Substituting the decompositions for $\closureclass{L}$ and $\badclass{L}$ using Lemma~\ref{lem:cross} proves the result.
\end{proof}

\subsubsection{Tight Five-Valued Semantics}
We move now to define the prefix-level semantics $\semfive{u \vDash L}$ using the 5 tight verdict values $\tightverdicts=\{\mathsf{?},\topt,\bott,\topp,\botp\}$. These correspond to pre-verdict, eager acceptance, eager rejection, irrelevant acceptance, and irrelevant rejection, respectively.
\begin{definition}[Five-valued prefix semantics]\label{def:five-valued-semantics}

For any $L\subseteq\Sigma^*$. For any $u\in\Sigma^*$:
\[
\semfive{u \vDash L}
\;:=\;
\begin{cases}
...
The following theorem summarizes all the desired identified properties for precise contract monitoring:
...
\begin{theorem}[Prefix Monotonicity and Determinacy]\label{prop:tightness-obligations-unindexed}
  ...
  \begin{enumerate}
      ...
      \item \textbf{Determinism:} $\semfive{u \vDash L}$ admits exactly one verdict.

      \item \textbf{Monotone evolution:}

      If $\semfive{u \vDash L} \in \{\topt, \topp\}$, then for all $x \in \Sigma^+$, $\semfive{u\concat x \vDash L} = \topp$.

      On the other hand, if $\semfive{u \vDash L} \in \{\bott, \botp\}$, then for all $x \in \Sigma^+$, $\semfive{u\concat x \vDash L} = \botp$.
...
\begin{proof}
\textbf{Monotonicity:} If $\semfive{u \vDash L}=\topt$, then $u \in \EAL{L}$. Any non empty $x$ in $u \concat x$ is a menber of  $\EAL{L}\,\Sigma^+ = \IAL{L}$, so the verdict is $\topp$. If $u \in \IAL{L}$, it is already an extension of a word in $\EAL{L}$, so  further extensions stay in $\IAL{L}$. The rejection case follows identical logic using $\ERL{L}$ and $\IRL{L}$.
\textbf{Unique Frontier:} If $u \in \IAL{L}$, then $u = m \concat x$ for some $m \in \EAL{L}$. Since $\EAL{L}$ contains \emph{minimal} accepted words, no strict prefix of $m$ is in $L$, and no strict extension of $m$  can be in $\EAL{L}$. Thus, $m$ is the unique frontier point.
\end{proof}
...
\begin{figure}[h]
  ...
  \end{tikzpicture}
  \caption{Visualizing the five-way semantic partition (Theorem \ref{prop:tightness-obligations-unindexed}). The diagram illustrates the irreversible flow of a trace from the undecided state (Yellow) to a decisive frontier (Eager Acceptance in Green or Eager Rejection in Red) and subsequently to an irrelevant post-verdict state. Arrows indicate possible transitions via trace extension. Dashed arrows stand for the possible existence of a suffix. Plain arrows stand for any non-empty suffix leads to the next verdict. Missing arrows imply that no direct transition is possible.}
  \label{fig:five-way-partition}
\end{figure}
...
\begin{example}[Five-region decomposition and Trace Evolution]
  ...
  Notably, $aa$ is a bad prefix in the static sense (it is not in $L$), but it extends the accepted word $a$. To preserve tight determinacy, the acceptance of $a$ takes priority. We formally compute the partition sets:
  \begin{itemize}
  ...
  \medskip
  \noindent
  \textbf{Trace A (Success):} First inputs $a$, then continues with $b$.
  \[
  ...
  \medskip
  \noindent
  \textbf{Trace B (Failure):} First inputs $b$, then continues with $a$.
  \[
    \underbrace{\varepsilon}_{\mathsf{?}\,(\in\Pre{L})}
    \xrightarrow{b}
...
\begin{example}[Five-region decomposition and Trace Evolution for $L'$]
  Let $\Sigma=\{a,b\}$ and let $L'=\{a, aba\}$. The goal is to detect the occurrence of the accepted words using the tight semantics.
  
  \paragraph{1. Static Partitioning.}
  First, we identify the topological frontiers.
  The set of prefixes is $\closureclass{L'} = \{\varepsilon, a, ab, aba\}$.
  However, the tight semantics halts at the \emph{first} definitive verdict.
  Since $a \in L'$, the monitor stops there, rendering the longer word $aba$ unreachable in the eager partition.
  We formally compute the partition sets:
  \begin{itemize}
      \item $\EAL{L'} = \{a\}$. This is the minimal accepted word.
      \item $\ERL{L'} = \{b\}$. This is the minimal rejected prefix (since $b \notin \closureclass{L'}$).
      \item $\IAL{L'} = \{aa, ab, aba, \dots\}$. These are extensions of the eager success $a$. Note that even though $aba \in L'$, it falls into the irrelevant region because the verdict was already decided at $a$.
  \end{itemize}
  ...
  \medskip
  \noindent
  \textbf{Trace A (Success):}  First input $a$, then continue with $b$ and $a$.
  \[
    \underbrace{\varepsilon}_{\mathsf{?}\,(\in\Pre{L'})}
    \xrightarrow{a}
    \underbrace{a}_{\topt\,(\in\EAL{L'})}
    ...
    \underbrace{ab}_{\topp\,(\in\IAL{L'})}
    \xrightarrow{a}
    \underbrace{aba}_{\topp}
  \]
  \emph{Observation:} At $u=a$, the verdict is locked to $\topt$. The subsequent extension $aba$, despite being in $L'$, is treated as a stable extension ($\topp$) rather than a new success, because the monitor has already halted.

  \medskip
  \noindent
  \textbf{Trace B (Failure):} first input $b$, then continue with $a$.
  \[
    \underbrace{\varepsilon}_{\mathsf{?}\,(\in\Pre{L'})}
    \xrightarrow{b}
    \underbrace{b}_{\bott\,(\in\ERL{L'})}
    ...
    \botp
  \]
  \emph{Observation:} At $u=b$, the verdict is locked to $\bott$.

  \textbf{Comparison with $L=\{a\}$.}
  This decomposition is \emph{identical} to the previous example where $L=\{a\}$. Because the tight semantics halts at the first definitive verdict, the acceptance of the prefix $a$ masks the longer requirement $aba$, rendering the two languages indistinguishable from a monitoring perspective.
\end{example}
...
To implement the tight semantics operationally, we now lift the standard deterministic language automaton into a tight monitor. While a standard automaton recognizes an accepted word, a tight monitor must instead classify every individual prefix into one of the five semantic regions. We achieve this by preserving the underlying transition structure of the original automaton while augmenting its output behavior: every state is assigned a tight verdict corresponding to the five-valued prefix semantics. The result is the tight monitor formalized below.



We first define the auxiliary components required for this transformation, then combine them into the final Moore machine.

\begin{definition}[Five-region automata]\label{def:five-aut}
Let $L\subseteq\Sigma^*$ be a regular language and let $\aut(L)$ be a DFA recognizing the language $L$:
\[
  \aut(L)=(Q,\Sigma,\delta,q_0,F)
  \quad\text{with}\quad \Lang{\aut(L)}=L.
\]
 We define the following derived DFAs, all over the same alphabet $\Sigma$:
\begin{itemize}
  \item $\aut_{\mathrm{EA}}(L)  := (Q_{\mathrm{EA}},\Sigma,\delta_{\mathrm{EA}},q_{0,\mathrm{EA}},F_{\mathrm{EA}})$
        with $\Lang{\aut_{\mathrm{EA}}(L)}=\EAL{L}$.
  \item $\aut_{\mathrm{PRE}}(L) := (Q_{\mathrm{PRE}},\Sigma,\delta_{\mathrm{PRE}},q_{0,\mathrm{PRE}},F_{\mathrm{PRE}})$
        with $\Lang{\aut_{\mathrm{PRE}}(L)}=\Pre{L}$.
  \item $\aut_{\mathrm{IAL}}(L) := (Q_{\mathrm{IAL}},\Sigma,\delta_{\mathrm{IAL}},q_{0,\mathrm{IAL}},F_{\mathrm{IAL}})$
        with $\Lang{\aut_{\mathrm{IAL}}(L)}=\IAL{L}$.
  \item $\aut_{\mathrm{ER}}(L)  := (Q_{\mathrm{ER}},\Sigma,\delta_{\mathrm{ER}},q_{0,\mathrm{ER}},F_{\mathrm{ER}})$
        with $\Lang{\aut_{\mathrm{ER}}(L)}=\ERL{L}$.
  \item $\aut_{\mathrm{IRL}}(L) := (Q_{\mathrm{IRL}},\Sigma,\delta_{\mathrm{IRL}},q_{0,\mathrm{IRL}},F_{\mathrm{IRL}})$
        with $\Lang{\aut_{\mathrm{IRL}}(L)}=\IRL{L}$.
\end{itemize}
\end{definition}

We build these automata using standard operations such as prefix-closure, complementation, and breadth-first identification of minimal prefixes \cite{HopcroftUllman2001}. These steps follow the language partitions described in Definition~\ref{def:partition-sets}.
...
\[
  ...
  \;:=\;
  \big(S,\ s_0,\ \Sigma,\ \tightverdicts,\ \delta,\ \lambda\big),
\]
...
\[
s_0 := \big(q_{0,\mathrm{PRE}},\,q_{0,\mathrm{EA}},\,q_{0,\mathrm{ER}},\,
             q_{0,\mathrm{IAL}},\,q_{0,\mathrm{IRL}}\big),
\]
the transition function operates component-wise
\[
\delta\!\big((q_1,q_2,q_3,q_4,q_5),\,\sigma\big)
:= \big(\delta_{\mathrm{PRE}}(q_1,\sigma),\ \delta_{\mathrm{EA}}(q_2,\sigma),\
          \delta_{\mathrm{ER}}(q_3,\sigma),\
          \delta_{\mathrm{IAL}}(q_4,\sigma),\
          \delta_{\mathrm{IRL}}(q_5,\sigma)\big),
\]
and the output function $\lambda$ assigns verdicts based on component acceptance
\[
\lambda(q_1,q_2,q_3,q_4,q_5)
:= \begin{cases}
      \topt & \text{if } q_2\in F_{\mathrm{EA}},\\
      \bott & \text{if } q_3\in F_{\mathrm{ER}},\\
      \topp & \text{if } q_4\in F_{\mathrm{IAL}},\\
      \botp & \text{if } q_5\in F_{\mathrm{IRL}},\\
      \mathsf{?} & \text{if } q_1\in F_{\mathrm{PRE}}.
    \end{cases}
\]
\end{definition}
...
Starting from $s_0$, inputs $a$ or $bb$ produce $\topt$, input $ba$ produces $\bott$,
...
Let $L\subseteq\Sigma^*$ be regular and let $\mathcal{M}_{\text{5tight}}(L)$ be the machine from Definition~\ref{def:five-moore}. For every input prefix $u\in\Sigma^*$, the machine output coincides with the semantic definition:
\[
\semfive{u \vDash L}\;=\;\lambda\!\big(\delta^{*}(s_0,u)\big).
\]
\end{lemma}
...
\[
\delta^{*}(s_0,u)
=\big(\delta_{\mathrm{PRE}}^{*}(q_{0,\mathrm{PRE}},u),\dots,\delta_{\mathrm{IRL}}^{*}(q_{0,\mathrm{IRL}},u)\big).
\]
Each component DFA recognizes one of the five semantic regions. For example, a prefix $u$ belongs to the eager acceptance region $\EAL{L}$ if and only if the eager acceptance DFA reaches an accepting state after processing $u$. The transition logic makes sure that every input is checked by all five DFAs. Theorem~\ref{thm:five-way} shows that these regions do not overlap and together cover all possibilities, so only one DFA will accept any input. The output function $\lambda$ then uses this accepting DFA to assign the correct verdict, as described in Definition~\ref{def:five-valued-semantics}.
\end{proof}
...
\begin{proof}
  ...
  We can prove this bound by looking at how the Moore machine is built. The first step of the construction consists of constructing 5 region automata from the original DFA $\aut(L)$ by changing only its accepting sets. Consequently, the remaining parts of the structure $(Q, \Sigma, \delta, q_0)$ are kept exactly the same across all five components.

  \[
    \delta_{\mathrm{PRE}}^*(q_{0,\mathrm{PRE}}, u) = \dots = \delta_{\mathrm{IRL}}^*(q_{0,\mathrm{IRL}}, u) = \delta^*(q_0, u).
  \]

For the second step, the Moore machine's state space is technically the Cartesian product :
 \[
    S_{\mathrm{reach}} = \{ (q, q, q, q, q) \mid q \in \mathrm{Reach}(\aut(L)) \}.
  \]
 In fact, any quintuple in the reachable states will be formed by the same state, since the transition functions and initial states are identical across all five automata. Thus,  any input prefix $u \in \Sigma^*$ will always drive every component to the exact same state. Therefore, the number of reachable states in the Moore constructions is linear to the number of states in the original DFA, $|Q|$.

\end{proof}
\paragraph*{Summary}
We have demonstrated using a \emph{correct-by-construction} that any regular language $L\subseteq\Sigma^*$ can be decomposed into the five semantic regions required for tight monitoring instead of just language menbership.
This transformation operates on a DFA using standard automata-theoretic operations, such as completion, complement, and breadth-first search for minimal frontiers, and is proved correct by showing that it maps the 5 languages up to the Moore machine recognizing them.

\subsection{Illustration Through Regular Expressions from \cDL}
\label{subsec:re-tight}
Having defined the general construction, we now illustrate it on \cDL regular expressions. The procedure begins by specifying the language semantics of the regular expression, then transforms it into a deterministic automaton, and finally lifts it to the five-valued monitor.
...
\begin{itemize}
  \item \textbf{Atom $\mathsf{A}$.} Matches exactly one period: $\pi=\langle A_1\rangle$ with $\mathsf{A}\subseteq A_1$.
  \item \textbf{Wildcard $\Gamma$.} Matches any single period: $\pi=\langle A_1\rangle$ for arbitrary $A_1\in\Gamma$.
...
that \emph{covers} $\mathsf{A}$, as in Definition~\ref{def:re-semantics}.
...
By application of the standard subset construction with $\varepsilon$-closures, we obtain a DFA $\aut_D(re)=(Q,\Gamma,\delta,q_0,F)$ that recognizes the same language~\cite{RabinScott1959,HopcroftUllman2001}.

\paragraph*{Stage 3: Completion, trimming, and minimization.}
For monitoring, we assume a \emph{complete} DFA: add (if needed) a sink state so that $\delta$ is total on $Q\times\Gamma$.
Next, we trim unreachable states to retain only $\Reach(\aut_D(re))$.
...
accepting state $s_7$. Determinization and completion of this NFA produces a DFA
...
We now close the pipeline by demonstrating the tight monitor construction. The construction below wraps the language DFA for $\Lang{\re}$ into the five-region Moore machine, producing a monitor whose output coincides with the five-valued prefix semantics on all prefixes.
...
% From $s_0$, input $a$ or $bb$ produces~$\topt$, input $ba$ produces~$\bott$,
...
% Practically, the approach is modular: each region is an ordinary DFA; adaptable: construct only the reachable part of products and minimize components; and reusable across
...
% The construction below performs exactly this lift: it wraps $\Lang{\re}$ into the five-region Moore machine and thereby produces a tight satisfaction monitor whose output coincides with the five-valued prefix semantics on all prefixes.
