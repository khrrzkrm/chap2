...
\begin{definition}[Closures and Frontiers]
  ...
  \begin{enumerate}
      ...
      \item \textbf{Minimal Language} ($\minlang{L}$): The set of the shortest words in $L$, containing those elements of $L$ that have no proper prefix in $L$.
      \[
...
\paragraph*{Summary}
This section introduced a \emph{correct-by-construction} framework that transforms any regular language $L\subseteq\Sigma^*$ into the five semantic regions required for tight monitoring, ultimately yielding a single executable monitor. This workflow operates on the underlying DFA graph using exclusively standard automata-theoretic operations, such as completion, complement, breadth-first search for minimal frontiers, and right-ideal saturation.
...
Apply the standard subset construction with $\varepsilon$-closures to obtain a DFA $\aut_D(re)=(Q,\Gamma,\delta,q_0,F)$ that recognizes the same language~\cite{RabinScott1959,HopcroftUllman2001}.x
