...
Without a mechanism to ``lock in'' a verdict at the earliest instance, semantic ambiguities arise. If we simply tag a prefix as ``accepted'' whenever it belongs to the target language $L$, we lose the distinction between the \emph{moment} credit is earned and subsequent irrelevant behavior.
...
For (4), it follows from the definition of the bad prefix language and as claim (3) of this Lemma covers the 3 other languages from the 5 partitions, leaving the rejection languages. Since the irrelevant rejection is the nonempty extension of the Eager rejection, they are disjoint.
...
\begin{proof}
\textbf{Monotonicity:} If $\semfive{u \vDash L}=\topt$, then $u \in \EAL{L}$. Any non empty $x$ in $u \concat x$ is a member of  $\EAL{L}\,\Sigma^+ = \IAL{L}$, so the verdict is $\topp$. If $u \in \IAL{L}$, it is already an extension of a word in $\EAL{L}$, so  further extensions stay in $\IAL{L}$. The rejection case follows identical logic using $\ERL{L}$ and $\IRL{L}$.
...
\end{proof}
\paragraph*{Summary}
We have demonstrated using a \emph{correct-by-construction} approach that any regular language $L\subseteq\Sigma^*$ can be decomposed into the five semantic regions required for tight monitoring instead of just language membership.
