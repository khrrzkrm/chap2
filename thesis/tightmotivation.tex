...
\begin{proof}
The first claim is immediate from the definition $\badclass{L}:=\Sigma^*\setminus\closureclass{L}$.

For the second claim, assume towards a contradiction that there is $u\in\IRL{L}\cap\IAL{L}$.
By definition of $\IRL{L}$ there exist $b\in\ERL{L}$ and a nonempty word $x\in\Sigma^{+}$ such that $u=bx$.
By definition of $\IAL{L}$ there exist $m\in\EAL{L}=\minlang{L}$ and a nonempty word $y\in\Sigma^{+}$ such that $u=my$.
Since both $b$ and $m$ are prefixes of $u$, they are comparable.
If $m\preceq b$, then $b=mz$ for some $z\in\Sigma^{+}$, hence $b\in\EAL{L}\,\Sigma^{+}=\IAL{L}$, contradicting $b\in\ERL{L}=\minlang{\badclass{L}}\setminus\IAL{L}$.
If $b\preceq m$, then $b$ is a prefix of the accepted word $m\in L$, so $b\in\closureclass{L}$, contradicting $b\in\ERL{L}\subseteq \minlang{\badclass{L}}\subseteq \badclass{L}=\Sigma^*\setminus\closureclass{L}$.
Thus $\IRL{L}\cap\IAL{L}=\emptyset$.

For the third claim, note that $\IRL{L}\subseteq\badclass{L}$ (by Lemma~\ref{lem:two-splits}) and $\minlang{L}\subseteq\closureclass{L}$ (by Lemma~\ref{lem:basic}(1)).
Hence
$\minlang{L}\cap\IRL{L}\subseteq \closureclass{L}\cap\badclass{L}=\emptyset$.
\end{proof}
...
\begin{theorem}[Prefix Monotonicity and Determinacy]\label{prop:tightness-obligations-unindexed}
  ...
  \begin{enumerate}
      \item \textbf{Stability of Verdicts:} If $\semfive{u \vDash L} \in \{\topt, \topp\}$, then for all extensions $v \succ u$, $\semfive{v \vDash L} = \topp$. Analogously for rejection.
      \item \textbf{Unique Frontier:} If $\semfive{u \vDash L} = \topp$, there exists a unique strict prefix $u' \prec u$ such that $\semfive{u' \vDash L} = \topt$.
...
\smallskip
\noindent\textbf{(1) Stability of verdicts.}
Assume $\semfive{u \vDash L}\in\{\topt,\topp\}$.

If $\semfive{u \vDash L}=\topt$, then $u\in\EAL{L}=\minlang{L}$. For any strict extension $v\succ u$ we can write $v=ux$ for some nonempty $x\in\Sigma^{+}$, hence $v\in \EAL{L}\,\Sigma^{+}=\IAL{L}$ and therefore $\semfive{v \vDash L}=\topp$.

If $\semfive{u \vDash L}=\topp$, then $u\in\IAL{L}=\EAL{L}\,\Sigma^{+}$. Any strict extension $v\succ u$ satisfies $v=uy$ for some $y\in\Sigma^{+}$, so $v\in\IAL{L}\,\Sigma^{+}=\IAL{L}$, and again $\semfive{v \vDash L}=\topp$.

The rejection case is analogous: if $\semfive{u \vDash L}=\bott$ then $u\in\ERL{L}$ and every strict extension lies in $\IRL{L}=\ERL{L}\,\Sigma^{+}$, while if $\semfive{u \vDash L}=\botp$ then $u\in\IRL{L}$ and $\IRL{L}$ is closed under strict extensions by definition.

\smallskip
\noindent\textbf{(2) Unique frontier.}
Assume $\semfive{u \vDash L}=\topp$. Then $u\in\IAL{L}=\EAL{L}\,\Sigma^{+}$, so there exist $m\in\EAL{L}$ and $x\in\Sigma^{+}$ such that $u=mx$. Let $u':=m$. Then $u'\prec u$ and $\semfive{u' \vDash L}=\topt$.

For uniqueness, suppose $u= m_1 x_1 = m_2 x_2$ with $m_1,m_2\in\EAL{L}$ and $x_1,x_2\in\Sigma^{+}$. Then both $m_1$ and $m_2$ are prefixes of $u$, hence they are comparable under the prefix order. If $m_1\prec m_2$, then $m_2$ is a strict extension of $m_1$ and cannot belong to $\minlang{L}=\EAL{L}$, contradicting minimality. Symmetrically, $m_2\prec m_1$ is impossible. Therefore $m_1=m_2$, and the strict prefix $u'$ with verdict $\topt$ is unique.

The same reasoning applies to the rejection side: if $\semfive{u \vDash L}=\botp$ then $u\in\IRL{L}=\ERL{L}\,\Sigma^{+}$, and the unique strict prefix with verdict $\bott$ is the unique factor from $\ERL{L}$.
...
$M_{\text{5tight}}(L)$ is linear in the state space of $\aut(L)$, \ie, $|Q|$:
\[
...
We work over the \emph{letter alphabet} as shown in the syntax of \cDL $\Gamma \ :=\ 2^{\Sigma_C^{(1)} \cup \Sigma_C^{(2)}}$,
...
\pi \modelsre \re_1\cdot\re_2 &\text{iff}& \exists k\leq \size{\pi} : 
  \pi_k \modelsre \re_1 \;\text{and}\; \pi^{k+1} \modelsre \re_2,\\[4pt]
\pi \modelsre \re^n &\text{iff}&  (\text{if } n>1 \text{ then } \pi \modelsre re \cdot re^{n-1}) \nd (\pi \modelsre re \text{ if } n=1),\\[4pt]
...
\begin{itemize}
  ...
  \item \textbf{Sequencing $(re_1 \cdot re_2)$.} There is a split index $k$ with
        $\pi[0,k]\modelsre re_1$ and $\pi[k{+}1,\size{\pi}]\modelsre re_2$.
  \item \textbf{Fixed power $re^n$.} Iterated sequencing of $re$ exactly $n$ times:
        $re^1\equiv re$; for $n>1$, $\pi\modelsre re^n$ iff $\pi\modelsre re \cdot re^{n-1}$.
