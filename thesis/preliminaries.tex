...
We transform the following timed constraints to intervals:
\begin{enumerate}
...
Allen's algebra \cite{allen1983maintaining} consists of 13 exhaustive and mutually exclusive relations between \textit{abstract}\footnote{Not inherently tied to a specific numerical domain.} pairs of intervals. To provide an intuition, in \ref{fig:ia} we give a visual interpretation of these relations. The different cases are based on the relative comparison of the start and end points of the intervals. In \ref{tab:IAdesc} we specify the constraints associated with each case for the domain of intervals $\mathbb{I}$ defined on natural numbers.
...
The {intersection} operator: $ \cap:\mathbb{I} \times \mathbb{I}  \to \mathbb{I}$  is defined as follows:
	\begin{align*}
		[a,b] \cap [c,d] := 	\left\{
		\begin{array}{lll}
			~	\varnothing & \text{iff} &  [a,b] \text{ before } [c,d]\text{ or } \\
			~               &       &  [c,d] \text{ before } [a,b], \\
...
The operators we define now are partially defined,since the interval domain is not closed under these operations.
...
To address this structural limitation, we introduce the concept of interval sets. An \emph{interval set}, denoted as $\is$, represents a collection of time points using a finite set of intervals. To ensure this representation is unique and minimal, we enforce a condition known as the \emph{canonical form}. This form guarantees that the set is composed of the smallest possible number of intervals by requiring that no two intervals in the set are overlapping or adjacent.
...
For the remainder of this dissertation, we adopt the following notational convention: we use $\is^*$ to refer to interval sets composed exclusively of finite (bounded) intervals. In contrast, we use $\is$ to denote interval sets that may contain infinite intervals, such as those extending to $+\infty$. 
...
These representations enable a symbolic encoding of timed constraints.  Consider
\[
\is_3 := \{ [2,4], [8,24] \}.
\]
Similarly, other constraints  could be represented using singleton interval sets. The constraint \textit{between 09:00 and 16:00}, for example, corresponds to:
\[
\is_1 := \{ [9,16] \},
\]
...
%    \end{example}
    ...
    For the sets of intervals $\is_1:= \{[9,16]\}$ and $\is_3:=\{[2,4],[8,24]\}$ we have: $\is_1 \subset \is_3$.
    \end{example}
    ...
    \begin{itemize}
        ...
        \item If $\mathfrak{I} := \{ [0, 4], [6, 10] \}$ and $\mathfrak{I}':= \{ [3, 7] \}$ then $\mathfrak{I}\cap \mathfrak{I}' = \{[3,4],[6,7]\}$.
    \end{itemize}

    For the intervals $\is_1$, $\is_2$, and $\is_3$ related to the previous examples:
    \begin{itemize}
...
where $\circ$ denotes set concatenation, and the subset of unifiable and non-unifiable intervals with $I$ from $\is$ are defined as:
\begin{align*}
...
$$\is \ominus \is' := \bigcup_{I \in \is}(I \ominus \is') \text{ with } I \ominus \mathfrak{I'}= \bigcap_{I_i \in \is'}(I \ominus I_i).$$
where the \emph{redefined} reduction of two intervals is now computed as a \emph{set of intervals} instead of a single interval as defined in the arithmetic of intervals: $\ominus: \mathbb{I} \times \mathbb{I} \to 2^\mathbb{I}$ is defined as :
	$$[a,b] \ominus [c,d] := \begin{cases} 		
...
From the intervals $\is_1, \is_2, \is_3$ studied before, we have:
\begin{itemize}
