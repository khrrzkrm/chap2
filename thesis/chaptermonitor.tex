...
\paragraph{Method and structure.}
 ...
 To reason about \emph{attempted, prevented, and successful collaboration}, we introduce our contribution of attempted/successful
  ...
  In this subsection, we make the abstraction process explicit by progressively encoding concrete interaction behavior into increasingly structured trace models. We start from raw timed observations, refine them into agent-local metric-timed traces, and then abstract them into synchronized, period-based set traces aligned with the contractual calendar.
  The purpose of this stepwise construction is methodological: each abstraction simplifies the representation while preserving the behavioral distinctions that matter for normative analysis, such as attempts, obstruction, and successful collaboration.
   In particular, the abstraction does not hide or reinterpret any behavior exhibited in the concrete execution, but instead re-expresses it at a granularity suitable for contractual reasoning.
   The start of the running illustration is Example~\ref{ex:timed-words-only} below illustrates this process starting from the rental contract use case, showing how observed interactions over four months are faithfully represented before any synchronization or success computation is applied.
  
   

   \begin{example}[Metric-timed traces from observation over a four-month rental interaction]
    \label{ex:timed-words-only}
    We return to the rental agreement introduced in Example~\ref{ME}, where a tenant (agent~1) and a landlord (agent~2) have signed a monthly contract regulating rent payment and occupancy.
    We observe their concrete behavior over a period of four months, corresponding to days $t\in\{0,\dots,120\}$.
    
    During this observation window, The but pays rent
    on day~5 (\textsf{pay\_rent}) but still does not occupy the flat, the landlord acknowledged on day~6 (\textsf{ack\_pay}). In the second month, the tenant
    ...
    \begin{aligned}
    \taumt_{1}
    &=\trace{(\textsf{pay\_rent},5),\ (\textsf{start\_occ},35),\ (\textsf{pay\_rent\_f},95)},\\
    \taumt_{2}
  ...
  In the fourth month, the tenant attempts to remedy the earlier violation by paying a late fee, which is explicitly rejected, followed by an active obstruction of occupancy through a lock change.
  ...
  A \defn{periodic synchronized set trace} over an alphabet $\Sigma^i$ for agent $i$ over a global clock with a fixed Period is sequence of sets of actions, i.e 
  \[
