...
We first ground the problem in a rental-agreement example and extract the modeling requirements. We then develop the trace and synchronization layer for collaborative interaction, introduce the logic \cDL, and finally present the monitor-based semantics, blame attribution, and quantitative extensions alongside with their monitor constructions.
...
The domain of formal contract specification is rich, with established languages ranging from contract specification lanaguages \cite{HvitvedKZ12,diaz2013specification,farmer2016fcl} to deontic based logics \cite{governatori2005representing,azzopardi2021}.  However, these existing frameworks often lack the specific combination of features identified above. Some formalisms lack concrete trace semantics or do not support operational run-time monitoring. Others focus on normative states but fail to capture the nuances of collaborative action or joint achievements. Conversely, certain logics are too expressive or abstract for the specific monitoring features we aim to capture. Consequently, rather than constructing a formalism from scratch, we adopt the trace-based architecture of \cite{Azzopardi2016}, previously refined in \cite{Kharraz23}, as our operational baseline. This choice provides a sufficiently expressive language with well-studied operational semantics, which we extend with (i) a layer of abstraction for collaborative interaction, (ii) specific normative operators, and (iii) a denotational semantics alongside its sound monitoring semantics supporting (i) and (ii).
...
First, these raw inputs are organized into \emph{agent-local metric-timed traces}, a format that strictly preserves individual responsibility and temporal order.
From this metric foundation, we impose contractual granularity by aggregating behavior into \emph{period-based set traces} aligned with the contract's calendar.
Finally, concrete actions within these sets are abstracted into \emph{collaborative objectives}.
