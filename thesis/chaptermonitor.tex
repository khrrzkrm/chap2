...
This chapter addresses the challenge of monitoring and diagnosing failures in collaborative systems where compliance depends on the joint behavior of multiple autonomous agents. Unlike traditional monitoring approaches that view system events as atomic and indisputable, we propose a framework that distinguishes between individual attempts and collective outcomes, enabling a more nuanced analysis of responsibility. By formalizing the interaction between agents as a collaborative process rather than a zero-sum game, we establish a rigorous basis for attributing blame when contractual obligations are not met. This foundation supports the development of a logic and a runtime verification pipeline capable of handling the temporal complexities, reparable violations, and open-ended nature of realistic multi-party contracts.
...
\item How can open-ended \emph{repetitions} of duties be specified and monitored within a contractual framework?
...
We introduce a formal algebra that distinguishes an agent's \emph{attempt} from the \emph{interaction outcome}. Unlike models that conflate failure to act with blocked action, we treat non-interference as a first-class semantic property, enabling the verification of ``active participation''.
