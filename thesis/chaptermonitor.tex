...
A \defn{periodic synchronized set trace} over an alphabet $\Sigma_i$ for agent $i$ over a global clock with a fixed period length is an infinite sequence of sets of actions indexed from $1$, that is,
\[
\pi_i \;=\; \trace{\Aik{i}{1},\ \Aik{i}{2},\ \Aik{i}{3},\ \dots}
\ \in\ \wordsets{\Sig{i}}^{\infty}.
\]
Each $\Aik{i}{k}\subseteq \Sig{i}$ represents the set of actions attributed to agent $i$ during period $k$
(empties allowed).
\end{definition}
...
Since $k<\ell$ and the windows are disjoint and ordered, we have
$\max(I_k) < \min(I_{\ell})$,
we obtain $t_a < t_b$.
\end{proof}
...
\[
\Ik{1}=[0,30],\qquad \Ik{2}=[31,60],\qquad \Ik{3}=[61,90],\qquad \Ik{4}=[91,120].
\]
...
\end{example}
  ...
  The tenant strategy and the landlord strategy are depicted explicitly by their Moore machine encodings in Subfigure~\ref{fig:moore-tenant-prod} and Subfigure~\ref{fig:moore-landlord-prods}, respectively.
  ...
  The individual Moore machines underlying the product construction are recalled in Sub-figure~\ref{fig:moore-tenant-prod} and Sub-figure~\ref{fig:moore-landlord-prods}, while their synchronous product is summarized in Sub-figure~\ref{fig:product-sketch} and presented as a whole in \ref{fig:moore-product-full}.
  \begin{figure}[h]
  \centering
  \begin{subfigure}[t]{0.70\textwidth}
  \centering
