...
\item How can a trace abstraction effectively capture the timing periodicity of open-ended contractual specifications?
...
\begin{itemize}
  ...
  \item \textbf{Normative Shift:}  The contract turns into a \emph{finite horizon contract} as it is now fixed to terminate at Month $8=5+3$. 
    ...
    \item \textbf{Verdict:} Violation and the blame will be attributed to the Landlord. The desired monitor semantics must enforce that the \emph{obligations persist} during the notice period. The exercising of power did not \emph{immediately} delete the obligations but merely set a horizon for their expiration.
...
We introduce the attempted/successful abstraction, suited for capturing interference and collaboration between agents in the context of collaborative normative specifications, and discuss how it resembles and differs from existing synchronization techniques in the literature.
 ...
 From these metric traces, where each event is a tuple of action and time point, we impose the contract-period granularity by aggregating behavior into \emph{period-based set of action traces} aligned with the contract’s period for repetition, where one event is a set of actions of all the actions performed during that time period.
 Then we merge both agents' traces \emph{collaborative objectives}, where an event is a set of actions tagged by the agent that performed them.
...
lockstep and handshakes on the collaboration alphabet.    
  ...
  However, this abstraction masks the specific source of interference, effectively hiding the agent responsible for the failure. As our framework explicitly assigns blame, we introduce this operator primarily as an optional view to characterize joint success.
