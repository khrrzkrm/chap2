...
This chapter addresses the challenge of monitoring and diagnosing failures in collaborative systems where compliance depends on the joint behavior of multiple autonomous agents, building on a corrected and reformulated syntax and semantics of our earlier work \cite{Kharraz23}. In a distributed setting, a core challenge is distinguishing the success of an action attributed to an agent from their attempts, as the success of this collaborative action may also depend on another agent's collaboration or non-interference. This distinction is essential for responsibility attribution in collaborative specifications, such as contracts, and cannot be reconstructed from standard trace semantics that do not reason at the attempt level. We establish a formal basis for blame attribution when contractual norms are not met. We build a logic and a runtime verification pipeline capable of handling temporal constraints, reparable violations, and the open-ended structure of real-world contracts.



The core novelty for handling collective norms lies in not abstracting away from collaboration itself by treating it as a semantic object. Unlike traditional runtime verification and temporal logic frameworks, which evaluate compliance solely in terms of successful outcomes, the framework checks compliance more precisely by considering what each agent attempted and what did succeed when collaboration was achieved.

\section{Overview}

In this section, we outline the theoretical and practical necessity for a blame-aware monitoring framework. We begin by motivating the problem through the lens of open-ended normative interactions, where simple binary verdicts of satisfaction or violation are insufficient for dispute resolution. We then detail the specific research questions that motivate this work and describe the methodology we use to address them. Finally, we summarize the chapter's core contributions, including the novel Attempt-Based Interaction Model and the Two-Agents Collaborative Normative Logic (\cDL), which collectively enable precise, automated accountability in contracts, which we decompose into a progression of four semantics.



\subsection{Motivation}

Normative specifications are typically presented as constraints on what an individual agent must do or avoid. In many practical contexts, however, where agents' actions may influence other agents' behavior, compliance is not achieved solely by individual agents but results from interactions in which different parties may enable, disregard, or obstruct one another. This chapter examines collaborative normative settings in which, even without zero-sum competition, strategic behavior can still arise to shift responsibility for failure to other agents. Our analysis focuses on two agent settings, which serve as the base case for normative specifications of multi-agent collaborations. 

\paragraph{Interactions and collaboration.}

Standard deontic formalism describes duties in isolation, yet many norms require that a desired outcome be realized through the joint contributions of both parties. For instance, in a rental contract, a tenant's payment is valid only if the landlord provides a channel for it.  On the other hand, occupancy requires both the tenant's attempt to occupy and the landlord's non-obstruction. A formal model must therefore distinguish between the \emph{attempts} made by each agent and the \emph{successful collaboration} resulting from their compatibility. Additionally, this model has to account for \emph{non-interference}, ensuring that compliance involves refraining from actions that obstruct the counterparty.

\paragraph{Open-ended normative interactions.}

Contracts often govern long-term collaborations that persist indefinitely through a repetition of routines. A rental contract, for example, imposes the repetition of obligations and conditions that recur over time unless terminated by an agreement between the parties. 

This motivates the development of a trace model that supports aggregation at the granularity of contract periods, such as months, and that can capture open-ended specifications for \emph{prefix-based violation} when the contract is violated and for \emph{prefix-based satisfaction} when the contract is terminated without prior violation.

\paragraph{Blame and accountability.}
When a collaborative norm is breached, it is necessary to identify the responsible agent and provide a justification or an out-of-contract resolution, such as going to court.

Courts, for example, must reconstruct event timelines to determine if a landlord's initial obstruction  justified a tenant's subsequent non-payment.

Beyond establishing causality, practical analysis requires a \emph{quantitative assessment} of non-compliance, distinguishing a simple binary verdict from a granular count of responsibility such as the number of unpaid months or denied access.
...
\item How can collaboration be efficiently modeled at the trace level, distinguishing individual attempts from jointly successful outcomes?

\item How can a trace abstraction faithfully capture the timing periodicity of open-ended contractual specifications?

\item How can open-ended clauses, specifically \emph{repetitions} of duties and rights, be specified and monitored within a contractual framework?

\item How can quantitative blame be semantically defined and automatically monitored  to reflect legal reasoning?
...
\paragraph{Phase 1: Semantic Trace Abstraction.}
We suggest a Periodic Synchronous trace model in which events are sets of tagged actions by the agent's name; each event captures collaboration or obstruction during that period. The synchronization is soundly computed as a handshake of actions of the same types when both agents ``suggest " them during that period.
\paragraph{Phase 2: Denotational Specification.}
We adopt a \emph{semantics-first} logic definition. We first focus on \emph{tightness}, i.e., the ability to render precise verdicts on finite prefixes. We start with binary compliance and refine it into a blame-assignment model. Secondly, we extend those semantics to the quantitative domain.
\paragraph{Phase 3: Structural Synthesis.}
We use \emph{automata-theoretic synthesis from semantics}. We propose sound and complete monitor construction to Moore and Mealy machines, employing structural induction and/or contract closures to ensure the monitor's structure mirrors the contract semantics.
\paragraph{Formal Verification.}
We validate the correctness of the formal pipeline through \emph{conformance proofs} between phases 2 and 3 and between the tight and quantitative semantics to enhance confidence in the framework. We prove that the synthesized automata are logically isomorphic to the denotational semantics, guaranteeing that the executable artifacts faithfully represent the normative intent. We also study the relationships of verdicts between the four semantics.
\subsection{Contributions}
This chapter's contribution is a progressive framework for specifying and verifying collaborative normative systems at runtime, where the final result is obtained via a transition over four semantics. The primary contribution is a verified pipeline that grounds the semantics' soundness in legal reasoning and synthesizes this semantics into online monitors. The list of contributions is:
...
We formalize \cDL, a logic to capture our formal challenges. It extends standard deontic specifications with atomic responsibility modalities, regular expressions used to condition and guard the end sequences of norms, and explicit operators for repetition and contrary-to-duty reparations.
\item \textbf{Theory of Tight and Quantitative Semantics.}
We introduce a \emph{Forward Tight Semantics} for online monitoring. This includes a 5-valued verdict domain that distinguishes earliest violations/satisfaction prefixes from post-violation suffixes, and a \emph{Quantitative Blame Semantics} that accumulates a violation/blame score over time for open-ended contracts beyond the first violation.
...
We first ground the problem in a rental-agreement example and extract the modeling requirements. We then develop the trace and synchronization layer for collaborative interaction, introduce the logic \cDL, and finally present the monitor-based semantics, blame attribution, and quantitative extensions alongside their monitor constructions.
...
To illustrate the theoretical development in a concrete real-life contract, we examine a simplified residential flat rental agreement. Some parts of the agreement are abstracted away to capture only the aspects discussed in our research questions. 



\begin{example}\label{ME}

    We present the structure of this simplified rental contract in \ref{fig:ucd}, involving a tenant (agent~1) and a landlord (agent~2). The clauses illustrate cooperative actions such as paying rent and granting occupancy: payment requires both the tenant's offer and the landlord's acceptance, while occupancy requires both the tenant's willingness to occupy and the landlord's permission. The contract also includes a reparation clause and a termination condition, both of which are applied monthly.
...
We analyze three hypothetical execution traces over the timeline $t=1, 2, \dots$, where each index corresponds to a month. These scenarios highlight the divergence between complete observation of the behaviors of both the landlord and the tenant, as reflected in the contract, and normative analysis, as reflected in a real-life court setting.
...
\begin{itemize}
    ...
    \item \textbf{Normative Analysis:} The court will conclude the prevention of performance, and the blame will be attributed to the landlord. A naive monitor observing only the ``payment status'' would correctly detect the violation of the payment clause, but would attribute the blame to the tenant. However, in a collaborative framework we want to design, the tenant has fulfilled their duty by \emph{attempting} the act. The failure is attributed to the landlord's interference, necessitating a distinction between \emph{attempted payment} and \emph{successful payment}.
...
\begin{itemize}
    ...
    \item \textbf{Month 2:} tenant pays the total accumulated fine amount.
...
\item \textbf{Normative Shift:} Clause C3 specifying the reparation activates. The tenant now holds a \emph{secondary obligation}: pay Month 1 Rent + Month 2 Rent + 10\% Fee at Month 2. As such, the violation of one norm does not necessarily imply the violation of the contract.
...
Consider the tenant exercising the power to terminate as in Clause C5.

\begin{itemize}

    \item \textbf{Month 5:} The tenant issues a formal written notice to terminate.

    \item \textbf{Month 6-8:} The tenant stops paying rent, assuming the notice ended the contract immediately.

  \item \textbf{Normative Shift:}  The contract turns into a \emph{finite horizon contract} as it is now fixed to terminate at Month $8=5+3$. 

    \item \textbf{Verdict:} Violation and the blame will be attributed to the tenant. The desired monitor semantics must enforce that the \emph{obligations persist} during the notice period. The exercising of power did not \emph{immediately} delete the obligations but merely set a horizon for their expiration.
...
Clauses C1 and C2 illustrate that compliance is a \emph{joint achievement}. For example, ``Rent Payment'' (C1) semantically implies that the tenant offers the money \emph{and} the landlord accepts it. Standard trace models often represent this as a single atomic event $pay$. However, in a dispute, we must distinguish between the tenant failing to pay rent (tenant violation) and the tenant offering to pay but the landlord blocking the transfer (landlord interference).\\

\textbf{Challenge 1:} The formalism must treat \emph{attempts} and \emph{successful outcomes} as distinct semantic layers, one for the fine-grained blame attribution, and may use the second for contract violation.

\paragraph{2. Contrary-to-Duty Structures: Reparation}

If C1 is violated, the contract does not simply conclude a violation, but instead, it transitions to a sub-ideal state in which a secondary reparation obligation becomes active. This creates a \emph{reparation chain}: Primary Duty $\to$ Violation $\to$ Secondary Duty.\\
...
\paragraph{Construction of the Formal Framework}

The domain of formal contract specification is rich, with established languages ranging from contract specification languages \cite{HvitvedKZ12,diaz2013specification,farmer2016fcl} to deontic-based logics \cite{governatori2005representing,azzopardi2021}.  However, these existing frameworks often lack the specific combination of features identified above. The deontic logic formalisms lack concrete trace semantics and thus do not support operational run-time monitoring. Temporal-logic-oriented frameworks focus on normative states but fail to capture the nuances of collaborative action or joint achievement.  Consequently, rather than constructing a formalism from scratch, we extend the existing work \cite{Azzopardi2016}, previously refined in \cite{Kharraz23}, as our operational baseline. This choice provides a sufficiently expressive language supporting basic aspects of contract with well-studied operational semantics, which we extend with (i) a layer of abstraction for collaborative interaction, (ii) specific normative operators, and (iii) a denotational semantics alongside its sound monitoring semantics supporting (i) and (ii).
...
This section builds on the trace models and synchronization operators introduced in \ref{traces}. We keep periodic synchronized set traces as the underlying execution model and define an interaction layer that (i) distinguishes each agent's local attempts  and (ii) prepares the ground for strategy analysis and, later, normative evaluation by grouping those local attempts according to the contract periods.
...
We introduce the attempted/successful abstraction, suited for capturing interference and collaboration between agents in the context of collaborative normative specifications, and discuss how it resembles and differs from existing synchronization techniques in the literature.
 ...
 Standard interleaving and lockstep semantics provide formal rules for combining agents’ actions over time. These standard models, however, abstract away from a distinct phenomenon central to normative reasoning: environment manipulation. The issue is not simply whether actions occur asynchronously, but whether an attempted action can be defeated by another agent altering the foundational settings before the attempted action. Standard semantics easily model a simple delay in a tenant's rent payment. Our trace abstraction allows us to still observe the landlord's action of closing his bank account, even without seeing the environment change. But a more fine-grained abstraction that also shows the account's state is still necessary.
 
 \subsection{From Metric-Timed to Collaborative Periodic Synchronized Set Traces}
 
 In this subsection, we explicate the abstraction process as a two-step transformation starting from two timed traces that record the timing and content of agent actions.
 
 From these metric traces, where each event is a tuple of action and time point, we impose the contract-period granularity by aggregating behavior into \emph{period-based action-set traces} aligned with the contract’s period for repetition, where one event is a set of actions of all the actions performed during that time period.
 Then we merge both agents' traces into \emph{collaborative objectives}, where an event is a set of actions tagged by the agent that performed them.
 This provides a representation where attempts, obstructions, and potential joint successes can be analyzed independently.
 This trace transformation pipeline preserves the timing setting required for blame assignment, although it loses the timing of actions performed during the same period.
 No behavior is discarded or reinterpreted; rather, it is lifted to a level of abstraction suitable for contractual reasoning.
 Example~\ref{ex:timed-words-only} illustrates four-month behaviors of both agents before our transformation is applied.
 ...
 In the second month, the interaction becomes more active. On day~35, the tenant begins occupying the flat (\textsf{start\_occ}) and then makes the rent payment the next day. The landlord responds by issuing an administrative acknowledgment on day~37 (\textsf{ack\_pay}).
 ...
 The observed behavior of each agent can be represented as a metric-timed trace, where each event is paired with its occurrence time:
 $$
 \begin{aligned}
 \taumt_{1}
 ...
 In the first month, rent is paid by the tenant and correctly accepted by the landlord, and occupancy is granted but not yet exercised. In the second month, occupancy begins, and payment is acknowledged without conflict.
 ...
 While this description summarizes each agent's behavior, these traces must be formatted to align with our contract semantics. The first transformation regroups them by contract period.
 ...
 within the same window into a \emph{set}. This ends up as a synchronous round-based abstraction aligned with the contract schedule.
...
\begin{proof}
 By case analysis applied on the definition of $\AGGI_x$, assume $t_a \in I_k$ and $t_b \in I_{\ell}$. Any two events either fall into  (i) the same period $k=\ell$, which does not concern the claim of the Lemma,  or (ii) into different periods preserving their strict temporal order.  For this case, By assumption,  $k < \ell$, the second case applies: $t_a \le \max(I_k) < \min(I_{\ell}) \le t_b$, hence $t_a < t_b$.
 \end{proof}

Nevertheless, aggregation has a cost, as it loses intra-period order, action multiplicity, and exact timestamps.
...
The second aspect of the abstraction is the notion of interaction alphabets. The action alphabet of both agents is merged into a single alphabet, and the trace encodes that when $a$ is an event, it indicates that an agent has either instantiated it or accepted to collaborate on it. Conversely, when no action is present in the event set $I_k$, the agent either did not perform it or actively performed a blocking action to prevent it. This transformation cannot be automated unless the set of enabling and interfering actions is explicitly stated in the normative specification. Otherwise, the digitization engineer must define them manually, as is illustrated in what follows.
...
In the next step, we need to define how the actions of the agents, namely $\Sigma_1$ and $\Sigma_2$, relate to collaborative action over $\Sigma_C$, more specifically, whether they are enabler or interference actions. We do not take the union \(\Sigma_1\cup\Sigma_2\) as a synchronization operator discussed in Section~\ref{operators}. Instead, we define a many-to-one \emph{abstraction} from concrete per-agent actions to the collaboration alphabet \(\Sigma_C\).

On the tenant side, \textsf{pay\_rent} and \textsf{pay\_rent\_f} are enablers because they instantiate the tenant’s contribution toward \PAY and \PAYF; \textsf{start\_occ} is an enabler for \OCC because it is the tenant’s side of taking possession. Chargebacks \textsf{return\_payment} and \textsf{return\_payment\_f} are blocking: they nullify the very transfer that \PAY and \emph{PAY\_F} rely on. On the landlord side, \textsf{ack\_pay} enables \PAY, while \textsf{grant\_occ} enables \OCC by authorizing access. In contrast, \textsf{ref\_pay} blocks \PAY even if the tenant initiated payment, and \textsf{change\_lock}, \textsf{cut\_power}, or \textsf{enter\_flat} block \OCC by making continued possession impracticable or unlawful. This enabler/blocker partition is precisely what our “suggested/successful” abstraction needs: success in a period occurs when both sides propose the required enablers and neither side performs a blocker.
...
\[
E_i:=\SigA{i}\quad\text{and}\quad B_i:=\SigI{i}\qquad \text{for } i\in\{1,2\}.
\]
...
\end{example}
  ...
  \subsubsection{Optional Step: Successful Collaboration Trace}
   ...
   The abstraction introduced so far separates agent-local attempts and interference while preserving all information required for responsibility analysis.
    ...
    In some situations, however, it is useful to additionally extract the \emph{jointly successful} outcomes that emerge when both agents contribute compatibly within the same contractual period.
     ...
     We introduce an optional transformation that computes such outcomes explicitly. The construction follows the same underlying principle as lockstep synchronization with handshakes  inspired by Definition~\ref{def:collapsed-hs}: a collaborative action is realized at a given period if and only if both agents propose it in that period. The result is obtained by computing the successful action according to the handshake mechanism studied in the preliminaries. 
  ...
  \]
  where the function $\unlab$ removes the agent identifier tags from the actions in each event, \eg,  $\unlab(\{\OCC^{(2)}\})=\{\OCC\}$.
  \end{definition}
  \begin{example}[Successful collaboration]
  From the two transformed traces  $\pi_1^{C}$ and $\pi_2^{C}$ from the Example~\ref{ex:collab}.
  We illustrate \(\mathrm{Succ}(\pi_1^{C},\pi_2^{C})\) in Example~\ref{example:succ-meet}.
  \end{example}
...
The \(\mathrm{Succ}\) operator is a point-wise set intersection after tag erasure; thus, it inherits three straightforward facts:

  \emph{(i) Commutative and idempotent}:
  \(\mathrm{Succ}(\pi_1,\pi_2)=\mathrm{Succ}(\pi_2,\pi_1)\) and
  \(\mathrm{Succ}(\pi^{(1)},\pi^{(2)})=\unlab(\pi)\).
  ...
  If we declare every collaborative objective to be a handshake, \(A=\Sigma_C\), then the lockstep-with-handshakes product enforces that \(\PAY\), \(\PAYF\), \(\OCC\) can only appear at a period when both agents present the same letter. Concretely, if we view each period’s set
  \(A^{(i)}_k\) as the multiset of letters occurring “at that round” and apply
  \(\parallel_{\mathrm{hs}}^{A}\) round-wise, then after collapsing the matched
handshaked unlabelled actions, we obtain the success:
  \[
  \mathrm{Succ}(\pi_1,\pi_2) \ =\  \unlab\!\big(\mathsf{coll}_{\Sigma_C}\big(\pi_1\parallel_{\mathrm{hs}}^{\Sigma_C}\pi_2\big)\big),
  \]

where \(\mathrm{Succ}\) is the set-theoretic intersection semantics of
lockstep and handshakes on the collaboration alphabet.    

  \end{remark}

  However, this abstraction masks the specific source of interference, thereby concealing the agent responsible for the failure. As our framework explicitly assigns blame, we introduce this operator primarily as an optional view to characterize joint success.
  ...
  Most two-agent automata constructions ignore agents' reactions to their counterparts' past moves and abstract away from this using nondeterminism. As a consequence, some impossible executions are present in the model executions. In this subsection, we encode the agents' strategies as Moore machines; each agent bases its next move on the other agents' last moves. By doing so, the composition of two Moore machines produces an infinite word instead of a Moore machine or automaton.
  ...
  \[
  \pi \;=\; \trace{A_1,A_2,A_3,\dots} \;\in\; \big(2^{\Sigma_O^i \cup \Sigma_O^j}\big)^\omega,
  \]
  where
  \[
  A_t \;=\; \lambda_i(s_{i,t}) \cup \lambda_j(s_{j,t}), \qquad t \ge 1,
  \]
  ...
  By induction on $t$, this produces a unique run of the product automaton starting from $(s^0_i,s^0_j)$.
  ...
  The individual Moore machines underlying the product construction are recalled in Figure~\ref{fig:moore-tenant-prod} and Figure~\ref{fig:moore-landlord-prods}, while their synchronous product is summarized in Figure~\ref{fig:product-sketch} and presented as a whole in Figure~\ref{fig:moore-product-full}.
  \begin{figure}[h]
  \centering
  ...
  \end{example}
  This subsection establishes a way to transform the agent plan and traces to accommodate higher-level reasoning about rules. We started with basic timed observations, then gradually turned them into periodic, synchronized traces.

This model naturally accounts for attempts, interference, and joint outcomes without relying on external assumptions. It also facilitates the analysis of compliance both \emph{a posteriori} and \emph{a priori}, by enabling the evaluation of agent strategies and planned actions.

Importantly, this approach separates individual compliance efforts from joint collaborative outcomes, providing the necessary granularity for blame assignment. The shared periodic trace thereby bridges raw interaction data and the normative logic developed in the following sections.
  
  \section{The Two-Agents Collaborative Normative Logic \cDL}

  The abstraction developed in the previous section provides the execution model for our normative reasoning. 

  Having isolated individual attempts, interference, and joint outcomes, we now define a logic whose semantics are directly tied to this periodic structure. 
  The Two-Agents Collaborative Normative Logic (\cDL) expresses obligations, permissions, reparations, and powers over collaborative objectives rather than low-level events.
Its formulas are evaluated over collaborative periodic traces, ensuring that normative judgments remain sensitive to agent-specific attempts and obstruction.
Thus, \cDL builds on the interaction model to provide precise, blame-aware contractual semantics.
...
Regular expressions in \cDL specify \emph{triggers and termination conditions} for contracts.

  An \emph{atom} is a set of agent-tagged actions \(A \in 2^\Gamma\), representing the joint behavior of both parties in a single time step. 

Regular expression combinations utilize classical operations: \emph{union} \((re \mid re)\), \emph{concatenation} \((re \cdot re)\) for temporal sequencing, \emph{power} \(re^n\) for exactly \(n\) repetitions, and \emph{Kleene plus} \(re^+\) for one or more repetitions. The \emph{wildcard} \(\Gamma\) denotes any $A \subseteq \Sigma$ and is used to advance by one timed position, while \(\emptyset\) represents the \emph{empty set of actions}.

These constructs enable the specification of patterns such as: (i) ``When the flat repair is requested by agent 2 this month'', with the regular expression \(\notifrepair^{(2)}\), (ii) ``at any month after the start of the contract, if Agent $1$ requests termination'', with \(\Gamma^* \cdot \{\notifterm^{(1)}\}\), or (iii) ``after three consecutive months'', with the expression \(\Gamma^3\).
  \begin{figure}
  \centering
  \fbox{%
 ...
 Let $a \in \Sigma_C$ denote a collaborative action, $A \in 2^{\Gamma}$ a party-tagged action set, and $n \in \mathbb{N}^*$ a non-zero natural number.
  ...
  \emph{obligation} \(\obl[p]{a}\), \emph{prohibition} \(\frb[p]{a}\), and \emph{power} \(\perm[p]{a}\), and the two \emph{valid} \(\top\) and \emph{invalid} \(\bot\).
  Here \(p \in \{1,2\}\) identifies the party in the set of the two agents, and \(a \in \Sigma_C\) is a collaboration action.
  Literals define \emph{what} is required, forbidden, or allowed of \emph{whom}, independently of timing. 

  \paragraph{Contracts (\emph{C}).}

A contract is composed of a combination of literals using the following operators:
\emph{conjunction} \((C \wedge C)\) to combine requirements in the same time position;
\emph{sequence} \((C;C)\) for next-time progression;
\emph{reparation} \((C \repair C')\) for contrary-to-duty fall-backs, which mandates $C$, but if $C$ is violated, enforces $C'$ as a fallback in the subsequent time step;
\emph{triggered} clauses \(\trig[re]{C}\) that activate \(C\) when a pattern \(re\) occurs;
\emph{guarded} clauses \(\guard[re]{C}\) that specify conditions under which a contract is terminated; 
\emph{n-repetition} \(C^n\) enforces the contract for \(n\) consecutive steps (\(n \in \mathbb{N^*}\)); and finally, \emph{repetition} \(\repit{C}\) enforces the contract indefinitely.

Together, these constructs formalize contractual clauses, their activation and termination conditions, and their temporal or reparative dependencies. In particular, combining repetition with a guarded contract captures the notion of open-ended agreements.
  ...
  \medskip
 Thus, clauses C1–C5 model the full lifecycle of the conditional and time-bounded obligations of the rental agreement.
  \end{example}
