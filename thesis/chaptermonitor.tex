...
Similarly, occupancy is meaningful only if one party grants access while the other actually attempts to occupy.
...
Rather than defining the logic in terms of its execution (e.g., how an automaton processes it), we first define its abstract truth conditions.
...
We start with binary compliance semantics, refine it into a blame-assignment model, and finally extend it to the quantitative domain.
...
Our methodology relies on structural induction: we define an operational transformation for each logical operator, ensuring that the monitor's structure mirrors the contract's.
...
This enables the formal verification of ``active participation'', determining whether an agent did everything in their power to fulfill a duty.
...
In this subsection, we make the abstraction process explicit by structuring it into a 2-steps transformation. We start from raw timed observations that record what each agent did and when. 
These observations are first organized into agent-local metric-timed traces, preserving responsibility and temporal order. 
We then impose the contractual granularity by aggregating behavior into period-based set traces aligned with the contract calendar. 
Finally, concrete actions are abstracted into collaborative objectives, yielding a representation in which attempts, obstruction, and potential joint success can be analyzed independently. Each step reduces representational detail while preserving all distinctions that are normatively relevant.
No behavior is discarded or reinterpreted; it is only lifted to a level suitable for contractual reasoning. Example~\ref{ex:timed-words-only} serves as the starting point of this pipeline, grounding the abstractions in a concrete four-month execution before any synchronization or success computation is applied.
...
A \defn{periodic synchronized set trace} over an alphabet $\Sigma^i$ for agent $i$ over a global clock with a fixed Period is a sequence of sets of actions, i.e 
\[
\pi_i \;=\; \trace{\Aik{i}{0},\ \Aik{i}{1},\ \Aik{i}{2},\ \dots}\ \in\ \wordsets{\Sig{i}}^{\infty}.
\]
...
\[
\taumt = \trace{(a_0,t_0),\dots,(a_n,t_n)}
\quad\text{with } a_j \in \Sigma,\ t_j \in \mathbb{N},
\]
we partition time into period windows
\[
I_k := [kx,(k+1)x-1] \quad (k \in \mathbb{N}).
\]
The aggregated periodic synchronized set trace
\[
\AGGI_x(\taumt) := \pi = \trace{A_0,\dots,A_K}
\]
is defined by
\[
...
where
\[
K := \max\{\, k \mid I_k \cap \{t_0,\dots,t_n\} \neq \emptyset \,\}.
\]
\end{definition}
\begin{proof}
...
Let
\[
\pi_i := \AGGI_x(\taumt_i) = \trace{A^{(i)}_0,\dots,A^{(i)}_K}.
\]
Then aggregation preserves inter-period order in the following sense:
\[
\forall k < \ell\ \forall a \in A^{(i)}_k\ \forall b \in A^{(i)}_{\ell}\;
\exists\, t_a \in I_k,\ t_b \in I_{\ell}:
...
\end{example}
  ...
  The tenant strategy and the landlord strategy are depicted explicitly by their Moore machine encodings in \ref{fig:moore-tenant} and \ref{fig:moore-landlord}, respectively.
  ...
  \noindent\textit{Formalization (matchingref{fig:moore-strategies}).}
  \[
  \begin{aligned}
  ...
  The individual Moore machines underlying the product construction are recalled in \ref{fig:moore-tenant-prod} and \ref{fig:moore-landlord-prod}, while their synchronous product is summarized in \ref{fig:product-sketch} and presented as a whole in \ref{fig:moore-product-full}.
  \begin{figure}[h]
  \centering
  \begin{subfigure}[t]{0.70\textwidth}
  \centering
  ...
  \noindent
  Concretely, The run of the product $M_1^1 \times M_2^1$ in  \ref{fig:product-sketch}, corresponds to the omega-word\\
  ...
  \emph{n-repition} clauses inforces the repition of the enforcement of a contract $n$ times, with $n \in \mathbb{N^*}$; and finally, \emph{repetition} \(\repit{C}\) for the infinite repetition of the enforcement of a contract.
