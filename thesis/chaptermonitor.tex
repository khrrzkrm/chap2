...
\begin{proof}
 By case analysis applied on the definition of $\AGGI_x$, assume $t_a \in I_k$ and $t_b \in I_{\ell}$. Any two events either fall into  (i) the same period $k=\ell$, which excludes the claim of the Lemma,  or (ii) into different periods preserving their strict temporal order.  For this case, By assumption,  $k < \ell$, the second case applies: $t_a \le \max(I_k) < \min(I_{\ell}) \le t_b$, hence $t_a < t_b$.
 \end{proof}
...
The \(\mathrm{Succ}\) is a point-wise set intersection after tag erasure; thus, it inherits three straightforward facts:
...
lockstep and handshakes on the collaboration alphabet.    
  ...
  Most two-agent automata constructions ignore agents' reactions to their counterparts' past moves and abstract away from this using nondeterminism. As a consequence, some non-possible executions are present in the model executions. In this subsection, encode this agent's strategies as Moore machines; each agent bases its next move on the other agents' last moves. By doing so, the composition of two Moore machines is an infinite word instead of a Moore machine or automaton.
