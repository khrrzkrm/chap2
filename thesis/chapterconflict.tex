...
As discussed in the introduction chapter, normative systems provide the formal backbone for how organizations, individuals, software, and autonomous agents are expected to behave. A \emph{normative specification} describes these expectations as a set of rules determining what an agent ought to do or what the state of affairs ought to be. However, dealing with such specifications is inherently difficult. They combine different types of rules, such as obligations, prohibitions, and permissions, whose interplay can lead to overlapping prescriptions. In many situations, complying with one norm causes the violation of another. This situation is referred to as a \emph{normative conflict}.
...
Early works such as~\cite{DBLP:conf/hicss/DignumK98,DBLP:conf/deon/BroersenDDM04} formalized temporal deontic constraints by linking obligations to deadlines.  

A second wave of studies illustrated in~\cite{BroersenBrunel2007} examined how unmet obligations propagate over time, capturing the principle that “what I fail to do today, I must do tomorrow,” which formalizes the notion of deadlines and reparation as a delayed disjunction when the deadline of an obligation has passed without complying with it.  

In parallel, \cite{governatori2007characterising,governatori2011justice} introduced a temporal defeasible logic that characterizes different types of deadlines; one of its main features is reasoning about when two norms' timed conditions coincide. Thanks to its defeasibility, it distinguishes between violations and exceptional conditions when a rule is considered stronger (defeater) than another one.
...
Conflicts in timed normative specifications, as opposed to untimed specifications, depend not only on \emph{how many} norms are active simultaneously, but also on when their timed conditions are enforced. For instance, when many obligations are active at the same time, an agent may fail to comply with all of them, as their timed conditions are not collectively achievable. Also, when an action is due at a specific time point but is also prohibited at that time, there is a conflict. As such, conflicts can be seen as a scheduling problem; one must also reason about their activation periods to detect them.
...
Early approaches, such as \cite{fenech2009automatic}, reasoned about incompatible obligations and prohibitions in a logical-timed setting through reachability analysis over automata whose states were annotated with deontic labels; every reachable state that contained both a prohibition and an obligation was reported as a conflict.  While effective for identifying logical contradictions, this view abstracts away from when and how conflicts arise.

 Other works, such as \cite{colombo2014detecting}, show that such inconsistencies result not only from obligation and prohibition clashes, as in $O\alpha \land O\lnot\alpha$, but from \emph{temporal infeasibility}, where no timed trace can satisfy the conjunction of active norms. While this work introduced a rich classification of conflict types, it drew a direct equivalence between normative conflicts and unsatisfiability.

More recent work, such as N-Check~\cite{10.1145/3597503.3639093}, reduces the analysis of normative requirements to first-order satisfiability checking, diagnosing conflicts as unsatisfiable rule combinations. They raise fewer conflict types but offer a more nuanced definition of conflicts and their relation to unsatisfiability, where they consider a conflict to be a situation that, either always or in some feasible scenario, inevitably leads to the violation of one or more rules.

\paragraph{Research gap and questions}

Related works define a type of conflict and/or a procedure for detecting when a normative specification conflicts; they typically adopt either a syntactic or a semantic approach, but rarely both, leaving it unclear what really \emph{constitutes} a normative conflict. 

This lack of formal precision raises three fundamental research questions:
...
\begin{enumerate}[RQ1]
  ...
  \item \textbf{Relation between Unsatisfiability and Conflict:} The relationship between general logical unsatisfiability and specific normative conflicts is currently not well formalized. Is every unsatisfiable formula a conflict? Conversely, does the presence of a conflict necessarily imply that the formula is unsatisfiable?
    ...
    \item \textbf{Properties-Preserving Resolution:} Is it always possible to resolve conflicts in a formula without compromising its structural compactness or changing its accepted behaviors?
...
To address these challenges, we first formalize the potential behaviors of agents as finite timed words by structurally encoding them as a sequence of timed events consisting of \emph{a unique action} per event. We introduce a light logic, $\TDDLfm$, that allows us to formalize normative specifications in which time conditions are represented as finite interval sets. This trace-based semantics serves as the ground truth for our analysis.



We first define two fundamental types of normative timed conflicts. The first is \emph{deontic}, where rules contradict each other. The second is \emph{ontic}, where rules demand more than the agent can physically perform, given the assumptions we impose on metric timed traces. We show that conflicts often hide within complex time intervals. To expose them, we develop a rigorous transformation process that unfolds specifications into their atomic components. Formally, we prove that this process is structure-preserving. It reveals only the conflicts present in the original design, without creating spurious ones or erasing existing ones.



With these atomic conflicts exposed, we introduce a quantitative metric to measure the density of inconsistencies in a system. We demonstrate that this measure is directly linked to whether a specification is logically solvable. A fully conflicting system is impossible to satisfy. Conversely, a partially conflicting one can always be automatically repaired. We provide an algorithm for this repair that soundly prunes the contradictory parts while preserving the valid compliant behaviors.
...
Finally, we address the practical trade-off between automation and conciseness. Automatically eliminating conflicts can sometimes result in verbose specifications. To mitigate this, we complement our algorithms with a \emph{conflict calculus}. This set of inference rules allows for high-level symbolic reasoning. It enables designers to resolve conflicts manually and simplify specifications without needing to decompose the entire formula.

\subsection{Contribution}



In this chapter, we provide a logical framework for \emph{metric-time normative conflicts}. Our specific contributions are as follows:



\begin{enumerate}[Con1]

    \item \textbf{Formal Machinery:} We define a compact, semantics-first, action-based logic over finite interval sets. We provide a syntactic definition of conflicts and the disjunctive punctual normal form that preserves conflicting time points, and build a sound and complete algorithm for conflict detection.

    

    \item \textbf{Conflict Characterization:} We prove that the types of conflicts we define correspond precisely to Minimal Unsatisfiable Subsets (MUS) of product terms in the disjunctive punctual normal form of the original input formula. Conversely, we show that any MUS is necessarily an instance of one of these conflict types.

    

    \item \textbf{Quantitative Metrics:} We introduce the quantitative notion of \emph{conflict density}, which measures the ratio of conflicting product terms to the total number of product terms in the disjunctive punctual normal form. We then relate a conflict density of 1 to unsatisfiability, and show that specifications with any lower density can be repaired.

   

    \item \textbf{Elimination and Guarantees:} We provide a sound conflict elimination procedure for not fully conflicting formulas that returns a repaired, conflict-free formula. We study the impact of elimination on the size of the repaired formula produced by this process, demonstrating that while eliminating conflicts can increase the syntactic size of the specification, the blow-up remains bounded under well-defined structural conditions.



    \item \textbf{Conflict Calculus:} We provide a conflict calculus that can be used as an interactive normative system assistant where the user can check conflicts using inference rules. The conflict calculus uses generalized rules over interval sets instead of time points to mitigate the blow-up induced by the disjunctive punctual normal form.
...
Individuals and legal entities affected by the normative system are called \emph{agents}. These agents are required to follow the prescriptions and rules of the contract or agreement, which are collectively termed \emph{norms}. In this dissertation, we focus on formalizing norms concerning a \emph{single} agent, leaving multi-agent scenarios for future work. 
...
First, the Delivery Clause in \ref{tab:contract}, where an agent can choose between delivering at the gate and delivering at the parking between 9:00 and 16:00,  and could be represented in \TDDLfm as:
$$\DC := \ob{\{[9,16]\}}{\delAtGate} \sqcup \ob{\{[9,16]\}}{\delAtPark}.$$
% 
The Safety Regulation norm from \ref{tab:contract} prohibits the delivery of dangerous goods at the parking between 10:00 and 14:00, and could be expressed in \TDDLfm as follows:
$$\SR:= \forb{\{[10,14]\}}{\delAtPark}.$$
%
Lastly, the Maintenance Announcement in \ref{tab:ma}, prohibits the delivery at the gate between  2:00 and 04:00 and between 8:00 and 24:00, and could be written in our logic as follows:
...
\begin{itemize}
\item For $\DC:=\ob{\{[9,16]\}}{\delAtGate} \sqcup \ob{\{[9,16]\}}{\delAtPark}$, we have \\ $\size{\DC}= (2+2)+1+(2+2) = 9$ and $\eta(\DC)=2$.
...
\subsubsection{Timed Words Semantics}\label{subsec:timed-words-semantics}
% \subsubsection{Timed Words Semantics}
\begin{definition}[Satisfaction relation]\label{def:satisfaction-relation}
Given a finite \timedword $\tau \in \domtr$ and a formula $\phi$ from \TDDLfm, the satisfaction relation $\tau \dutysat \phi$ (read as``the \timedword $\tau$ satisfies the formula $\phi$'') is defined inductively on the structure of the formula $\phi$ as follows:
    \begin{align}
   \tau \dutysat \ob{\is}{a}
    &\text{ iff } \exists t \subin \is: \rho(\tau,t) = \acta. \label{obis}\\
	\tau \dutysat \forb{\is}{a}
	&\text{ iff } \forall t \subin \is: \rho(\tau,t) \neq \acta. \label{fis}\\
%	\tau \dutysat \monadop{P}{\is}{a}
%	&\text{ undefined } \label{pis}\\
	\tau \dutysat \phi \sqcup \phi'
	&\text{ iff } \tau \dutysat \phi \sor \tau \dutysat \phi'.\\
	\tau \dutysat \phi \sqcap \phi'
	&\text{ iff } \tau \dutysat \phi \nd \tau \dutysat \phi'.
\end{align}
\end{definition}
...
\end{itemize}
\end{example}
\begin{remark}[Kinds of obligations and prohibitions]
 Note that there are many different ways to capture the idea of obligation and prohibition in ways such as achievement and maintenance related to the timed interpretation as defined in \cite{DBLP:conf/deon/BroersenDDM04,governatori2011justice}: Achievement refers to successfully achieving a task at least once before a specific deadline, like submitting a report on time. Once this task is accomplished, the achievement constraint is relieved. In contrast, maintenance involves consistently upholding a property over a specific \emph{period} of time. It is not enough to do it just to be performed once, a state has to be sustained over time.
...
This shows that distributing a conjunction over a disjunction results in a duplication of $\phi_1$, thereby doubling the number of obligation literals it contributes. As such, each application of the distributivity rule can increase the total number of obligation literals in the formula. 
...
\quad \text{(by merging quantifiers)} \\
...
When specifying time-dependent norms, the choice of how to represent temporal constraints directly impacts the syntactic size and readability of a formula. While punctual representations (i.e., using single time points) offer the finest granularity, they often lead to verbose encodings, especially when applied to longer durations or non-unifiable intervals. Conversely, more structured representations, such as intervals or sets of intervals, can convey the same semantics far more succinctly. This syntactic economy becomes especially valuable when reasoning over large normative systems or when transforming formulas into normal forms. The following remark highlights this advantage of conciseness.

\begin{remark}[Conciseness of Interval Sets in Literal Representation]
\label{obs:interval-conciseness}

The way literals represent time constraints directly determines the overall size and readability of formulas. Thanks to the compression equivalences (\textbf{ObDis}) and (\textbf{FConj}) in Lemma~\ref{lemma:literal-compression}, multiple intervals involving the same action can be merged into a single literal whose interval set is a union of the original intervals. This reduces the syntactic overhead and improves overall readability, which facilitates the subjects of the normative system to get a more straightforward interpretation.

Consider, for example, the literal $\forb{{[2,4],[8,24]}}{\delAtGate}$, which expresses a prohibition over two time ranges. As shown in \ref{table:ma-conciseness}, this specification can be written in multiple ways, with varying levels of granularity. We compare three syntactic alternatives: using single time points, using individual intervals, and using a set of intervals. The table highlights that the interval set representation is by far the most concise, requiring fewer symbols to express the same semantics.
...
\begin{example}\label{satsolver}
    Below, we show the results when applying our solver to the  sub-formulas from our use case in \ref{ex:interface}. The figure demonstrates the solver's interface for evaluating the satisfiability of \TDDLfm formulas involving timed obligations and prohibitions. For the satisfiable scenarios $\UC_1'$ and $\UC$, the tool successfully generates a valid minimum-length timed word, such as executing a specific action at timestamp 15. In contrast, it correctly identifies $\UC_2'$ as an unsatisfiable formula, indicating that the specified constraints contain a conflict that prevents any valid execution.
    \begin{figure}[htb]
    ...
    \#Use case $\UC$\\
     Enter a formula from \TDDLfm: (O \delAtGate \{[9,16]\} $\mid\mid$ O \delAtPark \{[9,16]\} ) \& F~\delAtGate \{[10,14]\} \& F~\delAtGate \{[2,4],[8,24]\}\\
        ...
        Event 0: Action = \delAtGate, Timestamp = 15
    \end{tcolorbox}
    \caption{Interface of the Solver with examples}
    \label{ex:interface}
  \end{figure}
\end{example}
...
While satisfiability checking determines whether a normative specification admits at least one compliant behavior, it falls short in providing explanatory insight. Specifically, it does not reveal which parts of the specification are the root cause for unsatisfiability, nor does it clarify how or why certain norms render the system infeasible. It offers a binary answer, satisfiable or not,without explaining the origins of unsatisfiability or identifying potential resolutions. To overcome this limitation, we now turn to a more detailed analysis of normative conflicts, based on the semantics of \TDDLfm.
...
We now generalize the earlier observed conflict patterns and formalize them as inference rules.  
These rules form a \emph{conflict calculus}, enabling systematic exploration of formulas and selective conflict removal.

\medskip

\noindent
\ref{syncnotation} summarizes the syntactic notations of our deductive system, grounded in the extensional semantics of \TDDLfm.  
...
$\vdash \phi$ & $\phi$ is valid & $\dutyclass{\phi} = \domtr$ \\ \hline
...
They lift the multiple punctual conflicts into larger reasoning steps, allowing one to handle clusters of conflicting intervals in a single inference.  
These big-step rules cover two fundamental forms of normative tension: \emph{deontic} and \emph{ontic} conflicts.  
Each lemma specifies the structural conditions under which such compound conflicts arise and how they can be resolved or equivalently rewritten.
...
\noindent
Case (i) expresses a \defn{total deontic conflict}: When all time points of an obligation are included within the prohibition time points, then the conjunction is unsatisfiable. 
Case (ii) captures a \defn{partial deontic conflict}, this cases illustrate the situation when not all time points of an obligation are included within prohibition time points, then only the included time points are removed, and the resulting rewriting is conflict-free.

\begin{proof}

For $\ob{\is}{a} \sqcap \forb{\is'}{a}$:
\textbf{(i) T-DeoConf.}  
 When $\is \subseteq \is'$,  by the semantic definition of \TDDLfm we have a quantifier clash, thus no trace can satisfy both, hence the conjunction is unsatisfiable.

\smallskip
\textbf{(ii) P-DeoConf.}  
When $\is\cap\is'\neq\emptyset$ and $\is\not\subseteq\is'$, by the semantics definition, one can encode the quantified formula for each literal as a constraint and prune the conflicting time points.
\end{proof}
...
\noindent
This rule captures the situation in which more actions are required than the available time points allow for simultaneous fulfillment.  
We intentionally omit creating a partial ontic conflict rule, as discussed earlier in \ref{table:ontic-triple-conflict} , due to combinatorial explosion.
...
This resulting encoding corresponds to the pigeonhole principle~\cite{Dirichlet1834Vorlesungen,ajtai1994complexity}, at least one action cannot be filled within the timeline specified by the interval sets.
\end{proof}
We now move to construct a proof system that supports a mechanized, proof-oriented study of conflicts in our logic.
\subsection{Conflict Calculus}

The conflict calculus in \ref{fig:conflict-calculus} admits eight inference rules derived from  previously proven lemmas in Section~\ref{syntaxandsemantics}.  
Together, they form a structured system for detecting, rewriting, and resolving normative conflicts.

\begin{itemize}
\item \textbf{(O-Union)} and \textbf{(F-Union)} correspond to the literal compression Lemma~\ref{lemma:literal-compression}
\item \textbf{($\bm{\bot\sqcap}$)} and \textbf{($\bm{\bot\sqcup}$)} streamline the construction of a proof of unsatisfiability in conjunction and disjunction of two formulas.
\item \textbf{($\bm{\sqcup\bot}$)} prunes the unsatisfiable formula in its disjunction with another one.
\item \textbf{($\bm{\bot\equiv}$)}  proves unsatisfiability through equivalence.
\item \textbf{($\bm{\equiv\sqcap}$)} is a substituation in a conjunction through equivalence.
\item \textbf{(Distr1L)} represents the distributivity rule.
\item \textbf{(T-DeoConf)} and \textbf{(P-DeoConf)} respectively conclude unsatisfiability or prune punctual deontic conflicts as in Lemma~\ref{Gdeontic-conflict}.
\item \textbf{(OntConf)} derives unsatifiability when obligations according to Lemma~\ref{lemma:ontic-conflict}.
\end{itemize}
...
\[
\begin{array}{c}
\textbf{(O-Union)} \quad \displaystyle \frac{  }{ \ob{\is}{a} \sqcup \ob{\is'}{a} \dutyequiv \ob{\is \cup \is'}{a} } 
\\[3ex]

\textbf{(F-Union)} \quad \displaystyle \frac{ }{ \forb{\is}{a} \sqcap \forb{\is'}{a} \dutyequiv \forb{\is \cup \is'}{a} } 
\\[3ex]

\textbf{(Distr1L)} \quad \displaystyle \frac{~}{ \phi \sqcap (\phi' \sqcup \phi'') \dutyequiv (\phi \sqcap \phi') \sqcup (\phi \sqcap \phi'') } 
\\[3ex]

\begin{array}{rlcl}
\textbf{($\bm{\equiv\sqcap}$)} & \displaystyle \frac{ \phi \equiv \phi' }{ \phi \sqcap \phi'' \equiv \phi' \sqcap \phi'' } 
& \quad
...
\textbf{(OntConf)} \quad \displaystyle \frac{ \size{\is} <_\infty \size{A} }{ \bigsqcap_{a\in A} \ob{\is}{a} \dutymodel \bot } 
\\[3ex]
...
\medskip
\noindent
The conflict calculus for \TDDLfm captures all types of deontic conflicts and only total ontic ones. It is designed with a set of simple rules to isolate the minimal sources of unsatisfiability and eliminate timed normative conflicts. The designer may navigate logical equivalence bidirectionally. This formalism not only identifies unsatisfiability and conflicts but also provides a structured way to resolve them through systematic refinement.
 We plan to encode the entire framework in a theorem prover such as Isabelle \cite{wenzel2008isabelle}, Lean \cite{de2015lean}, or Rocq \cite{bertot2013interactive} as future work.
...
\noindent \textbf{Answer:} We can always resolve conflicts while preserving the intended \emph{compliant behaviors} (semantics). However, we cannot guarantee \emph{structural compactness}; conflict elimination may increase the size of the specification, necessitating a post-processing refactoring step.
        ...
        \begin{tikzpicture}[
          ...
          rect/.style={rectangle, draw, rounded corners, text width=42mm, align=center, minimum height=11mm},
          ell/.style={ellipse, draw, minimum width=32mm, minimum height=16mm, align=center},
          ...
          \node[manual, below left=5mm and 8mm of smt] (calc) {Conflict\\Calculus};
          \node[auto, below right=5mm and 8mm of smt] (det) {Conflict Detection\\(\dpnf)};
          \node[auto, below=10mm of det] (elim) {Conflict Elimination};
          \node[res, right=15mm of det] (pc) {$\mathcall{PC}(\phi)$};
          \node[res, below right=5mm and 8mm of det] (cdens) {$\cd(\phi)$};
          ...
          \draw[arr] (det) -- (elim);
          \draw[arr] (det) -- (cdens);
          \draw[arr] (elim.south) -- ++(0,-0.7) -| (ref.east);
          \draw[arr] (calc) to[bend left=20] (ref);
          \draw[arr] (ref) to[bend left=20] (calc);

          % Legend (positioned under the Refined node and centered horizontally)
          \begin{scope}[shift={(spec |- ref.south)}, yshift=-10mm, xshift=-27.5mm]
            \node[draw, rounded corners, fill=white, minimum width=55mm, minimum height=34mm, anchor=north west] (legbox) at (0,0) {};
            \node[draw, rectangle, fill=ConflictGreen!20, minimum width=5mm, minimum height=2.5mm] at (5mm, -6mm) {};
            \node[anchor=west] at (11mm, -6mm) {Specification};
            
            \node[draw, rectangle, fill=orange!20, minimum width=5mm, minimum height=2.5mm] at (5mm, -13mm) {};
            \node[anchor=west] at (11mm, -13mm) {Analysis Results};
            
            \node[draw, rectangle, fill=blue!10, minimum width=5mm, minimum height=2.5mm] at (5mm, -20mm) {};
            \node[anchor=west] at (11mm, -20mm) {Automatic Procedure};
            
            \node[draw, rectangle, fill=red!15, minimum width=5mm, minimum height=2.5mm] at (5mm, -27mm) {};
            \node[anchor=west] at (11mm, -27mm) {Manual Procedure};
          \end{scope}
        \end{tikzpicture}
        }
        ...
        Ultimately, the framework produces a refined normative specification $\phi'$ that may be conflict-free and, ideally, more amenable to implementation or automated reasoning. The conciseness ratio $\frac{\size{\phi'}}{\size{\phi}}$ evaluates the trade-off between conflict elimination and formula size. In scenarios where full normalization introduces syntactic blow-up, this ratio provides helpful guidance for designers balancing readability and conflict elimination.
        
        

        \section{Related Work}

To our knowledge, our framework is the first to offer a systematic approach that studies metric-time deontic logic with action-based semantics and interval-set representations to compactly encode temporal constraints. This enables a syntactic and semantic study of timed normative conflicts using algorithmic conflict detection and elimination, a proof-theoretic conflict calculus, and quantitative measures of conflict density and conciseness. We now proceed to discuss related work.

The closest work to our approach is Colombo and Governatori~\cite{colombo2014detecting}. In their work, conflicts are characterized as unsatisfiable or non-compliant formulas. These formulas are formed with untimed literals over actions and states, while the models are timed traces where events capture \begin{inparaenum}[(i)] \item the action performed by the agent, \item the states of the system, and \item the triggers and deadlines of the untimed literals\end{inparaenum}. Additionally, their logic relies on specification assumptions to encode action incompatibilities (for example, that going to a pub is incompatible with submitting a research paper). Our framework, on the other hand, focuses on \begin{inparaenum}[(i)] \item actions only, \item uses literals tagged with timed constraints, \item characterizes conflicts as Minimal Unsatisfiable Subsets (MUS) of a normal form, and \item defines ontic conflicts (the equivalent of achievement obligations) using the axiom that actions are never achievable at the same time points (single action per event)\end{inparaenum}. This intrinsic treatment of metric intervals allows purely semantic detection without auxiliary declarations about actions. Nevertheless, our framework could relax the single-action assumption by introducing a feasibility predicate that defines which actions are co-executable, thereby bridging their semantic traces to our interval semantics.

Tamargo et al.~\cite{DBLP:journals/flap/TamargoMRG21} study normative systems as dynamic systems where rules are introduced or updated and norms are timed. They use a belief-revision operator $\otimes$ that trims sub-intervals leading to contradictions in the updated normative system. This method partially removes conflicts while preserving as much temporal structure as possible. Our approach differs in that it handles both deontic and ontic conflicts under a uniform action-based semantics, treats the entire specification rather than single insertions, and reasons at the level of interval sets rather than single intervals, thereby supporting concise rewritings after elimination. Moreover, our conflict calculus provides a formal proof framework that explains each elimination step.

Pace and Schapachnik~\cite{pace2020general} focus on another type of ontic conflicts, namely \emph{environmental conflicts} in logical time. This generalizes our ontic conflicts by formalizing more infeasible combinations of required actions due to external constraints (e.g., an agent cannot be in different places at the same time). Their work uses a dynamic, operational approach to detect both deontic and environmental constraints as they materialize over time.

Another perspective on conflict detection is presented in~\cite{10.1145/3597503.3639093}, which models normative specifications as a set of rules that can express deadlines over time points, conditionals, and exceptions (defeaters). They distinguish mainly two types of conflicts: (i) \emph{vacuous} conflicts, where the involved rules can never be jointly satisfied, and (ii) \emph{situational} conflicts, which only arise under specific triggering conditions. While their specification language \tool{SLEEC} is more expressive than \TDDLfm, their conflict specification is close to our deontic conflicts. However, we specify a granularity at all time points where a literal is in force, and we also consider ontic conflicts. To detect conflicts, they encode the types of conflicts as unsatisfiability conditions and translate the normative specification into first-order logic with relational objects. They then apply a causality check to find the root causes of the unsatisfiability of a formula by eliminating unnecessary rules from the proof. In our approach, we go further: even if the formula is unsatisfiable, we trim the unsatisfiable part of the rules.

In summary, these cited frameworks focus on more expressive norms that are dynamic due to triggers and exceptions that are difficult to detect statically. On the other hand, our work establishes both an axiomatic and a semantic characterization of conflicts, which we study through a variety of methods, such as SAT solving, conflict-preserving rewriting, proof systems, and conflict elimination procedures.
% \section{Conclusion}
...
This chapter presented \TDDLfm, a minimal metric-time normative logic in which normative specifications can be expressed over timed conditions as interval sets for obligation, prohibition, and their combination via disjunction and conjunction. The core backbone of this work is the formal pipeline for \emph{explainable} conflict analysis: with syntactic normalization to disjunctive/punctual forms, a sound QF-LIA encoding for satisfiability, a syntactic conflict-detection procedure, and a proof-theoretic conflict calculus. 



 

By identifying the correspondence between punctual conflicts and Minimal Unsatisfiable Subsets (MUS) and by introducing a proof-theoretic conflict calculus, we linked the syntactic and semantic properties of conflicts, which was straightforward at the beginning of this work. This correspondence is novel in the field of normative conflicts and precisely distinguishes the notion of unsatisfiability from normative conflicts.



Beyond only detecting conflict, we also contributed to conflict engineering by: (i) defining a \emph{conflict-density} measure  to  reflect the severity  of conflicts in a formula and proving a \emph{pruning theorem} that proves that any partially conflicting specification has a semantically equivalent, conflict-free refinement; and (ii) establishing that \TDDLfm\ satisfiability is NP-complete with a tight QF-LIA encoding bounded by the number of obligations, along with explicit size blow-up bounds for conflict detection via \dpnf.



We established that the complete static conflict elimination often causes a substantial syntactic blow-up. Especially when using the \dpnf repair algorithm. This suggests that forcing a specification to be entirely conflict-free at design time may sacrifice conciseness and readability. In practice, regulators might prefer to keep compact specifications and defer resolving some conflicts to the agent's operational planning at runtime. Our conflict calculus is an interactive way for choosing which conflict to keep or eliminate. We also avoided defining rules for eliminating partial ontic conflicts, as they lead to instances of large-scale blow-up.
 ...
 Our methodology could be applied to existing logics lacking frameworks for reasoning about conflicts. More specifically, agent-centered timed deontic formalisms. The core principles of this framework provide a systematic, reusable foundation for conflict reasoning, verifiable repair, and compliance monitoring across diverse normative  and ontic domains.





We plan to enrich \TDDLfm gradually in two directions: (1) \emph{expressivity}: actions with duration and ought-to-be literals (defined on states), a reparation operator, conditional/permission literals, and a defeasibility layer; and (2) \emph{interaction}: frames for prevention, enabling, and interference between actions, and relaxing the single-action ontic constraint. We expect the second direction to require substantial methodological changes to the full logic specified in \cite{kharraz2021timed}, as it requires additional structural properties on the underlying frames used to model the ontic properties of the enriched logic.



\paragraph{From Static Specifications to Dynamic Collaboration.} 

Our study so far has focused on the time consistency of a specification from a single agent's perspective. However, real-world digital contracts govern interactions among multiple potentially adversarial parties. Even if a specification is statically conflict-free, an agent could still fail to comply at runtime due to interference or inaction by a collaborator. Having secured the static foundations of metric time norms, the next chapter turns attention to the dynamic lifecycle of contracts. We will explore how to monitor collaborative compliance at runtime and, when breaches occur, how to precisely assign blame among interacting agents.
