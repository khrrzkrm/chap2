...
\begin{itemize}
\item \textbf{(O-Union)} and \textbf{(F-Union)} correspond to the literal compression Lemma~{lemma:literal-compression}
...
\medskip
\noindent
The conflict calculus for \TDDLfm captures all types of deontic conflicts and only total ontic ones. It is designed with a set of simple rules to isolate the minimal sources of unsatisfiability and eliminate timed normative conflicts. The user may navigate through logical equivalence in any direction he wants. This formalism not only identifies unsatisfiability and conflicts but also provides a structured way to resolve them through systematic refinement.
...
To our knowledge, our framework is the first to offer a systematic approach that studies metric-time deontic logic with action-based semantics with interval-set representation to compactly encode temporal constraints to offer syntactic and semantic study of timed normative conflicts by using algorithmic conflict detection and elimination, a proof-theoretic conflict calculus, and quantitative measures of conflict density and conciseness. We now proceed to a discussion of related work.


The closest work to our approach is Colombo and Governatori~\cite{colombo2014detecting}: 
Conflicts are first characterized as unsatisfiable/non-compliant formulas, where the formulas are formed with untimed literals over action and states, and the models are timed traces where events capture \begin{inparaenum}[(i)]
 \item the action performed by the agent, \item states of a system, and  \item the triggers and deadlines of the untimed literals \end{inparaenum}. Additionally, the logic relies on the specification assumptions to encode action incompatibilities, such as the fact that going to a pub is incompatible with submitting a research paper. Our framework, on the other hand, focuses on \begin{inparaenum}[(i)] \item actions only, \item uses literals tagged with timed constraints, \item characterizes conflicts as Minimal unsatisfiable subsets of a normal form, and \item Our definition of ontic conflicts, the equivalent of achievement achievement, rely on an axiom are never achivievable at the same time points (single action per event).\end{inparaenum} Nevertheless, we could relax this assumption by introducing a feasibility predicate that defines which actions are co-executable, in contrast to  which ones are non-co-executable, in order to encode real-life settings for continuous actions.  
This intrinsic treatment of metric intervals allows purely semantic detection without auxiliary declarations about actions.  
Nevertheless, our framework could relax the single-action assumption by introducing a feasibility predicate that defines which actions are co-executable, thereby bridging their semantic traces to our interval semantics.

Tamargo et al.~\cite{DBLP:journals/flap/TamargoMRG21} study normative systems as dynamic systems in which rules are introduced or updated, and norms are timed. In this work, they use a belief-revision operator $\otimes$ that trims sub-intervals that lead to contradictions in the updated normative system. Their method partially removes conflicts while preserving as much temporal structure as possible.  
Our approach differs in that it handles both deontic and ontic conflicts under a uniform action-based semantics, treats the entire specification rather than single insertions, and reasons at the level of interval sets rather than single intervals, thereby supporting concise rewritings after elimination.  Moreover, our conflict calculus provides a formal proof framework that explains each elimination step.

Pace and Schapachnik~\cite{pace2020general} focus on another type of ontic conflicts, namely \emph{environmental conflicts} in logical time, which generalizes our ontic conflicts by formalizing more infeasible combinations of required actions due to external constraints, such as an agent cannot be in different places at the same time. The work uses a dynamic, operational approach to detect both deontic and environmental constraints as they may materialize over time.
...
Another perspective on conflict detection is presented in~\cite{10.1145/3597503.3639093}, which models normative specifications as a set of rules that can express deadlines over time points, conditionals, and exceptions (defeaters). They distinguish mainly two types of conflicts: (i) \emph{vacuous} conflicts, where the involved rules can never be jointly satisfied, and (ii) \emph{situational} conflicts, which only arise under specific triggering conditions. While their specification language \tool{SLEEC} is more expressive than \TDDLfm, their conflicts specification is close to our deontic conflicts, but we specify a granularity at all time points where a literal is in force, and we also consider ontic conflicts. To detect conflicts, they encode the types of conflicts as unsatisfiability conditions and the normative specification into first-order logic with relational objects, and apply a causality check to find the root causes of the unsatisfiability of a formula by eliminating unnecessary rules for the proof. In our approach, we go further: even if the formula is unsatisfiable, we trim the unsatisfiable part of the rules.

In summary, these cited frameworks focus on more expressive norms that are dynamic due to triggers and exceptions that are difficult to detect statically. On the other hand, our work established both an axiomatic and a semantic characterization of conflicts, which we study through a variety of methods, such as SAT solving, conflict-preserving rewriting, proof systems, and conflict elimination procedures.
% \section{Conclusion}
...
We found out that the complete static conflict elimination often causes a significant syntactic blow-up. Especially when using the \dpnf repair algorithm. This suggests that forcing a specification to be entirely conflict-free at design time may sacrifice conciseness and readability. In practice, regulators might prefer to keep compact specifications and defer resolving some conflicts to the agent's operational planning at runtime. Our conflict calculus is an interactive way for choosing which conflict to keep or eliminate. We also avoided defining rules for eliminating partial ontic conflicts, as they lead to instances of large-scale blow-up.
