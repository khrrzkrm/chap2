...
Fix the tagged collaboration alphabet 
...
For downstream use (e.g., compliance checking),
we collapse the five tight judgements into a two-valued view:
\[
\begin{array}{rcl}
\pi\ \sat\ C
&\iff&
...
Given a single event word $\trace{A}$ with $A\in\Gamma$, 
the tight semantics of literals for party~$p=1$ are:
\[
\begin{array}{l@{\quad}c@{\quad}l@{\qquad\qquad}l@{\quad}c@{\quad}l}
\multicolumn{3}{c}{\textbf{Tight satisfaction on } \trace{A}} &
\multicolumn{3}{c}{\textbf{Tight violation on } \trace{A}}\\[2mm]

\trace{A}\ \satt\ \top      &\mydef& \text{true}. &
\trace{A}\ \violt\ \top      &\mydef& \text{false}.
\\[4pt]

\trace{A}\ \satt\ \bot      &\mydef& \text{false}. &
\trace{A}\ \violt\ \bot      &\mydef& \text{true}.
\\[6pt]
...
\trace{A}\ \violt\ \frb[1]{a} &\mydef& \{a^{(1)},a^{(2)}\}\subseteq A.
\\[6pt]

\trace{A}\ \satt\ \perm[1]{a} &\mydef& (a^{(1)}\in A)\Rightarrow(a^{(2)}\in A). &
\trace{A}\ \violt\ \perm[1]{a} &\mydef& (a^{(1)}\in A)\wedge(a^{(2)}\notin A).
\end{array}
\]
...
\trace{A'}\ \satt\ \frb[2]{a}\quad\text{(no joint $a^{(1)}{+}a^{(2)}$ occurs).}
\]
...
Consider $\trace{A,A'}$.
Since literals decide at the first letter,
the overall collapsed verdict follows from $\trace{A}$:
\[
\begin{array}{l}
...
parallel, sequential, or conditional behaviour.  
In \cDL\ we use
\[
...
For conjunction, the decisive point for satisfaction is the latter of the two individual successes;
for violation, the first conjunct that fails.
For sequencing, a split index $k$ witnesses that $C$ succeeds before $C'$ is checked.
Reparation requires, on the other hand, that either $C$ succeeds directly,
or at the first tight violation of $C$ the repair $C'$ must succeed on the remainder.
...
\emph{Conjunction} succeeds once both parts succeed (possibly at different times);
its decisive index is the latter of the two.  
It fails as soon as either part fails.  
\emph{Sequence} requires a witness split $k$:
first $C$ succeeds on $[0,k]$, then $C'$ on $[k{+}1,s]$.
\emph{Reparation} allows $C'$ to take over at the first violation of $C$;
overall success means either direct success of $C$
...
\paragraph{Tight satisfaction (longest prefix).}
Consider the trace
\[
...
\paragraph{Tight satisfaction (shortest prefix).}
...
satisfies either immediately when both conjuncts hold,
or later when a reparation compensates a missed rent,
while violation arises when both payment and its repair fail.
\end{example}
...
A violation occurs either when the first occurrence of $C$ fails,
or when some later repetition cannot be fulfilled after a previously satisfied segment.
Hence, $C^n$ behaves as a \emph{sequential chain} of obligations,
and any broken link invalidates the entire chain.
...
a trace violates $\repit{C}$ once it violates one of its finite unfoldings $C^n$.
...
Contracts guarded by regular expressions specify that a normative condition 
becomes active only after the trace matches a given regular pattern.  
...
Clause~$C_4$ specifies that when the tenant requests a repair,
the landlord must perform it within the following period:
\[
...
Coherence requires that for any fixed contract and trace there is never more
than one decisive verdict. A trace cannot both tightly satisfy and tightly
violate the same contract on different prefixes, since this would yield two
incompatible outcomes for a single execution. Forward semantics must therefore
rule out situations where tight satisfaction appears on one prefix and tight
violation appears on another prefix of the same trace. Ensuring this exclusion
...
$\trace{A}\ \violt\ \ell$ iff it does not. These are complements on that step,
so the two implications are immediate and uniqueness follows.
...
Since a tight violation of a conjunct is a tight violation of the conjunction,
...
(IH-$C$-viol) or (IH-$C'$-viol) no earlier prefix tightly satisfies that conjunct,
hence no earlier prefix tightly satisfies the conjunction.
...
The dual implication follows by (IH-$C$-viol) and (IH-$C'$-viol).
...
$\pi^{k+1}$. In the first branch, (IH-$C$-sat) excludes earlier violations.
...
before its tight success. The dual implication is symmetric, using
(IH-$C$-viol) and (IH-$C'$-viol).
...
While $\pi$ \emph{closes} $re$ (that is, $\pi$ is in the open region for $re$),
any tight or post failure of $C$ yields a tight or post failure of the composite.
...
\medskip
All constructors preserve the two “no-backtrack” properties, hence the claim for all $C$.
\end{proof}
...
satisfaction monitor. The construction is a function
that maps each contract $C$ to a tight satisfaction monitor that enforces. The construction proceeds by structural induction on
the syntax of $C$, and each operator in \cDL is matched by a corresponding monitor combination operator. Regular expression guards and triggers rely on the
...
The literal clauses above define one-step satisfaction and violation
judgments for a single event word~$\trace{A}$.
We lift these clauses into a five-valued Moore machine
whose outputs track the evolution of the tight verdicts over prefixes.
...
\path[->]
  ...
  (qps) edge[loop right] node {*} (qps)
  (qpv) edge[loop right] node {*} (qpv);
\end{tikzpicture}
\caption{Compact 5-verdict Moore machine for the obligation literal
$\obl[1]{a}$. The symbol $*$ stand for any $A \subseteq 2^\Sigma$.
Each node displays its internal state and the corresponding output
verdict $\in\tightverdicts$.
...
$\textsf{op} \in \{\wedge,\ ;\ ,\ \repair\}$, the monitor for the composite
contract is obtained by combining the already constructed monitors
...
The monitor for $C \wedge C'$ checks both components at the same time
and decides the global verdict according to the following rules:
...
\begin{itemize}
  ...
  \item A tight violation in one component produces $\bott$ whenever no permanent
...
post-satisfaction, and post-violation for the contract $C \wedge C'$. The monitor
therefore implements the tight semantics of conjunction correctly.
\qed
...
suffix. The construction below reuses the state spaces of both component
monitors and redirects transitions that correspond to tight success of $C$
into the initial state of $C'$. This yields a tight prefix monitor that exactly
matches the semantics of sequential composition.
...
= (Q_{;}, q_0^{;}, \Gamma, \tightverdicts, \delta_{;}, \lambda_5^{;})$. For every trace $\pi$, the monitor output the same verdict according to the sequence semantics:
\[
...
\smallskip
\emph{Violation case.}
If $C$ violates before any tight success, the redirection never occurs
and the global verdict is exactly the violation of $C$,
matching $\pi\ \violt\ C;C'$.
...
\noindent
\textbf{Intuition.}
The reparation operator activates the secondary contract $C'$ after the primary
contract $C$ reaches a tight failure. The construction mirrors the sequential
case, except that the redirection applies to the transitions that lead to a
tight violation of $C$ rather than to tight satisfaction.
...
As long as no violation occurs, the monitor behaves exactly as $C$. Once a tight
failure is detected, the remainder of the input is processed by $C'$.
...
\smallskip
\emph{Inductive step.}
Assume correctness for prefixes up to length $n$ and consider the next letter
$A$. If $\lambda_5(\delta(x,A))=\bott$, then
...
If no tight violation occurs, $\delta_{\repair}$ behaves exactly as $\delta$,
so the inductive hypothesis applies.

\smallskip
\emph{Violation case.}
If $C$ reaches a tight failure and $C'$ later fails on the suffix,
the monitor outputs $\botp$, exactly as prescribed by the semantics.
...
$m \in \{\mathbf{L}, \mathbf{R}\}$, with $\mathbf{L}$ for left-hand side and $\mathbf{R}$ for the right, 
to determine which component is active,
our transformation achieves the same effect directly on the transition graph:
deleting the terminal states of the left monitor
and redirecting the transitions that reach a decisive verdict
($\topt$ for sequence, $\bott$ for reparation)
to the initial state of the right monitor.
This compile-time construction encodes the same operational behavior
as a mode-augmented monitor that switches from $\mathbf{L}$ to $\mathbf{R}$
when the switching condition is met.

This idea parallels the phase-based runtime semantics
proposed in the RV literature~\cite{falcone2009runtime,bartocci2018rv},
...
and the secondary reparation $\obl[1]{\PAYF}$ to pay a late fee
if the first is violated.
...
We construct $C_2 \wedge C_3$ by the synchronous product of the 5-output monitors
for $C_2=\perm[2]{\OCC}$ and $C_3=\obl[1]{\PAY}\repair\obl[1]{\PAYF}$, then keep only
...
Reading Fig.~\ref{fig:c2andc3} with the letter classes defined above:
from $s_0$ (pre) there are three behaviors.
(i) If \,$\OCC^\surd \land \PAY^\surd$\ holds in the current letter, both conjuncts succeed
(perm is respected and the primary obligation is met), so the product emits $\topt$
...
Thus, the decisive index for the conjunction is the latter of the two successes when both
succeed, or the earlier tight failure when any conjunct fails, exactly as the figure shows.
...
reaches its first tight match. Before that point, the monitor behaves exactly
as the regular-expression monitor for $re$ and emits only $\mathsf{?}$. Once
the trigger fires, the monitor switches permanently to the contract monitor
$\tsmc(C)$. If the pattern becomes impossible before it fires, the contract is
vacuously satisfied. The construction follows the same blueprint as sequence
and reparation, but applied to the decisive states of the regular-expression
monitor.
...
\paragraph{Intuition.}
The trigger monitor is obtained with the same redirection recipe used for sequence, but applied to the trigger. Keep only states that are still open in the trigger (outputs in \(\{\,\mathsf{?},\topt,\topp\,\}\)). Remove its decisive states. Redirect every transition that would be the first tight match of the triggering regular expression (the step that enters \(\topt\)) to the initial state of the contract monitor \(C\). From that point on, the global output is exactly the output of \(C\) on the suffix. Redirect every transition that would make the guard impossible (enter \(\bott\) or \(\botp\)) to a vacuous-success sink that emits \(\topt\) once then \(\topp\). While the guard remains open, only the guard component advances and the product emits \(\mathsf{?}\), so no premature verdict appears. This realizes the prefix clauses: success either because the trigger never becomes true which we refer to as vacuous satisfaction, or because it fires at the earliest index and the suffix satisfies \(C\); violation only if the guard fires and the suffix violates \(C\).
...
\noindent
\textbf{Proof sketch.}
The proof follows the same pattern as for sequence and reparation, by induction
on the length of $\pi$.
...
\]
coincides with the tight semantics of $\trigg[re]{C}$ on the empty trace:
no trigger has fired, and no violation has occurred.
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    guard remains open and the global verdict stays $\mathsf{?}$. This matches
    the semantic clause that no decisive information is available as long as
    neither a match nor impossibility has been detected.
  \end{itemize}
  The inductive hypothesis on the guard monitor ensures that the moment of
  redirection coincides with the earliest decisive prefix of $re$.

  \item \emph{Contract-active region ($y\in Q_c$).}
  Once the monitor has been redirected into $c_0$, all transitions are given
  by $\delta_c$, and outputs by $\lambda_5^c$. The induction hypothesis for
  $\tsmc(C)$ gives
  \[
  \lambda_5^{\trigg}\bigl(\delta_{\trigg}(q_0^{\trigg},\pi)\bigr)
...
\smallskip
\emph{Conclusion.}
At each prefix, the monitor is either simulating the guard with the correct
decisive redirection points, or simulating $C$ on the correct suffix. Hence
...
The resulting case table is symmetric and total, and it collapses to the expected
two-valued clauses once tight/post outcomes are merged into satisfied vs.\ violated.
...
\begin{itemize}
  ...
  The guarded semantics requires that any tight or post failure of $C$ becomes
  the global failure.  
  ...
  The guarded semantics declares vacuous acceptance provided that $C$ has not
  already failed.  
...
sequence. The monitor construction follows the intuition that each repetition
runs an independent copy of the monitor for $C$, with the next copy becoming
active exactly when the previous one reaches tight success. For finite
...
\medskip
\noindent
This completes the construction of the five-valued monitor
for $\nrep(n,C)$.
\end{definition}
...
In the next sections we use the different languages defintion in the semantics of \cdl and the automata constructions and transformations to enable automatic detection of violations and attribution of blame for synchronous interactions over contracts in \cDL.
...
We illustrate the constructions for the guarded open-ended contract
\(\guard[re]{\repit{C_3}}\) where
\(re = *^{+};\{\notifterm^{(1)}\}\) and
\(C_3=\obl[1]{\PAY}\repair\obl[1]{\PAYF}\).
...
The monitor contains the states
\(q_0/\mathsf{?}\), \(q_w/\mathsf{?}\), \(q_{\bott}/\bott\), and \(q_{\botp}/\botp\).
...
The monitor has states
\(s_0/\mathsf{?}\), \(s_1/\mathsf{?}\), \(s^t/\topt\), and \(s^{+}/\topp\).
...
\(s_1\times q_w\),
and \(s_1\times q_0\).
Their transitions follow the product rule
\((x,y)\xrightarrow{A}(\delta_r(x,A),\delta_c(y,A))\).
Examples include:
\[
\begin{aligned}
...
% --- Transitions ---
\path
  (s0) edge node[above,pos=0.4] {$*$} (g)
  (g) edge[loop below] node[below] {$\overline{T}$} ()
  (g) edge node[above] {$T$} (acc)
%   (t2) edge node[left,pos=0.5] {$*$} (acc)
  (acc) edge node[above] {$*$} (ap)
  (ap) edge[loop above] node {$*$} ();

% --- Legend ---
\node[below=10mm of g,align=center,font=\scriptsize] (leg){
$T := \{\,A\in\Gamma\mid \notifterm^{(1)}\in A\,\}$,\;
$\overline{T} := \Gamma\!\setminus\! T$,\;
$*$ = any letter in $\Gamma$.
};
\end{tikzpicture}
\caption{Monitor for the regular expression  $*^+\ ; \ \{terminate^{(1)}\}$.}
\label{fig:guard-}
\end{subfigure}
\vspace{3mm}
...
\path
 ...
 % (Oqw) edge[bend left=12] node[left]{\scriptsize $\PAYF^\surd$} (Oq0)
  (Oqv) edge              node[left] {$*$} (Oqpv)
  (Oqpv) edge[loop left]  node {$*$} ();
...
% --- Imposs region loops ---
\path
 % (Iq0t) edge[loop right] node {$*$} ()
  (Iqwt) edge[loop right] node {$*$} ()
  (Oqq00) edge[loop right] node {\scriptsize$\PAY^\surd \wedge \overline{T}$} ();

\end{tikzpicture}
\caption{Tight satisfaction monitor construction for $\guard[*^+\ ; \ \{terminate^{(1)}\}]{\repit{C_3}}$.}
\label{fig:guard-repC3}
\end{subfigure}
\caption{Progressive construction for  $\guard[(*^+ ; \notifterm^{(1)})]{\repit{C_3}}$.}
...
The blame goes to the agent responsible for the first violation of the contract: so either C or C', but both contract could be violated on the same time point, in this case the agent or agents responsible for \emph{both simultaneous} violation gets the blame.
...
\subsection{Conclusion and Limitation}      
          ...
          Consequently, any subsequent violations The next lemma makes this coherence property explicit.committed by other agents at later time steps are effectively masked by the first failure.
