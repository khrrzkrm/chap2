...
\title{Chapter 2: On Monitoring compliance to contract for }
\author{kharrazkaram }
\date{September 2025}
...
In many real settings, however, compliance is not an individual achievement.
It is the outcome of an interaction in which each party can enable, ignore, or obstruct the other.
This chapter studies such \emph{collaborative} normative settings, where agents are not assumed to play a zero-sum game, yet strategic behavior and conflicting incentives remain possible.
We focus on two agents, since the core difficulties already arise in the simplest bilateral contracts.

\paragraph{Interactions and collaboration.}
Standard deontic formalisms describe duties and prohibitions of a single agent in isolation.
By contrast, many norms presuppose that a desired outcome materializes only when both parties contribute.
For instance, a tenant can attempt to pay rent, but successful payment may depend on the landlord providing a payment channel or accepting the transfer.
Similarly, occupancy is meaningful only if one party grants access while the other actually occupies.
A useful model must therefore distinguish between (i) \emph{attempts} that each agent performs and (ii) \emph{successful collaboration} that emerges from a compatible combination of attempts.
It must also account for \emph{non-interference}, where compliance requires one party to refrain from actions that would prevent the other from fulfilling its duties.

\paragraph{Open-ended normative interactions.}
Normative specifications s may specify long terms collaborations.
...
This motivates a trace model that supports (a) aggregation at the level at which the norm is written (such as months) and (b) prefix-based evaluation, since monitoring and dispute resolution rely on partial observations of an ongoing relationship.

\paragraph{Blame and accountability.}
When a collaborative norm is violated, it is not enough to say that the contract is violated.
One must explain \emph{who} is responsible and \emph{why}.
This is equally important in contractual disputes and in regulatory settings, where enforcement decisions require an explicit account of causation and justification.
In bilateral scenarios such as landlord--tenant disputes, courts typically reconstruct a timeline and ask ``who blocked whom'': did the tenant fail to pay while still being entitled to possession, did the landlord deny access without following the legally required steps (for example notice, deadlines, and, where applicable, a court order), or did the landlord deny access first so that rent withholding or reduction could be justified and proportionate.
Often, both parties contribute to escalation, and the resolution hinges on allocating remedies to the harms caused at each step, such as restoring access, ordering arrears payment, granting rent reduction for unusable periods, awarding damages for proven losses caused by denial of access, and checking whether termination and eviction procedures were complied with.
Beyond this qualitative attribution, practical compliance analysis also needs a quantitative view of \emph{how much} non-compliance occurred, for instance how many months rent was not paid, how often access was denied, or how many repair obligations were missed.
A blame notion that is useful for compliance tooling must therefore be formally defined, operationally checkable, and robust under incremental observation of the interaction.
This chapter develops such a notion alongside executable monitors, and then extends it with quantitative scores that go beyond the common ``first-failure-only'' perspective.


\paragraph{Research questions.}
These considerations lead to the following questions from a formal method perspective:
\begin{enumerate}
  \item How can we model collaboration as a first-class semantic object, separating individual attempts from jointly successful outcomes?
  \item Which trace abstractions capture both cooperative behavior and non-interference at the granularity induced by contractual periods?
  \item How can we specify and monitor open-ended, potentially infinite, streams of duties and right in a contract setting?
  \item How can we define blame and quantitative accountability in a way that is more faithful to the real life legal systems reasoning and supports automated monitoring?
\end{enumerate}


\section{Methodology}

To address the research questions outlined above, we adopt a constructive \emph{formal methods} methodology.
Our approach is predicated on the rigorous separation of \emph{specification} (what the contract means) from \emph{implementation} (how compliance is checked).
This separation ensures that our monitoring tools are not merely ad-hoc scripts, but correct-by-construction artifacts derived from a principled logical foundation.

The research is structured into four distinct methodological phases, progressing from conceptual modeling to operational deployment:

\paragraph{Phase 1: Domain Abstraction and Trace Modeling.}
The first phase involves creating a mathematical model of the domain that captures the nuances of collaborative behavior.
Standard trace models (sequences of atomic events) are insufficient for distinguishing between a party's unwillingness to act and their inability to act due to obstruction.
\emph{Methodologically}, we address this by elevating ``attempts'' and ``outcomes'' to first-class semantic objects.
We define a trace algebra that models interactions not as simple events, but as the resolution of conflicting or cooperating individual strategies.
This abstraction forms the bedrock of our analysis, allowing us to reason formally about non-interference.

\paragraph{Phase 2: Semantics-First Specification.}
In the second phase, we define the normative requirements declaratively.
We introduce the \emph{Two-Agents Collaborative Normative Logic} (\cDL) to capture the temporal and structural complexities of real-world contracts.
A key methodological choice here is to define the \emph{denotational semantics} of the logic before considering its execution.
We first develop a family of \emph{Forward Tight Semantics} that maps finite interaction prefixes to definitive verdicts.
This baseline semantics evaluates whether a contract has been successfully terminated without violation.
We develop a foundational family of \emph{Forward Tight Semantics} that maps finite interaction prefixes to definitive verdicts regarding contract compliance.
This initial formalism determines whether a contract has been satisfied without violation.
From there, we derive a \emph{blame semantics} to attribute responsibility for the initial breach.
However, to overcome the limitation of the ``first-failure'' of the forward tight semantics, we extend the framework to a \emph{quantitative violation semantics} that accumulates violations and blame over time, persisting even after the first violation is detected.
This entire semantic layer provides the theoretical ``ground truth'' for compliance, defined independently of implementation constraints.
Furthermore, we establish the formal relationships between these semantics, specifically demonstrating their mutual consistency.

\paragraph{Phase 3: Automata-Theoretic Operationalization.}
The third phase bridges the gap between theory and practice by translating the declarative logic into executable artifacts.
We employ an automata-theoretic approach, mapping the formulas of \cDL to deterministic Moore machines.
Unlike the abstract semantics, which describe truth, these machines describe computation.
Our methodology focuses on the structural synthesis of these monitors, ensuring that every logical operator (e.g., sequence, reparation) has a direct, constructive counterpart in the universe of automata transformations.

\paragraph{Phase 4: Verification and Refinement.}
The final phase validates the pipeline.
We do not treat the generated monitors as black boxes; rather, we provide formal proofs of \emph{conformance}, demonstrating that the outputs of the automata are isomorphic to the semantic verdicts defined in Phase 2.
Once this binary compliance baseline is established, we refine the model to address accountability.
We extend the semantics to propagate ``blame'' and ``quantitative scores,'' ensuring that the resulting analysis provides the granular detail required for legal and regulatory dispute resolution.

\medskip
\noindent
By following this methodology, we ensure that the resulting compliance monitors are both legally more expressive and mathematically sound.
...
Unlike approaches that define monitoring behavior through ad-hoc operational rules, we adopt a \emph{semantics-first philosophy}.
We postulate that the \emph{meaning} of compliance (what constitutes a violation) must be defined abstractly and independently of the \emph{mechanism} used to detect it.

Consequently, the primary contribution of this chapter is the development of a verified pipeline that bridges the gap between high-level normative abstractions and low-level executable monitors.
We ensure that the operational behavior of the generated monitors is conceptually sound with respect to the underlying legal intuition.

The specific theoretical and technical contributions are structured as follows:

\begin{enumerate}[label={(Con\arabic*)}]
  \item \textbf{A Conceptual Distinction Between Attempt and Outcome.}
  We introduce a novel interaction model that distinguishes between an agent's individual \emph{attempt} to act and the actual \emph{successful outcome}.
  Standard models often obscure the difference between a failure to act (laziness) and a failure caused by the other party's obstruction.
  By treating these as separate layers, we can model non-interference and ``active participation'' as first-class concepts.
  This allows us to verify not just whether an action occurred, but whether an agent did everything in their power to make it occur.

  \item \textbf{The Two-Agents Collaborative Normative Logic (\cDL).}
  We formalize \cDL, a contract logic designed explicitly for bilateral, periodic interaction.
  Existing logics often struggle with the nuances of long-running contracts.
  Our logic integrates complex temporal triggers (defining when obligations activate) and contrary-to-duty structures (defining what happens when obligations are violated).
  Crucially, we define the semantics of this logic first, providing a ground truth for compliance that exists independently of any software tool.

  \item \textbf{A Theory of Timely Verdicts (Tight Semantics).}
  A major theoretical contribution is the resolution of the ``infinite wait'' problem in monitoring.
  Standard theoretical models often delay verdicts indefinitely, waiting for an infinite future to determine compliance.
  We introduce a \emph{tight semantics} that identifies the earliest possible moment a contract is satisfied or violated.
  This ensures that our definitions of compliance are actionable in real-time, providing immediate feedback rather than theoretical eventualities.

  \item \textbf{Correct-by-Construction Monitor Synthesis.}
  We provide a constructive method to compile any contract written in \cDL{} into an executable monitor.
  Rather than writing monitoring code by hand, which is error-prone, we derive the monitor automatically from the logic.
  We provide structural proofs demonstrating that this generated monitor is \emph{operationally equivalent} to the abstract logic.
  This guarantees that the software is correct by design, eliminating the need to verify the monitoring tool itself.

  \item \textbf{Rich Accountability: Blame and Quantification.}
  Finally, we extend the framework to support legal reasoning beyond simple binary compliance.
  We introduce a notion of \emph{blame} that refines generic violations into agent-specific responsibilities, determining exactly who caused a failure.
  Furthermore, we introduce a \emph{quantitative} dimension to monitoring.
  Instead of stopping at the first error, our approach allows for the dynamic counting of cumulative violations over time, essential for calculating penalties or damages in open-ended contracts.
\end{enumerate}
...
\begin{itemize}
  ...
  \[
  C_5 := \guard[\,\Gamma^+\,;\,\{\notifterm^{(1)}\}\,;\,\Gamma^{3}\,] (\ C_3\ \wedge\ C_2\ \wedge\ C_4\ ).
  \]
\end{itemize}
